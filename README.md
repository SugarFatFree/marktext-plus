...
| **`control`** | 14 actions: open, close, save and switch tabs, write content, change the view mode, close a pane, run a plugin command, open a plugin's panel, change a setting, reopen a tab you closed, move and resize the window — **and install a plugin or replace the application with a build from CI** |
