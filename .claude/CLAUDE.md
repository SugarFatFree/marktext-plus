...
槽位名对应位置：`right`＝右上、`bottom`＝左下、`corner`＝右下。

**只填一个窗格时，方向由槽位名决定**：填 `bottom` 是上下分割，填别的是左右
（默认）。这一条 2026-09-07 由人工测试改过——原先是「一律左右，一个窗格就是
一个窗格，不管它自称什么」。那个理由对**哪一个**窗格成立，对**哪个方向**不
成立：改写一段话要放在它下面比较，同样的行宽、眼睛往下扫，而不是挤成两条窄
栏往旁边看。官方插件的 AI 写作与 AI 纠错因此用 `bottom`，翻译仍用 `right`。

**三宫格下读者还能自己切**：窗格标题栏有一个上下互换的按钮（`swap_vert`），
把分割的那一半换到另一边。插件的选择只决定**初始**形态；哪一半被分割是一种
看法，不是插件可以一直攥着的决定。

**二宫格也有这个按钮**（在左右与上下之间翻转），理由同上。四宫格两半都已分割，
没有可翻的，所以只有那一种形态不画它——一个按了没反应的按钮比没有按钮更糟。
...
3. **一份清单对外宣称，另一份是实现，没人比较它们。** 这是第 1 条的对外版本，
   2026-09-08 一天之内命中六次，差额全是用户会撞上的：

   | 对外的清单 | 实现的清单 | 后果 |
   |-----------|-----------|------|
   | MCP `control` 的 8 个动作 | switch 里的 6 个 | 两个动作全代码库只存在于那个枚举里 |
   ...
   **修法不是把差额逐条补上**——那样下次还漏。**把两份合成一份，让编译器或
   测试对账**：动作做成 enum，schema 从 `values` 生成，switch 穷尽（加一个不实现
   就编译不过）；做不成 enum 的写一条两个方向都查的测试（如
   `readme_dependency_count_test` 从 pubspec 数出来再去核每份文档）。

   **判别信号**：某个名字 `grep` 全库只出现一两处，且都在「声明」类文件里
   （schema、默认配置表、i18n 显示名、README）。
