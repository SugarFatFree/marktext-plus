...
5. **README.md 已更新**——功能表、版本引用、截图要反映本版真实能力
   （历史教训：README 从 v1.1.2 之后连续多版没更新，功能表停在 8 行，
   Mermaid / 命令面板 / 导出 / 大纲 等一概没写）

   **图片存不存在不用你查**：`readme_images_exist_test` 会盯着 12 份文档引用的
   每一张图（它们至今在 `docs/v1.1.2/picture/`，正是容易被顺手清理掉的位置）。
   **要你判断的是内容**：那八张主题截图拍于 v1.1.2，之后加了右侧边栏、插件窗格、
   MCP——界面是否已经不像它们了？功能表列的是**能力类别**，不是每版的改进，
   所以一条修复通常不需要动它；改动的是新增或去掉了一整类能力的时候
