...
- **Choosing to overwrite a file that changed on disk could fail without saying so.** The banner cleared, the other version stayed on disk, and nothing suggested the write had been refused — which reads exactly like success. It now says why, and the conflict stays until it is genuinely resolved. Picking a different encoding for a file that has since become unreadable says why too, instead of leaving the label where it was
