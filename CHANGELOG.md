...
- **The outline listed headings the preview does not draw.** A document explaining markdown shows ``` inside a ```` block — this one's own README does — and the outline ended the block on the inner fence, so the prose underneath was read as markdown. From the first disagreement on, every outline entry scrolled to the wrong heading. The rule for what closes a code block is written out five times across the editor; two of the five had drifted, and nothing tied them together. Now they are tied together
...
- **Copying a paragraph that contains a formula gave plain text.** So did one with a picture, a footnote marker, an exponent or a ruby annotation: the preview draws each of those as a widget, which occupies exactly one position in the text and cannot be selected letter by letter, and rich copy was looking for the letters instead. It never found the selection, fell through to plain text, and took every heading, bold run and link in it along. Inline code did the same thing, for a different reason — see below
- **Choosing to overwrite, or to reload, a file that changed on disk could fail without saying so.** The banner cleared, the other version stayed on disk, and nothing suggested the write had been refused — which reads exactly like success. It now says why, and the conflict stays until it is genuinely resolved. Reloading is the same: the banner stayed by itself, which said the conflict was unresolved without saying that the file could not be read. Picking a different encoding for a file that has since become unreadable says why too, instead of leaving the label where it was
- **A line of `<!--` could lock the source pane up for twelve seconds.** Without a closing `-->` anywhere on the line, the comment rule scanned to the end of the line from every opener before giving up. It now asks once whether the line closes a comment at all, which it does in a single pass — twenty thousand openers went from twelve seconds to six milliseconds, and a comment still gets its colour
- **The same rule was written a third time, in the paste path.** Pasting HTML from a browser strips its comments first, and that copy had the shape too — with no line to bound it, since a paste arrives whole. Ten thousand unclosed openers took most of a second, forty thousand would have taken thirteen, and the conversion happens while the paste is waiting. It no longer builds a pattern at all: scanning forward from each opener is linear whatever is on the clipboard, which is not a thing the editor gets to choose
- The line below that one had the same shape — the one that drops `<script>` and `<style>` out of a paste — and the same cost: ten thousand unclosed `<style>` took a second. It is linear now, and an element left unclosed still does not hide the closed one after it
- **A gantt chart with a line of colons in it took five and a half seconds.** Working out which colon separates a task's name from its definition copied the rest of the line for every colon on it — a task name is allowed to contain one, so every colon has to be tried. It reads the line in place now: the same line takes seven milliseconds. Twenty of the twenty-two diagram types had never been handed a line of noise at all; all of them have now, against the same table of samples that checks each type can be drawn, so a type added later cannot quietly skip either
- **A line of `[^` could lock the editor up for seconds.** The footnote pattern let its label run to the end of the line and then hand it back a character at a time from every starting point, so ten thousand of them took two and a half seconds to parse — and the source pane, which learned the same pattern this release, took three and a half. It is linear now: twice the input, twice the time
- **Quitting could cut itself short.** A watchdog is armed when the window starts closing, so that a close which hangs does not leave you waiting — it gives up after six hundred milliseconds and exits. Nothing called it off once the close had finished normally, so it stayed armed through the rest of the shutdown, and whatever was still being done past that point was at its mercy
- **A plugin that will not read now says so.** One with a mistake in its manifest was skipped without a word, and a plugin missing from the list looks exactly like one that was never installed — so the reader installs it again, and again nothing appears. The manifest reader has always refused these with a sentence naming the key and what it expected; that sentence went into a `catch` and reached nobody. It is on the plugins page now, under the ones that did load
- **One mistyped value in the settings file reset every setting.** The configuration is JSON in a directory you can open, and a single `"yes"` where `true` belonged threw the whole file out: theme, fonts, key bindings, recent files, the tabs you had open. Each field now falls back on its own, and the rest of the file survives. A list holding something that is not a path drops that entry instead of failing later, somewhere with no visible connection to it
- **Pasting from a web page dropped the marking.** `<mark>`, `<u>`, `<sup>` and `<sub>` all arrived as bare words — a footnote reference from Wikipedia, a highlighted passage from anywhere — although this editor writes all four of them itself and has markdown for each. It exported `<mark>` and then could not read `<mark>`. A fragment copied without a surrounding paragraph, which is what the clipboard holds when you select a single word, lost `<code>` the same way
- **`==marked==` exported to Word as plain text**, with no ground behind it at all — while the exponent, the underline and the struck-out run beside it came through. It uses Word's own highlighter now, which is what marking text means there. (The library writes a run's properties only when it thinks the run has some, and its list of what counts left out background fill: a run whose only property was the ground was written without any properties, and the ground with them. Marked text that also carried, say, bold came through fine — the more formatting it had, the better it survived.)
- Copying from the source pane ignored the inline-HTML switch. With it off — the default — `<b>x</b>` was literal text in the preview and arrived in Word as bold, so which of the two you got depended on the pane you had selected in, and neither on what you had asked for
- The two spaces that kept the ground off `inline code` were content, not decoration: they were selected with it, copied with it, and counted in every offset — while the search branch beside them never added any, so opening the find bar changed the length of the paragraph. The ground sits against the letters now, the way it already did whenever the find bar was open
- Inline maths and footnote markers are coloured as you type them too — `$E = mc^2$` in the colour code gets, because in a pane showing markdown a formula is the same kind of thing, and `[^1]` in the colour the preview raises it in. A line about money is still a line about money: `$` opens nothing when a space follows it
...
- **A plugin could make network requests without asking for permission.** `network.request` was declared, shown to you, and checked in one of the two places a plugin can reach the network — the other was the host fetching a picture on the plugin's behalf, and `![](http://…)` inside a plugin's markdown went around the permission, the log and your proxy all at once. A plugin granted `ui.webview` needs the network to show anything, and that was not implied anywhere, so a plugin declaring it honestly still could not load a page
- **A plugin package could unpack to gigabytes.** Nothing bounded the size of a plugin ZIP or of what came out of it. A downloaded plugin's address must be HTTPS now, which only half the download paths enforced
- **A compiled plugin with no build for your machine installed anyway**, and said so when you clicked one of its commands. Whether it runs here is in its manifest and can be read before installing
- **Remote pictures and the update check ignored your system proxy.** Everything the editor fetches goes through it now
- **A plugin that will not read had no way to be removed.** The one plugin you most want to delete — the one showing an error — was the one with no delete button, since the button was drawn from a manifest that had failed to load. Community search failures showed you `HttpException:` and the class name with it
- Plugin settings showed only the first line of a multi-line value, which for a prompt template is the least useful line of it. A plugin's own icon was ignored in the side bar. A long permission sentence ran off the edge of its list. A floating card could only be dragged inside the document pane. A plugin opened from the right side bar could not ask you anything, which for several of them is the whole interaction


- **An 8.6 MB document took four seconds to put anything on screen.** A large file is parsed in two goes — enough of the top to draw something, then the rest — and how much counted as "the top" was measured in lines. A document with few lines and a great deal on each of them, which is what an exported page or a generated report looks like, had almost all of itself in that first pass. It is measured in bytes now: the same document draws its first frame in 94 ms
- **Typing paused for a sixth of a second in a long document, every time you stopped.** The word count walked the whole text on the interface thread. It runs off it now, and so does the outline, which had the same shape — the comment beside the word count said so, and only the word count had been moved
- **A list with an empty item swallowed everything after it.** `- one`, a bare `-`, then `- two`: from the empty marker on, the rest of the list was drawn a level deeper than it was written, because an item with nothing in it has no content to measure the indentation of
- `\$5` in the source pane was coloured as the start of a formula, and `\*not emphasis\*` as emphasis. A backslash escapes the character after it — the preview has always known that; the colouring did not
- **The preview did not update for anything that was not typing.** A plugin writing back its result, a file reloaded after changing on disk, an edit arriving over the automation interface: the preview kept a widget it had built once and never asked for a new one. It also affected "Reload images" (F5), which did nothing at all, and changing the code font or size, which the preview ignored — the cached block's signature did not include any of them
- Four diagram types redrew nothing when their data changed. Comparing two diagrams compared their data lists with `==`, which for two different lists holding the same numbers is false, and for the same list mutated in place is true
- Reloading images kept the pictures it replaced. Each generation was held under its own key, and a picture can be eight megabytes; every press of F5 added one and let go of nothing

- **Eleven keyboard shortcuts were drawn in the menus and did nothing.** Zoom in, zoom out, reset zoom, typewriter mode, full screen, print, export PDF, settings, new window, quit and reload images could all be rebound in Settings, all showed their key beside the menu entry, and none of them answered when that key was pressed. Flutter draws a menu item's shortcut without handling it, and the handler that makes them work listed fourteen actions and stopped; the keybinding table, that handler and the menu bodies were three lists and only the first was complete. They are one list now, so an action cannot be offered for binding without something carrying it out
- **A rebound shortcut showed the new key and kept the old one working.** Matching a keypress goes through an index built once and kept, and three places drop it — rebinding, resetting to defaults, loading from disk. Nothing checked any of them: changing Bold to Ctrl+Shift+B showed the new key in the menu, did nothing when pressed, and left Ctrl+B bolding
- **The command palette listed nine commands beyond formatting, out of twenty-five.** Zoom, print, export, full screen, settings, new window, quit, reload images, close tab and find next or previous could not be found in it at all — someone searching "zoom" would reasonably conclude the editor cannot. Every command with a shortcut is in it now, each showing the key it is currently bound to
- **Ticking a checkbox in the preview half of a split.** It worked, and it modified editor state from inside a widget lifecycle, which Flutter's state library refuses in a debug build. It also meant no test of editing from a split preview could be written — the defect was keeping its own guard out. Undo now reaches the tick, and the status bar follows the caret
- **Turning inline HTML on or off did not reach the preview until the next keystroke.** The parsed document is kept against the text it was parsed from, and that setting is the one thing that changes how the same text is read
- **"Check for updates" could say you were on the latest version without having asked anyone.** Offline, the check failed and the answer was the cheerful one. It says the check failed now — including when an earlier check had found a version, since what you asked about is this one
...
- **Guards inside the plugin were not guarding.** `lua_dardo` treats a bare `return` inside a nested function as doing nothing, so `if not ok then return end` continued into the failure it was checking for, and `while true do return end` was an infinite loop — which is one way a plugin freezes the editor
- Its bundled copy of the SDK was five options behind the published one
...
### Added
- **Answer an AI agent over MCP.** Off unless you turn it on in Settings: it opens a port on your machine that lets whatever reaches it read your documents and drive your editor, so it is opt-in and carries a token. An agent can read the log — the editor's and its plugins' — take a screenshot, record up to five seconds as a GIF to look at an animation, ask what is open, and drive the editor. Port 10100, walking upward if it is taken; the configuration to paste into your agent is shown filled in
- **Plugins can put panes around the document.** The editor could already divide a tab between source and preview, and that division is offered out: one pane sits beside the document at half each, two leave the bottom row whole, three make four equal cells. The split view's own two halves count as two of them. The dividers drag, and with three cells the plugin suggests which half is divided and you can move it
- **A right side bar**, its own thing like the left one — a rail of plugin icons and a drawer — and nothing at all until a plugin contributes a panel to it
...
- Windows on ARM builds, native rather than emulated, alongside the x64 ones. This had been recorded as impossible because Flutter publishes no arm64 Windows SDK, which is still true — but the arm64 Dart and engine binaries have been published separately since 3.44.0, and an arm64 runner fetches them once the stamp pinning the x64 one is out of the way. What the build actually produced is read out of the executable's machine type, because a build that succeeded is not proof it built the thing asked for

### Fixed
- **Opening a large file.** A 739 KB document took seven seconds; five and a half of those were syntax highlighting, whose limit was set at two megabytes — a size no document anyone opens ever reaches. It gives up at 128 KB now, the last size that still opens in about a second: 256 KB went from 8 seconds to 0.2, and 512 KB from 45 seconds to 0.3. Anything under the limit is unchanged, the document stays fully editable either way, and deleting back under the limit brings the colours back
- **The room under the last line** is the same in both panes, and is under the text rather than taken off the pane. The source pane's was added as padding inside the text field, which lengthens what can be scrolled and shortens the visible box by the same amount: in a 900-pixel window the source pane had 659 pixels of text and 241 of white at the bottom, whatever you scrolled to
- **The Format menu no longer edits the first line** while you are looking at the preview
