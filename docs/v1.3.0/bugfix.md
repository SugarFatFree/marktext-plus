...
| BUG-004 | 2026-08-27 | Mermaid 缺失 erDiagram / journey / gitGraph / mindmap / quadrantChart 等类型 | P1 | erDiagram、journey、gitGraph、mindmap、quadrantChart、requirementDiagram、sankey-beta 已完成；block-beta / C4Context 见 FEAT-008 |
...
| BUG-067 | 2026-08-27 | 窗口菜单三项全是死的，「最小化」调的还是退出通道 | P1 | 已修复 |
| BUG-068 | 2026-08-27 | 「检查更新」只是打开网页；网络失败会被谎报为「已是最新」 | P2 | 已修复 |
| BUG-069 | 2026-08-27 | 目录把代码块里的 `#` 注释当成标题，且与预览的标题编号错位 | P1 | 已修复 |
| BUG-070 | 2026-08-27 | `- [X]` 不算任务、`1)` 不算有序列表，但编辑器却认这两种写法 | P1 | 已修复 |
| BUG-071 | 2026-08-27 | `***粗斜体***` 渲染成粗体加一个多余的星号 | P1 | 已修复 |
| BUG-072 | 2026-08-27 | `2 * 3 * 4` 里的乘号被当成斜体标记 | P1 | 已修复 |
| BUG-073 | 2026-08-27 | `~~~` 围栏不识别；```` 围栏内的 ``` 会把内容切碎丢失 | **P0** | 已修复 |
| BUG-074 | 2026-08-27 | 双反引号行内代码被拆碎：``` ``a`` and ``b`` ``` 变成三段 | P1 | 已修复 |
| BUG-075 | 2026-08-27 | 硬换行的标记残留在正文里：反斜杠可见、尾随空格进入导出 | P2 | 已修复 |
| BUG-076 | 2026-08-27 | 表格：省略外侧竖线整个不认；单元格里的 `\|` 会把一格拆成两格 | P1 | 已修复 |
| BUG-077 | 2026-08-27 | 引用块里的列表、标题、代码块全部退化成纯文本 | P1 | 已修复 |
| BUG-078 | 2026-08-27 | 编号列表下挂项目符号子项会被吞进上一项；嵌套编号不重新计数 | P1 | 已修复 |
| BUG-079 | 2026-08-27 | 链接不支持尖括号包裹的路径与单引号标题，写了就散成乱码 | P2 | 已修复 |
| BUG-080 | 2026-08-27 | README 徽章（图片作为链接）解析错乱成三段 | P1 | 已修复 |
...
| BUG-087 | 2026-08-27 | 标题末尾的闭合井号被当成正文显示 | P2 | 已修复 |
...
| 修复方案 | 按使用频率排期补齐：**erDiagram（已完成）** → **journey（已完成）** → **gitGraph（已完成）** → **mindmap（已完成）** → **quadrantChart（已完成）** → **requirementDiagram（已完成）** → **sankey-beta（已完成）** → block-beta → C4Context。<br>erDiagram 实现要点：`ArrowType` 新增四个 crow's foot 记号（`erZeroOrOne` / `erExactlyOne` / `erZeroOrMore` / `erOneOrMore`），十六种基数组合由左右两个两字符 token 分别映射；`ErDiagramLayout` 与类图同法继承 `DagreLayout` 只覆写节点测量，因为实体框高度取决于属性行数 |
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | 「窗口」菜单下的**最小化 / 全屏 / 置顶三项全都没有反应** |
| 根因分析 | 全屏与置顶的实现是**空函数体**，注释写着「not available without window_manager」—— 但 `window_manager` **本来就是项目依赖**（`main.dart` 与 `home_screen.dart` 一直在用），`minimize()`、`setFullScreen()`、`setAlwaysOnTop()` 全都现成。注释所说的理由是不成立的<br>而「最小化」更糟：它调的是 `SystemChannels.platform.invokeMethod('SystemNavigator.pop')` —— 这是**请求退出当前路由**的通道，在桌面端等同于关闭应用。一个写着「最小化」却可能把应用关掉的菜单项，比什么都不做更危险 |
| 修复方案 | 三项都改用 `window_manager` 的对应方法。全屏与置顶**先向窗口查询当前状态再取反**，这样即使用户用系统快捷键在菜单外改过，下一次点击仍然是对的，而不是反的。菜单上显示对勾 |
| 涉及文件 | `lib/ui/widgets/app_menu_bar.dart`、`lib/providers/window_provider.dart`（新增） |

---

## BUG-068 「检查更新」不检查，还会把失败说成「已是最新」
...
| 优先级 | P2 |
| 状态 | 已修复 |
| 现象 | 帮助菜单里的「检查更新」只是**在浏览器里打开发布页**，并不检查 |
| 根因分析 | 应用其实有 `UpdateService.checkForUpdate`，启动时会自动调用；菜单项却只挂了一个 `_launchUrl` |
| 改的过程中发现的更严重问题 | `checkForUpdate` 把**所有异常都吞掉并返回 null**，于是「没有新版本」和「根本没查成」两种结果**完全无法区分**。如果照这样接上菜单，网络不通时就会明确告诉用户「已经是最新版本」—— 这是在撒谎 |
| 修复方案 | ① `checkForUpdate` 改为返回 `({UpdateInfo? update, bool reachable})`，把「查到了什么」和「查成没有」分开；② 启动时的自动检查**继续对失败保持沉默**（这是对的，不该拿网络问题打扰用户），只有用户主动点的检查才报告三种结果之一：有新版本 / 已是最新 / 没能连上 |
| 涉及文件 | `lib/services/update_service.dart`、`lib/ui/widgets/app_menu_bar.dart`、`lib/ui/screens/home_screen.dart`、`lib/core/i18n/l10n/*` |

---

## BUG-069 目录把代码块里的注释当成标题
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | 文档里只要有 shell / Python 代码块，侧边栏目录就会多出一堆条目（`# install deps` 之类）；点击**正常**的目录条目还会跳到不相干的位置 |
| 根因分析 | 目录面板与预览各自**逐行**用 `^(#{1,6})\s+(.+)$` 找标题，**都不识别代码围栏**。而 `MarkdownParser` 本身是先解析代码块的，所以渲染出来的标题比目录列出的少 |
| 为什么会跳错位置 | 预览是把**第 N 个标题控件**对应到目录的**第 N 条**。围栏里多认出来的那些条目会把后面每一条都**整体挪位**，于是点「Section」跳到的其实是渲染结果里的第 3 个标题 |
| 还有一处两边不一致 | 预览会剥掉 BOM 再匹配，目录面板不会。文件带 BOM 时，第一行的 `# 标题` 在预览里认得、在目录里认不得 —— 又是一次编号错位 |
| 顺带 | 目录面板把 `RegExp(...)` 写在**循环体内**，文档每一行都重新编译一次正则 |
| 修复方案 | 抽出 `MarkdownParser.headingOutline`，返回 `(行号, 层级, 文本)`，两处共用。围栏内的行不计；BOM 统一剥离；正则提为静态字段 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`lib/ui/widgets/side_bar.dart`、`lib/ui/editor/markdown_renderer.dart`、`test/services/markdown_parser_test.dart` |
| 实测对比 | 一份含 bash 与 python 代码块的示例文档：旧实现报出 **5 个标题**（含 `install deps`、`not a heading` 两个注释），新实现 **3 个**，与实际渲染一致 |
| 同源 | 与 BUG-062（四份图表语言清单）、BUG-064（六条文档读写路径）同型：**同一件事有多份实现，迟早会漂移**。这次同样是把出口收敛成一个函数 |

---

## BUG-070 解析器不认 `[X]` 与 `1)`，编辑器却认
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | ① 写成 `- [X] done`（大写 X）的任务**不显示复选框**，而是原样显示 `[X] done`；② 写成 `1) one` 的有序列表**根本不成列表**，退化成一个普通段落 |
| 根因分析 | 这两种写法都是规范允许的 —— GFM 规定 `[x]` 与 `[X]` 等价，CommonMark 规定有序列表可以用 `.` 也可以用 `)`。而项目里**同一件事有两份实现**：`SourceEditor._listPrefixRe` 写的是 `(?:[-*+]\s+(?:\[[ xX]\]\s+)?|\d+[.)]\s+)`，**两种写法都认**；`MarkdownParser` 写的是 `^\[( |x)\]` 与 `^\s*\d+\.\s+`，**两种都不认** |
| 后果 | 编辑器把 `- [X] item` 当成任务行来做缩进和标记切换，预览却把它当普通文本渲染。用户看到的是「Tab 缩进正常、但复选框不见了」这种自相矛盾的行为 |
| 修复方案 | 解析器的两条正则对齐到规范：任务标记 `^\[([ xX])\]`，勾选判定按**大小写不敏感**比较；有序列表 `^\s*\d+[.)]\s+` |
| 涉及文件 | `lib/services/markdown_parser.dart`、`test/services/markdown_parser_test.dart` |
| 验证方式 | 本地 11 条断言覆盖两组写法及其否定情形（`1.no space`、`a) x`、`- [y]` 都不应成为列表/任务）；仓库内单测 4 条，另确认导出侧（HTML 复选框、PDF/Word 的 ☑/☐）读的是模型字段，会自动跟随 |
| 同源 | 本版本第五次遇到「同一行为多份实现」：图表语言清单 4 份（BUG-062）、文档读写 11 处（BUG-064）、标题识别 2 份（BUG-069），加上这次的列表语法 2 份 |

---

## BUG-071 `***粗斜体***` 渲染成粗体加一个多余的星号
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | `***bold italic***` 渲染成**粗体的 `*bold italic`**，后面还多挂一个星号；`___bold italic___` 渲染成粗体的 `_bold italic_`，下划线原样显示 |
| 根因分析 | 行内正则里**根本没有 `***` 分支**。`\*\*(.+?)\*\*` 匹配 `***x***` 时，先吃掉前两个星号，惰性组取到 `*x`，再匹配 `**` 收尾 —— 剩下最后一个星号变成普通文本。`___` 同理落进了 `__` 分支 |
| 修复难点 | 正则分支**必须按顺序**：同一位置上，Dart 的选择结构取**先列出**的那一支，所以 `***` 必须排在 `**` 前面。而排在前面就意味着**插入**，插入就会让后面所有分支的捕获组编号整体后移 —— 源码里专门留了注释警告过这件事（本会话早些时候就因为漏改一处 `match.group(7)` 挂过 CI） |
| 如何降低风险 | `markdown_parser.dart` 是**纯 Dart**（只依赖 `dart:convert`），可以脱离 Flutter 直接跑。做法是：① 先用真实解析器对 **42 条语料**（涵盖仓库测试断言过的全部写法）跑出**基线**；② 在副本上插入分支、脚本化平移组号；③ 重跑并与基线 `diff`。结果**只有那两行发生变化**，其余 40 条逐字节一致 —— 编号平移的正确性由行为验证，而不是靠肉眼核对 |
| 修复方案 | 新增 `InlineType.boldItalic`；`***` 与 `___` 各自成为独立分支并排在对应的双标记之前；预览、HTML 导出、PDF 导出、Word 导出四处消费点全部接入（`<strong><em>`、`FontWeight.bold + FontStyle.italic`、`DocxFontWeight.bold + DocxFontStyle.italic`） |
| 涉及文件 | `lib/services/markdown_parser.dart`、`lib/ui/editor/markdown_renderer.dart`、`lib/services/export_service.dart`、`test/services/markdown_parser_test.dart` |
| 遗留项 | 已在 BUG-072 中修复 |
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | 正文里写 `2 * 3 * 4`，中间的 `3` 会**变成斜体**，两个星号消失；`a * b * c`、`foo _ bar _ baz`、`_ spaced _` 同样中招 |
| 根因分析 | 斜体分支写的是 `\*(.+?)\*`，**对分隔符两侧是不是空白毫不关心**。CommonMark 明确规定：紧挨着分隔符内侧是空白时，它既不能开启也不能闭合强调 —— 正是为了让散文里的乘号、脚注星号这些不被误伤 |
| 修复方案 | 两条斜体分支改为 `\*([^\s].*?[^\s]|[^\s])\*` 形式：内容首尾都不能是空白，单字符情形由第二个分支兜住。下划线那条同时保留原有的「词内不算强调」前后视断言 |
| 有意未改 | 粗体 `** x **` 同样不合规范，但本次没动 —— 散文里意外出现成对 `**` 的概率远低于单个 `*`（乘号、脚注标记），而收紧粗体会让一部分「原本能用」的文档失效。风险收益不划算 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`test/services/markdown_parser_test.dart` |
| 验证方式 | 沿用 BUG-071 的做法：语料扩到 **61 条**（新增大量散文里出现星号/下划线的情形），改前建基线、改后 `diff`。结果**只有 6 行变化**，全部是「原本被误判为斜体」→「保持原样文本」，其余 55 条逐字节一致 |
...
| 优先级 | **P0** |
| 状态 | 已修复 |
| 现象 | ① 用 `~~~` 写的代码块**根本不成代码块**，整段当普通正文渲染；② 用四个反引号包住一段含 ``` 的内容（写 Markdown 教程时的标准做法），会被切成**两个空代码块，中间的内容彻底消失** |
| 根因分析 | `_codeFenceRe` 写死为 `^\s*```(\w*)`、`_codeFenceEndRe` 写死为 `^\s*```\s*$` —— 既不认 `~~~`，也**不比较围栏长度**。于是 ```` 开头的块被当作 ``` 开始，内部那行 ``` 被当作结束，剩下的 ```` 又开一个新块 |
| 严重性 | 属于**内容丢失**：用户写在四反引号块里的东西在预览和导出里都不见了，而源码里还在，很难察觉是渲染问题 |
| 修复方案 | ① 开启围栏改为 `(`{3,}|~{3,})`，三个或更多；② 新增 `_closesFence`：闭合围栏必须**字符相同且长度不小于**开启围栏；③ 语言标签由 `(\w*)` 放宽到 `([^`\s]*)`，`objective-c`、`c++` 这类不再被截断 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`test/services/markdown_parser_test.dart`、`test/fixtures/showcase.md` |
| 验证方式 | 15 条断言覆盖两种围栏字符、长短组合、跨字符不闭合、未闭合、空块、语言标签含标点，以及**目录跳过两种围栏**；另用 61 条语料基线对拍，确认此改动**零回归** |
| 顺带 | 这条是用「探针」找出来的 —— 拿 25 种常见 Markdown 写法逐个喂给真实解析器看输出。同一批还暴露了双反引号行内代码、setext 标题尾随井号、裸邮件地址、链接文字含方括号等问题，逐条评估后另行排期 |
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | ① 写 ``` ``含有 ` 反引号的代码`` ``` 时，代码在**第一个内部反引号处被截断**，后半截连同结尾的两个反引号一起当普通文本显示；② 写 ``` ``a`` and ``b`` ``` 会得到**三段代码**（`a`、` and `、`b`），中间那段本是正文 |
| 根因分析 | 行内代码正则是 `` `([^`]+)` ``，**只认单个反引号成对**。而反引号串的意义恰恰在于长度：用两个反引号包起来，正是为了让内容里能出现单个反引号 |
| 修复方案 | 改为 `` (`+)(...)\8(?!`) `` —— 开启用一串反引号，闭合必须是**等长**的一串（`\8` 反向引用），且后面不能再跟反引号。另按 CommonMark 补上「首尾各有一个空格时各去掉一个」，于是 ``` `` ` ``` ``` 得到的是一个纯粹的反引号而不是带空格的 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`test/services/markdown_parser_test.dart` |
| 验证方式 | 13 条断言覆盖单/双/三反引号、内部反引号、相邻两处不粘连、字面反引号、空格裁剪的三种边界（两侧都有才裁、只有一侧不裁、全空格不裁）、未闭合、行首三反引号仍是块级围栏；61 条语料基线对拍**零回归** |
| 关于组号 | 这次新增了一个捕获组，`\8` 反向引用依赖绝对编号，后面所有分支要整体后移一位。仍用「基线 → 改 → diff」验证，本次 diff 为**空**，说明平移完全正确 |

---

## BUG-075 硬换行的标记残留在正文里
...
| 优先级 | P2 |
| 状态 | 已修复 |
| 现象 | 用反斜杠强制换行（`line one\` 换行 `line two`）时，行尾那个**反斜杠原样显示**；用两个空格强制换行时空格看不见，但会**混进复制出来的文本和全部导出结果** |
| 根因分析 | 这两个都是「请在此换行」的标记，本身不该出现在正文里。而本项目的段落**每个换行本来就会换行**，标记的作用已经达成，却没有人把它去掉 |
| 修复方案 | 构建段落时剥掉每行末尾的「两个及以上空格」或「一个反斜杠」。**最后一行不处理** —— 它后面没有内容，谈不上换行请求，用户写的尾随空格应当保留 |
| 不影响回写 | 剥离只作用于解析出来的 `content`；预览里的块编辑读的是 `sourceOfBlock`（按源码位置取原文），所以用户写的标记原样往返，不会被悄悄改掉 |
| 与规范的差异（有意保留） | CommonMark 规定**普通换行**在 HTML 里应渲染成空格而非换行，只有带标记的才是 `<br>`。本项目一律按换行处理 —— 这与源项目 MarkText 的「保留换行」行为一致，改成规范行为反而会让现有文档的排版全部改变。如实记录，不做变更 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`test/services/markdown_parser_test.dart` |
| 验证方式 | 5 条断言含「末行保留尾随空格」与「源码往返不变」两条边界；61 条语料基线对拍**零回归** |
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | ① 省略外侧竖线写的表（`a | b` / `--- | ---` / `1 | 2`）**根本不成表格**，整段当正文渲染；② 单元格里写 `\|` 想表示一个竖线时，这一格**被拆成两格**，还多出一个反斜杠；③ 某行单元格数与表头不符时不做处理，表格排版错位 |
| 根因分析 | ① 行的正则写死 `^\|(.+)\|$`，强制要求首尾都有竖线，而 GFM 明确规定外侧竖线可省；② `_parseCells` 直接 `split('|')`，**完全不认转义**——而 `\|` 是把竖线放进单元格的**唯一**办法；③ 没有 GFM 规定的「按表头列数补齐/截断」 |
| 修复方案 | ① 行与分隔行的正则都允许省略外侧竖线；② `_parseCells` 改为逐字符扫描，遇到 `\|` 输出一个竖线并跳过；③ 数据行按表头列数补空或截断 |
| 放宽后引入的新风险，以及如何堵住 | 外侧竖线可省之后，「一句含竖线的正文 + 一条水平线」也满足了「行 + 分隔行」的形状，会被误判成单列表格。GFM 对此有明确约束：**分隔行的单元格数必须与表头一致**。补上这条检查后，`a | b` 后跟 `---` 正确地解析为 setext 标题 + 正文；顺带也修掉了原先「表头三列、分隔行两列」被当成表格的问题 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`test/services/markdown_parser_test.dart` |
| 验证方式 | 12 种表格写法逐一探查（对齐、单列、空单元格、无数据行、表头含格式、前后空格等）+ 7 种误判风险场景（水平线、setext 标题、列数不符、正文含竖线）；61 条语料基线对拍**零回归** |
| 未改（符合规范） | 单元格里 `` `x|y` `` 这样**代码跨度内的竖线仍会拆格** —— GFM 规定即便在行内代码里，竖线也必须转义。当前行为与规范一致 |

---

## BUG-077 引用块里的块级结构全部丢失
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | 引用别人的一段列表 `> - a` / `> - b`，预览里显示的是**字面的「- a」「- b」两行文字**，不是项目符号；`> # 标题` 显示成字面的「# 标题」；`> ``` ` 包起来的代码块连反引号一起原样显示。PDF、Word、HTML 三种导出同样如此 |
| 根因分析 | `BlockquoteNode` 只有 `content` 和 `inlineSpans` 两个字段 —— 它把引用块**当成一段带格式的文字**，而引用块实际可以装下文档能有的任何东西。四处消费方（预览 + 三种导出）都只渲染 `inlineSpans`，块级标记自然就以字面形式露出来了 |
| 修复方案 | ① `BlockquoteNode` 增加 `children`，解析时把剥掉 `>` 之后的内容**递归解析**一遍；② 预览新增 `_buildQuotedNode` 渲染子块（有意比顶层分派窄：引用块内的块不单独可编辑、不参与标题滚动定位、也不会是图表）；③ HTML 递归调 `nodeToHtml`；④ PDF 递归调 `_nodeToPdfWidgets`；⑤ Word 没有容器元素，所以段落沿用引用样式（左边框 + 缩进 + 底纹），非段落的子块按自身结构加入 |
| 兼容性 | `content` 与 `inlineSpans` 保留不动，任何还在读它们的地方行为不变 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`lib/ui/editor/markdown_renderer.dart`、`lib/services/export_service.dart`、`test/services/markdown_parser_test.dart`、`test/fixtures/showcase.md` |
| 验证方式 | 11 条断言覆盖引用内的列表/标题/代码块/嵌套引用/多段落，以及「普通引用仍是单段落」「content 与 inlineSpans 仍在」两条兼容性断言；61 条语料基线对拍**零回归**；showcase 夹具补了一个含列表、代码块与嵌套引用的引用块 |
| 过程记录 | 中途出过一次事故：解析器的改动只写在了草稿副本里，随后为了跑断言又用真实文件覆盖了草稿，**改动丢失**，而渲染器和导出已经在引用 `children`。是断言脚本编译不过才发现的 —— 若只做静态检查（括号配平之类）不会暴露，会一直推到 CI |

---

## BUG-078 混合嵌套列表：子项被吞、编号不重启
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | ① 在编号步骤下挂项目符号子项（`1. a` / `   - b`），子项**被并进上一项的正文**，显示为「a - b」一行；反过来在项目符号下挂编号子项也一样；② 嵌套的编号列表**不重新计数** —— 本该读作 `1. / 1. 2. / 2.` 的结构在预览里成了 `1. 2. 3. 4.` |
| 根因分析 | ① `_collectListItems` 拿**整个列表一种标记**去找条目，另一种标记的行不被认作新条目，就当成上一条的续行了；② 预览按 `index + 1` 在**整个扁平列表**上计数，也用列表整体的 `ordered` 决定每一项画数字还是圆点 |
| 一处值得注意的现状 | PDF 与 Word 导出**早已**按层级计数（比预览做得对），但同样用列表整体的 `ordered`。也就是说同一份文档，预览、PDF、Word 三处的编号各不相同 |
| 修复方案 | ① `ListItem` 增加 `ordered`，收集与构建都**按每行自己的标记**判定；② 抽出 `MarkdownParser.listMarkers`：按层级计数、回到浅层时清空更深层的计数（否则下一个父项的子列表会接着上一个父项数）、项目符号不占用编号；③ 预览、PDF、Word 三处**统一调用**它；④ HTML 导出按每层自己的标记开 `<ol>`/`<ul>`，原先固定用外层标签，编号步骤下的项目符号子列表会被渲染成数字 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`lib/ui/editor/markdown_renderer.dart`、`lib/services/export_service.dart`、`test/services/markdown_parser_test.dart`、`test/fixtures/showcase.md` |
| 验证方式 | 标记生成 7 条断言（含「两个父项各带子列表」这条会暴露计数不归零的用例）；解析 6 条断言；61 条语料基线对拍**零回归** |
| 同源 | 本版本第六次遇到「同一行为多份实现」。这次直接把标记生成收敛成一个函数，四处共用 |

---

## BUG-079 链接的两种规范写法不被支持
...
| 优先级 | P2 |
| 状态 | 已修复 |
| 现象 | ① `[doc](<my file.md>)` —— 用尖括号包住含空格的路径，是 CommonMark 里**唯一**能这么写的办法，本项目不认，整行散成 `[a](<`、一个半截链接、` b>)` 三段乱码；② `[a](url 't')` 单引号标题同样不认，同样散架 |
| 为什么对本项目尤其要紧 | 这是个**本地** Markdown 编辑器，链接到带空格的本地文件（`[说明](<我的 文档.md>)`）是很自然的写法 |
| 修复方案 | 图片与链接两条分支的目标地址改为 `<...>` 或原有形式二选一，标题改为双引号或单引号二选一。每条分支因此各多两个捕获组 |
| **过程中被基线抓到的回归** | 新增捕获组导致后面所有组号后移 4 位，而行内代码分支用的是**反向引用 `\8`** —— 它按绝对编号指向反引号串那一组，组号一移就指错了地方，于是 `` `code` `` 变成了 `code:"c"` 加一段乱码文本。61 条语料基线对拍**立刻报出这一行差异**；把反向引用改成 `\12` 后重新对拍归零。若只测「我这次想改的链接写法」，这个回归会直接推到 CI 甚至用户手里 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`test/services/markdown_parser_test.dart` |
| 验证方式 | 12 条新断言 + **重跑此前八轮全部断言脚本**（粗斜体、斜体空白、围栏、行内代码、硬换行、表格、引用块、列表标记）全部通过；语料基线零回归 |
| 待办 | 已在 BUG-080 中修复 |

---

## BUG-080 README 徽章解析错乱
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | `[![build](https://img.shields.io/b.svg)](https://ci.example.com)` —— README 顶部徽章的标准写法 —— 被拆成**三段**：一个 alt 为 `![build` 的坏链接、字面的 `](`、再一个链接。打开任何一个正经 README，第一眼看到的就是这堆乱码 |
| 根因分析 | 图片与链接是两条独立分支，外层的 `[...](...)` 会先匹配到内层图片的一部分。模型也是**扁平**的：`InlineSpan` 没有子节点，链接跨度装不下图片跨度 |
| 修复方案 | ① 新增一条**排在图片分支之前**的分支专门匹配这个形状；② `InlineSpan` 增加 `linkHref`，图片自带链接目标而不是靠嵌套；③ 预览中给这类图片套上点击手势并把光标改成手型；④ HTML 导出输出 `<a><img></a>` |
| **两处静默丢字段** | 新增字段后，`_restoreEscapes` 与实体解码处**都会重建 `InlineSpan`**，而它们只逐个复制已知字段 —— 新字段被默默丢掉。表现是「徽章的图片地址对、链接地址却是 null」。两处都补上后才通过。这类 bug 静态检查看不出来：代码合法、编译通过，只有跑数据才暴露 |
| 组号平移 | 新分支带来 5 个捕获组，其后全部后移 5 位，行内代码的反向引用也从 `\12` 改到 `\17` |
| 涉及文件 | `lib/services/markdown_parser.dart`、`lib/ui/editor/markdown_renderer.dart`、`lib/services/export_service.dart`、`test/services/markdown_parser_test.dart`、`test/fixtures/showcase.md` |
| 验证方式 | 14 条断言（含「实体处理后 linkHref 仍在」这条专门守住上述丢字段问题）；**九轮历史断言脚本全量回归**；61 条语料基线零回归 |
| 未做 | PDF 与 Word 导出中徽章仍只呈现为图片，不带超链接 —— 两个导出库对「图片上挂链接」的支持需要另行确认，且纸质/文档场景下超链接价值有限 |
...
| 优先级 | **P0** |
| 状态 | 已修复 |
| 现象 | 流程图是 mermaid 里用得最多的类型，而以下五种标准写法都不对：<br>① `A ----> B`（长箭头）→ 目标节点名变成字面的 `-> B`，**A 节点消失**<br>② `A -- 是 --> B`（标签写在破折号之间）→ **A 节点消失**，标签丢失<br>③ `A <--> B`（双向）→ **A 节点消失**，只画单向<br>④ `A --> B & C`（一对多）→ **C 节点整个不存在**<br>⑤ `A["含空格的标签"]` → 引号被**当成标签内容画出来** |
| 根因分析 | 连线的正则是一串手写的备选项 `(==>\|-->\|---\|...\|---->\|...)`：<br>• `---` 排在 `---->` **前面**，而备选结构取先匹配的那一支，所以长箭头被当成短箭头，剩下的 `-> B` 成了节点名<br>• 里面有个**未转义的点** `-.->`，它会匹配 `-X->` 这样的任意字符<br>• **完全没有**「标签夹在两段线之间」这种形式<br>• **完全没有**双向与 `&` 的处理<br>标签的引号则是从没剥离过 |
| 修复方案 | 连线改为一条结构化的正则：可选的近端箭头 + 线段（`-{2,}`/`={2,}`/`-\.+-`/`-\.`）+ 可选的「行内标签 + 第二段线」+ 远端箭头 + 可选的管道标签。`&` 按括号深度切分，所以 `A[Tom & Jerry]` 里的 `&` 仍是内容。标签剥掉成对的引号 |
| 涉及文件 | `lib/ui/editor/mermaid/parser/flowchart_parser.dart`、`test/ui/editor/mermaid/mermaid_parser_test.dart`、`test/fixtures/showcase.md` |
| 验证方式 | 先用**独立原型**验证连线正则本身（15 种写法，含各种线型与标签形式），确认无误后再接进解析器；随后 23 种流程图写法逐一探查、18 种图表类型全量回归；仓库内新增 9 条断言 |
| 探针的价值 | 这五个问题都不是「某个用户报的」，而是拿 **10 种节点形状 + 8 种连线 + 5 种其他写法**逐个喂给真实解析器看输出发现的。仅靠 showcase 夹具里那几个示例，它们会一直藏着 |
...
| 优先级 | **P0** |
| 状态 | 已修复 |
| 现象 | ① `graph TD` / `开始 --> 结束` —— 中文节点名的流程图**一个节点都不生成**，却生成了指向这些不存在节点的边，图渲染出来是空的；`甲[开始] --> 乙{判断}` 更糟，边的端点是**带括号的原始文本** `甲[开始]`；② 时序图 `用户->>系统: 登录` **完全解析不出参与者和消息** |
| 根因分析 | 两个解析器的标识符模式都写作 `(\w+)` —— Dart 里的 `\w` **只含 ASCII**。流程图里有 **17 处**这样的模式，时序图的消息与别名模式各一处 |
| 严重性 | 本项目面向中文用户（界面默认中文、文档全中文），而流程图是 mermaid 最常用的类型。写中文节点名是最自然的做法，结果是一张空图 —— 用户只会觉得「mermaid 渲染有问题」，无从判断是名字的问题 |
| 同批修复的时序图问题 | ① `A-x B`（叉号后带空格）整行被丢弃；② `A->>+B` / `B-->>-A`（激活标记）整行被丢弃。激活条本身仍未绘制，但消息至少要能读出来 |
| 修复方案 | 节点标识改为「除空白与形状定界符之外都可以」，从而支持任何文字系统；时序图的消息模式同样放宽，并允许箭头两侧的空格与 `+`/`-` 标记 |
| 放宽后立刻出现的回归 | 标识符不再限定 ASCII 后，`A-->B;` 把 `B;` 当成了节点名 —— 分号是语句终止符。已在行级剥除。**这是在探针回归里当场发现的**，说明放宽字符集时必须同时检查「原本被字符集顺带挡掉的东西」 |
| 涉及文件 | `lib/ui/editor/mermaid/parser/flowchart_parser.dart`、`lib/ui/editor/mermaid/parser/sequence_parser.dart`、`test/ui/editor/mermaid/mermaid_parser_test.dart`、`test/fixtures/showcase.md` |
| 验证方式 | 14 种图表类型各用中文内容跑一遍（全部通过）；流程图 23 种写法、时序图 14 种写法全量回归；仓库内新增 7 条断言，夹具补入中文流程图与带激活标记的中文时序图 |
| 同源 | 与 BUG-049（mermaid 节点 id 的 ASCII 白名单把日/韩/俄文合并成同一个节点）是同一类问题。当时只修了 `identifier.dart` 这一处共用函数，**没有回头检查各解析器自己的模式** |

---

## BUG-083 看板列用中文 id 时整块看板失败
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 现象 | `kanban` / `待办栏[待办事项]` —— 列同时带中文 id 和标题时，**整块看板都解析不出来**，不只是那一列 |
| 根因分析 | 列的模式是 `^(?:(\w+)?\[([^\]]+)\]\|([^\[\]]+?))…`：第一支要求 id 是 ASCII，第二支不允许出现方括号。中文 id 加方括号标题**两支都不匹配**，于是这一行既不是列也不是任务，解析随之整体失败 |
| 为何比 BUG-082 更隐蔽 | 纯中文列名（`待办`）走第二支，是正常的；只有「中文 id + 方括号标题」这一种组合才炸。前一轮的中文探针恰好只覆盖了前者 |
| 修复方案 | id 改为「除空白与方括号之外都可以」，两处列/任务模式同步 |
...
| 优先级 | P2 |
| 状态 | 已修复 |
| 现象 | `## 小节 ##` 这种「两边都加井号」的写法（很多从 Word / 旧 wiki 转过来的文档都这么写）在渲染和大纲里都显示成 `小节 ##` |
| 根因分析 | `_headingRe` 是 `^(#{1,6})\s+(.+)$`，`.+` 一路吃到行尾。CommonMark 规定行尾这串井号是**闭合标记**不是内容，前提是它前面有空白 |
| 修复方案 | 正则改成 `^(#{1,6})\s+(.+?)(?:\s+#+)?\s*$`。改在正则里而不是在两个取值处各 trim 一次 —— 渲染和大纲读的是同一个正则，这样两边不可能再分叉（本版已有 BUG-069 是「两个标题扫描器各写一份」踩的坑） |
| 边界 | `# C#` 保留井号（前面没有空白，不是闭合标记）；`# a # b` 中间的井号照留；`# a # b #` 只去掉末尾那个 |
| 已知未覆盖 | `#### ####`（内容为空的标题）CommonMark 判为空标题，本项目仍解析出内容 `####`。空标题在大纲里也是空条目，价值不大，暂不处理 |
| 涉及文件 | `lib/services/markdown_parser.dart`、`test/services/markdown_parser_test.dart` |
| 验证方式 | 61 条语料基线对拍**零差异**（说明只影响带闭合井号的写法）；12 种井号写法逐一探查；五个历史断言脚本全部通过 |
