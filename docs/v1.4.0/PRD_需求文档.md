...
| FEAT-031 | 2026-08-28 | 在列表里按回车自动续下一项，空项回车结束列表 | **高** | 简单 | 已实现 |
| FEAT-032 | 2026-08-28 | 代码块行号（对齐上游 codeBlockLineNumbers，默认开启） | 中 | 中等 | 已实现 |
| FEAT-033 | 2026-08-28 | 读得懂 GBK 编码的中文文档 | **高** | 中等 | 已实现 |
| FEAT-034 | 2026-08-28 | 状态栏可点选编码，按选定编码重读文件 | 中 | 简单 | 已实现 |
...
|---|---|
| **实现日期** | 2026-08-28 |
| **需求描述** | 源码编辑器里在列表项行按回车，自动带出下一项的标记；在一个还没写内容的空项上按回车，则去掉标记、结束列表。 |
| **用户场景** | 上游 muya 给每种块都配了 `enterHandler`，我们的源码编辑器是个普通 `TextField`——**回车什么都不做**。写一份十步的清单，得手敲十次 `- `，编号列表还得自己数到十。 |
| **实现方案** | 纯逻辑放进 `MarkdownParser.listContinuation(line)`（返回"要带到下一行的标记"与"这项是不是空的"），编辑器的按键处理里挂上回车分支。要点：<br>• 编号**递增**且保留作者用的分隔符（`1)` → `2)`）；<br>• 缩进与标记后的空格原样保留（`   -  ` 宽间距也保留）；<br>• 任务项带出的是**未勾选**的框，`- [x] 已做` 回车得到 `- [ ] `；<br>• 空项回车删掉标记而不是再生一个；<br>• Shift/Ctrl/Meta + 回车不接管，选区非折叠时不接管。 |
| **为什么另写正则** | 解析用的 `_ulRe`/`_olRe` 都要求内容非空（`(.+)`），因为空标记不构成已解析文档里的一项——而"在空项上按回车"恰恰是这件事最要紧的时刻，所以续行有自己的一套读法。 |
| **一处已知偏差** | `- - -` 和 `* * *` 是合法的水平分割线，但本解析器的列表分支排在分割线之前，会把它们读成列表项（既有偏差，非本次引入）。续行**单独挡掉**了这种行——编辑器没理由在分割线下面补个标记。解析优先级本身没动，改它会牵动既有的块判定顺序。 |
| **涉及文件** | `code/lib/services/markdown_parser.dart`、`code/lib/ui/editor/source_editor.dart`、`code/test/services/list_continuation_test.dart` —— 新增 |
| **验收标准** | 14 条测试：三种圆点、编号递增（含 9→10）、`)` 分隔符、缩进与宽间距、任务框转未勾选；空项四种；非列表行六种（普通文字、标题、连写与分开写的分割线、引用、空行）。 |

---

---

## FEAT-030 补充：把"我判断它是对的"变成测试

FEAT-030 之后连着出了三个 P1（BUG-089 图表没图片、BUG-090 公式不加载 KaTeX
与图片不内嵌），根子都一样：**某处代码只遍历顶层 AST**。修完第三个时我扫了
全仓的 `for (final node in ast)`，共十二处，判断其余都是正确的顶层渲染入口
——但那只是我的判断。

这次把判断落成测试：

- `nested_block_export_test.dart`（15 条）：把**七种构造**（代码块、引用、
  表格、标题、水平线、第二段落、更深的列表项）分别写在顶层和写在步骤下面，
  逐一断言导出的 HTML 里都在。每条都**先断言它单独写时确实出得来**，
  否则"嵌套版也没出来"证明不了任何事。
- 同一批构造再断言解析器确实把它们当成该步骤携带的块。
- PDF 与 Word 各一条，确保嵌套内容不会让导出崩掉。
- 预览端补两条组件测试（步骤下的代码块、步骤下的引用真的画了出来）——
  FEAT-030 当时只验了解析和导出，没验预览。

顺带纠正了自己两处断言：代码块经语法高亮后 `void main()` 被拆进多个 span，
字面串找不到，应当断言 `<pre>`；`- inner` 缩进在步骤下**不是携带的块**，
而是同一列表里更深的一项（靠 depth 表示），已单独断言其 depth 为 `[0, 1]`。

剩余的顶层遍历经核实都正确：三个导出主循环靠 `nodeToHtml` 等函数内部递归；
`_selectedTextToHtml`（预览复制成富文本）同理；`walk` 是遍历本身；
渲染器另一处是语法高亮的节点，与文档 AST 无关。
...
|---|---|
| **实现日期** | 2026-08-28 |
| **需求描述** | 预览里的围栏代码块左侧显示行号，可在设置中关闭。对齐上游 `codeBlockLineNumbers`（上游默认**开启**，本项目此前完全没有）。 |
| **用户场景** | 讨论一段代码时是按行说的——"第 12 行那个判断"。这是上游对比清单里最后一个明显的功能缺口。 |
| **实现难点：与自动换行对不齐** | 把代码画成一整块文字、行号画成另一块，只在没有任何一行换行时才对得齐。做法是**一行一个 `Row`**：换行的那一行自己长高，号码靠上与它齐平。<br>不换行（横向滚动）时则相反：**只让代码滚动，行号固定**——号码是用来告诉读者位置的，滑出视野就没用了。 |
| **实现难点：切分高亮** | 高亮器给出的样式段与行边界无关：一段可以跨好几行，一行也可以由好几段拼成。**按行分别高亮**能对齐，但会丢掉所有跨行的构造（块注释、分成三段的字符串）。所以新增 `CodeHighlighting.splitByLine()`：先整块高亮，再把结果按换行切开，样式随之带下去（子段没有自己的样式时继承父段的）。 |
| **涉及文件** | `code/lib/core/config/app_config.dart`、`code/lib/ui/editor/code_highlighting.dart`、`code/lib/ui/editor/markdown_renderer.dart`、`code/lib/ui/screens/settings_screen.dart`、`code/lib/core/i18n/l10n/app_*.arb`（12 个）、`code/test/ui/editor/code_line_numbers_test.dart` —— 新增 |
| **验收标准** | 7 条切分测试：跨行的段切成每行一片、同行多段合并、样式保留、子段继承父样式、**空行也算一行**（丢掉会让之后所有号码错位）、空输入仍是一行、行数与源文本一致；2 条组件测试：三行代码出现 1/2/3 且**尾部换行不多出第 4 个号**、代码本身仍在。 |

### 一处位置上的修正

`splitByLine` 我先写进了 `IncrementalMarkdownHighlighter`——那是源码编辑器里
负责 **markdown** 高亮的类，与代码高亮无关。发现后移到了 `CodeHighlighting`。
放错位置的工具函数，下一个人就找不到、于是再写一个。

---

## FEAT-033：读得懂 GBK 编码的中文文档

| 字段 | 内容 |
|---|---|
| **实现日期** | 2026-08-28 |
| **需求描述** | 用 GBK 保存的中文文档正确打开，状态栏显示 `GBK`，保存时按原编码写回。 |
| **用户场景** | UTF-8 普及之前，中文文档基本都是 GBK。此前这类文件会退化成 Latin-1——**一个汉字变成两个乱码字符**，整篇不可读。既有测试和注释把这当成"已知取舍"记着（Latin-1 至少能字节级原样写回），但那只是不损坏文件，不解决读不了的问题。上游有 `autoGuessEncoding`。 |
| **难点：GBK 与 Latin-1 分不开** | 两者都是高位字节。`é`（0xE9）后面跟一个字母，**看起来就是一个合法的 GBK 双字节**，所以"每个高位字节都能组成合法 GBK 序列"这个判据会把法语德语误判成中文。真正的区别是**比例**：中文几乎整篇都是双字节，而带重音的欧洲文字是"ASCII 里偶尔夹一个"。判据因此是两条并用——全部高位字节都能配成合法对，**且**成对字节占全文不少于 30%。 |
| **实现方案** | 加 `charset: ^2.0.1`（提供 GBK 编解码，内置的 `dart:convert` 只有 UTF-8/16 与 Latin-1）。`FileEncoding` 新增 `gbk`，解码顺序为 UTF-16 BOM → UTF-8 → **GBK（形状+比例判据）** → Latin-1。写回时按 GBK 编码；遇到 GBK 装不下的字符（例如在旧笔记里打了 emoji）退回 UTF-8 而不是让保存失败。 |
| **涉及文件** | `code/pubspec.yaml`、`code/lib/models/file_encoding.dart`、`code/test/models/gbk_encoding_test.dart` —— 新增、`code/test/models/file_encoding_test.dart` |
| **验收标准** | 8 条新测试**全部用真实字节**（Python 生成的 GBK 与 Latin-1 样本，不是手写常量）：GBK 中文散文、GBK 与 markdown 混排（ASCII 部分不受影响）、**字节级往返一致**；法语与德语**必须**仍判为 Latin-1；以及不能被破坏的三条——UTF-8 仍然优先、纯 ASCII 不会被判成 GBK、空文件不做猜测。 |

### 一处既有测试的更新

`file_encoding_test.dart` 里有一条断言"GBK 文档以 Latin-1 打开"，注释写着那是
当时的取舍。这次它断言的正是要改掉的行为，已改为断言读成 GBK；同时另加一条
守住退路：`0xFF` 开头的字节序列（GBK 前导字节范围是 0x81–0xFE，不含 0xFF）
仍然退回 Latin-1，且字节级往返一致。
...
|---|---|
| **实现日期** | 2026-08-28 |
| **需求描述** | 点状态栏上的编码标签，选另一种编码，按它重新读一遍文件。 |
| **用户场景** | FEAT-033 的编码识别是**启发式**，必然有判错的时候。此前编码只是一段纯文本，读者看得见文件开成了乱码，却**毫无办法**。换行符指示器早就是可点的，这里照同一范式补上。 |
| **实现方案** | `FileEncoding.decodeAs(bytes, encoding)` 按指定编码解码（宽容模式：选错编码应当看起来不对，而不是把文档关掉）；`TabNotifier.rereadAs` **从磁盘重读**而不是重新解释屏幕上的文本——内存里的文字已经过了一道解码器，再过第二道找不回第一道丢掉的东西。有未保存修改的标签不重读（会丢工作），没有文件的标签也不重读。 |
| **涉及文件** | `code/lib/models/file_encoding.dart`、`code/lib/providers/tab_provider.dart`、`code/lib/ui/widgets/status_bar.dart`、`code/test/providers/reread_encoding_test.dart` —— 新增 |
| **验收标准** | 5 条：能换成另一种编码；换回去要**从文件重读**（内容与首次打开一致）；Latin-1 文件不被误判；有未保存修改时拒绝重读且内容不变；无文件的标签拒绝重读。 |

### 写这条时暴露并修掉的 FEAT-033 缺陷

`reread_encoding_test` 里用德语 `Grüße\n` 作 Latin-1 样本，**它被判成了 GBK**：
只有 6 字节，其中 `FC DF` 恰好构成合法 GBK 对，占比 33%，越过了 30% 的门槛。
**是我自己写的测试抓到了自己的启发式缺陷。**

加了两条判据：

1. **解码结果落入私用区就否决**。`charset` 的解码器读不出来时不抛异常，而是
   插入一个标记字符，实测落在 U+E3EA（私用区 U+E000–U+F8FF）——真正的中文
   不会解码到那里。注意包里声明的常量 `replacementCharacterUnicode = 0xE7B3`
   与实际输出不符，所以判的是**整个私用区**而不是那个具体码点。
2. **短于 12 字节不猜**。`Öl\n` 只有 3 字节，而 `D6 6C` **确实是一个合法的
   GBK 汉字序列**——信息量不足，任何启发式都分不开。这种情况退回 Latin-1
   （能字节级往返），并靠这条新增的手动选择兜底。

两条既有测试因此改用了**真实长度的样本**（23 字节的中文文档）而不是 6 字节的
片段——不是放宽规则，是样本本来就不真实。
