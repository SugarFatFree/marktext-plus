...
`test/core/i18n/one_action_one_name_test.dart`：18 对键 × 12 种语言，
译文必须一致。另有一条「守卫的守卫」断言确实数到了 12 个文件——
一个匹配不到文件的 glob 会让下面每条断言都静悄悄地通过。

变异验证：把中文改回不一致 → 红；改一个从没出过问题的语言（韩语）→ 也红。
