...
| BUG-375 | 2026-09-09 | 预览模式下采用插件改写，撤销记录记到了别的标签上 | P1 | 已修复 |
...
| BUG-390 | 2026-09-10 | SDK 的 12 份 README 描述的是编辑器已废弃的窗格规则 | P1 | 已修复 |
| BUG-391 | 2026-09-10 | README 的测试数在三个地方是三个不同的错数 | P3 | 已修复 |
| BUG-392 | 2026-09-10 | 12 份 README 都没提插件能画自己的界面 | P2 | 已修复 |
| BUG-393 | 2026-09-10 | SDK 英文文档自称没有 require，而五处在教怎么用 | P1 | 已修复 |
| BUG-394 | 2026-09-10 | CHANGELOG 停在两天前，右侧栏与语言两批修复一条没写 | P2 | 已修复 |
| BUG-395 | 2026-09-10 | 追加一轮后按「采用」，文档一个字没变 | P0 | 已修复 |
| BUG-396 | 2026-09-10 | 生成中整条右侧栏变成转圈，刚打字的地方消失了 | P1 | 已修复 |
...
| BUG-414 | 2026-09-11 | 列表项里的围栏，内容像列表项时被拆成真列表项 | P1 | 已修复 |
...
> **后来改成八倍**（2026-09-11 补注，不改写上面这段记录）：六在本机连过五次、
> 在 CI 上以 6.05 挂掉，于是买了余量。**当时 README 没跟着改**，
> 于是首页承诺的是一条比实际更严的规则——一个照着它做优化的贡献者，
> 会从「测试没红」里得到错误的安心。
> 现在这个数字收在 `test/support/cost_limits.dart`，
> `readme_counts_test` 拿它去核首页那一行。
...
在**预览模式**下用插件改写文档并点「采用」，那一步撤销记录**不会记在这个标签
上**——它落到「上一次有源码窗格的那个标签」的历史里。

后果有两层：

1. 这次改动**撤销不回来**
2. 更糟：切到那个标签按 Ctrl+Z，会把**这个文档**改动前的文字写进**它**里面

### 根因

撤销历史按标签分开存，而「当前是哪个标签」只有一个地方会说：

```
setHistoryTab 的调用点：lib/ui/editor/source_editor.dart:759   （唯一一处）
```

**预览模式下源码窗格根本不构建**，所以没有人更新过它。而从源码窗格之外写历史的
路径有两条，都可能在预览模式下发生：

- `plugin_apply.dart` —— 点「采用」
- `plugin_command_actions.dart` —— 插件返回 `replace` 动作

而 AI 助手面板正是在预览模式下用的（官方插件传 `as = view_of(ctx)`）。

### 修复方案

`pushHistory` 接受一个 `tabId`，说清这条记录属于谁；两个调用点都把自己正在改的
那个标签传进去。给别的标签压记录时不动当前的 `canUndo`，只清那个标签自己的重做栈。

### 怎么找到的

不是扫描扫出来的。上一步实机验证了「采用」按钮画得出来（BUG-369），
接着问一句：**按下去真的写进文档了吗？** 补测试时才发现第二条断言（撤销）红了。

**「按钮存在」和「按钮管用」是两回事**，而 BUG-369 恰恰就是一个按钮形状的洞。

### 验证

两条测试：按下「采用」文档确实变化且抽屉收起；撤销记录落在被改的那个标签上、
**且别的标签没有多出一条它没做过的改动**。

变异（采用时不说是哪个标签）：只有新守卫红，全套其余全绿。

> 测试里有一处要留意：基线不能和被替换的文字相同——`pushHistory` 会跳过
> 与栈顶重复的内容，那样断言就什么也观察不到了。

---

## BUG-376：在预览里往回滚，源码跳到底部
...
| 优先级 | P1 |
| 状态 | 已修复 |
| 报告人 | 用户（「在预览块滚动或拖动滚动条就有问题，向上滚动的时候，左侧源码块经常就直接滚动到底部」） |

### 现象

用户的描述精确到了方向，这正是找到它的关键：

- 在**源码**里滚动、拖滚动条：正常
- 在**预览**里滚动、拖滚动条：**向上滚时源码经常直接跳到底部**

### 根因：参照物取错，误差随偏移增长

预览把「我在第几行」报给源码，源码用 `_followPreviewToLine` 换算该滚到哪：

```dart
final paneTop = box.localToGlobal(Offset.zero).dy;          // ← 输入框自己
final target = _editorScrollController.offset + (lineTop - paneTop);
```

`box` 是**输入框**，而输入框是随内容一起滚动的——它的屏幕位置本身就等于
「当前偏移的负数」。于是 `lineTop - paneTop` 得到的是「这一行在输入框内的 Y」，
再加一次当前偏移，**等于把滚动量算了两遍**。

诊断打印把它坐实了：

```
line 177  at=0     lineTop=4584   paneTop=0      -> 4584   ✓
line 133  at=4584  lineTop=-1144  paneTop=-4584  -> 8024 → 夹到 5630（底部）✗
```

**当前偏移为 0 时两者恰好相等**，所以从文档顶部开始滚一切正常；
一旦已经滚下去，误差就等于当前偏移，向上滚得越多、错得越远，直到撞上底部。

这也解释了方向性：源码→预览用的是 `_contentOffsetForLine`，返回的是内容内的
绝对位置，不受此影响。
...
```dart
final viewport = position.context.storageContext.findRenderObject() as RenderBox?;
final target = _editorScrollController.offset + (lineTop - viewportTop);
```

### 验证

新增一条：先在预览里滚到 80%，再分四步往回滚，每一步都断言源码**没有比之前更靠下**，
最后断言它确实回来了。变异（参照物换回输入框）只有这一条红，全套其余全绿。

---

## BUG-377：等你回答，却装作正在工作
...
FEAT-142 改成一个输入框时，撞到两处把 `_running` 当「忙」的判断——
而**命令正等着你回答的时候恰恰就是「运行中」**。当时修了那两处。

回头把这个标志的**每一处读法**都问一遍，第三处也在说同样的假话：

```dart
if (_running && _content.isEmpty)   // ← 问题摆在那里，下面转着圈
```

于是抽屉一边问「要我怎么写」，一边转圈说自己在算。

### 修复方案

把判据从代码里散落的读法收敛成一个说得清的名字：
...
`_running` 现在只在**赋值**处出现；所有读取都走 `_working`。
四处读法（转圈两处、「采用」按钮、输入框可用性）与两处提前返回，
一并改过来——**下一处读它的人不必再想一遍这件事**。

### 教训

上一轮修了同一个标志的两处误读，**却没有把它剩下的读法读完**。
这正是本库反复出现的「改一个分支就读完它的兄弟」——
而这次的兄弟不在别的文件里，就在同一个类的几行开外。

**做法**：修好一个标志的误读之后，`grep` 那个标志，把每一处读法逐个问一遍
「它在这里到底是什么意思」。若答案不止一种，就该起两个名字。

### 验证

新增一条：面板提问时不该有转圈。变异（`_working` 退化回 `_running`）
被两条测试同时抓住。

---

## BUG-378：同步的另一向，向上滚没人测
...
BUG-376 修的是「预览→源码」向上滚时把源码抛到底部。分屏同步是**双向**的，
而测试覆盖成了这样：
...
源码→预览走 `_contentOffsetForLine`，返回的是内容内的**绝对**位置，
不像另一向那样要拿当前偏移去做相对换算——所以它天然免疫 BUG-376 那个错。
**没有现存缺陷。**

### 守卫不是空过

把 BUG-376 那个错**原样复制到这一向**（跟随时也把当前偏移加进去），
新加的这条测试立刻变红。说明它确实在看这件事，而不是恰好通过。
...
## BUG-380：macOS 的 zip 大了三倍，而且解开是坏的
...
要求 1 的四条主张里，「加载快/大文件」「占用低」「秒启动」都量过了，
**「轻量级」从没量过**。去看发行版的资产大小，八个版本一路稳定
（Windows 安装包 14.3 → 15.0 MB，Linux deb 12.5 → 13.1 MB），
唯独一处刺眼：

| v1.6.1 的 macOS 包 | 大小 |
|-------------------|------|
| `macos-universal.dmg` | 25.2 MB |
| `macos-universal.zip` | **75.7 MB** |

同一个应用，三倍。而且每个版本都是这个比例。

### 根因

```yaml
zip -r ".../macos-universal.zip" "MarkText Plus.app"
```

macOS 的 `.app` 里满是符号链接——`Versions/Current`，以及框架的二进制和
`Resources`。**`zip -r` 不加 `-y` 会跟随符号链接、把目标内容再存一份**，
于是每个框架二进制都进去了两遍。

大小只是表象，**解开之后的 bundle 是坏的**：本该是符号链接的位置成了重复文件。
这个包没有签名，或许还能启动，但它已经不是一个结构正确的 bundle 了。
...
```yaml
ditto -c -k --sequesterRsrc --keepParent "MarkText Plus.app" ".../....zip"
```

`zip -ry` 也能保住链接，`ditto` 还一并保住元数据。

### 守卫，以及它第一版被自己的注释骗过

新增一条检查 `release.yml` 的守卫。**第一版是错的**：它读整个步骤块找
`ditto` 这个词——而我在命令上方写的注释里正好解释了「用 ditto 而不是 zip -r」，
于是把命令改回 `zip -r` 之后，**守卫靠那句注释照样通过**。

改成只看**命令行**（去掉 `#` 开头的行）之后，变异才如实变红。

> 这和「断言查得太宽」是同一类：一段文字里同时出现「对的做法」和「错的做法」时，
> 用 `contains` 去找那个词，找到的可能是解释而不是代码。

### 涉及文件

- `.github/workflows/release.yml`
- `code/test/utils/release_assets_test.dart`

---

## BUG-381：最对外的两个数字，没人对过账
...
README 首页上有两个可数的承诺：**「22 diagram types」**和**「8 themes」**，
连同 11 份翻译一共说了 12 遍。而 `readme_validation_test` 只验它们**能解析**。

加一种图表或一个主题，十二份文件仍写着旧数字，**第一个发现的人是自己去数的那个**。
这正是本库第三条排查视角点名的形状——一份对外宣称、一份是实现、没人比较——
用在最对外的那份清单上。
...
| 说的 | 实际 |
|------|------|
| 22 种图表 | 解析器枚举 22 种（`unknown` 除外）✓ |
| README 逐个列出的名字 | 22 个 ✓ |
| 8 个主题 | `AppTheme.themeNames` 8 个 ✓ |

> 中途虚惊一场：`grep -c "AppThemeTokens("` 得到 9，看着像多出一个主题。
> 第 9 处是**构造函数声明本身**。又一次「扫描的结果不是证据」。

### 守卫

四条：

1. **十二份 README 都在**（守卫的守卫；目录找不到时上面几条会无声空转）
2. 每份都必须出现图型数这个数字——**数字在每种语言里都写作阿拉伯数字**，
   周围的词翻译，数不翻译，这才使它可查
3. 每份都必须出现主题数；顺带断言**明暗两张表加起来等于总表**
   （少一个或重一个，设置页就会漏掉或重复某个主题）
4. 英文 README 那一行**逐个列出的名字个数**必须等于枚举数量——
   数字和列举是两个承诺，只有一个是数字
...
`sdk_schema_agrees_test` 已经有 11 条，把 SDK 那份 schema 的**内容**钉在编辑器上：
哪些权限、哪些运行时、哪些设置类型、哪些菜单条件、哪些图标、界面上限、贡献点。

**没有一条比过字段清单本身。** 而 schema 写着 `additionalProperties: false`，
于是两种写错方式会从相反方向砸到插件作者头上：

| 情况 | 作者看到的 |
|------|-----------|
| 编辑器读、schema 没声明 | 用一个**真实存在**的功能，被 schema 判为「无效的 manifest」 |
| schema 允许、编辑器不读 | 写了，**什么也不会发生** |

### 实测：今天 19 对 19

schema 的 19 个顶层属性，与 `PluginManifest.fromJson` 的 19 个构造器实参
逐个对上，两个方向差集都为空。

### 守卫

编辑器这一侧**从 `fromJson` 自己的实参里读出来**（正则取构造器命名参数），
不是在测试里另抄一份——否则这条守卫自己就成了第三份没人对账的清单。

### 验证

| 变异 | 结果 |
|------|------|
| schema 删掉 `toolbar` | 红：`Actual: ['toolbar']`，说「作者用真实功能反而被判无效」 |
| schema 加一个编辑器不读的 `sponsorUrl` | 红：`Actual: ['sponsorUrl']`，说「写了不会有任何反应」 |

> 写完先踩了一次 `repo_dependent_tests_test`：它要求依赖相邻仓库的用例必须用
> `skip:` 而不是 `if (!present) return;`，否则 CI 上会以
> `LateInitializationError` 失败而不是被跳过。**守卫抓住了新写的守卫。**
...
## BUG-390：编辑器改了规则，SDK 的十二份文档留在原地
...
| 日期 | 2026-09-10 |
| 优先级 | P1 |
| 状态 | 已修复（改动在 SDK 仓库，守卫在本仓库） |

### 问题

窗格方向的规则 **2026-09-07 由人工测试改过**：一个窗格原先总在文档旁边，
现在**方向由槽位名决定**——只填 `bottom` 放在文档下面，填别的在旁边。

编辑器改了，它自己的 `plugin_panes_layout_test` 也跟着改了。
**SDK 的 12 份 README 一份都没改**，原样写着被废弃的那句概括：

| 语言 | 那句话 |
|------|--------|
| en | One pane is one pane whichever slot it claimed |
| zh | 一个分栏就是一个分栏，不管它自称什么 |
| ja | ペイン一つはペイン一つで、何を名乗っていても同じです |
| de | Ein Bereich ist ein Bereich, wie immer er sich nennt |
| ru | Одна область есть одна область, как бы она себя ни называла |
| ar | والمساحة الواحدة مساحةٌ واحدة مهما سمَّت نفسها |

于是**第三方插件作者只填 `bottom`，文档告诉他会得到文档旁边的窗格，
实际得到的在下面**——而官方的 AI 写作与 AI 纠错早已在依赖新行为。

**为什么看起来不像坏的**：那段里 `corner` 的例子在新规则下**仍然成立**
（corner → 旁边）。过期的是它上面那句概括，例子还对着，就没人往下想。

### 修复

12 份都改写：保留 `corner` 的例子，把概括换成实际规则，
并说明理由（改写你正在看的那一段，放在下面行宽相同、眼睛往下扫）。
SDK 的 CHANGELOG 记在 `[Unreleased] / Fixed` 下。

### 守卫：跨语言只能数标识符

散文在 11 种语言里，守卫读不了。但 **`bottom` 是标识符，不翻译**：
...
修改前 12 份都是 1，修改后都是 2。`sdk_schema_agrees_test` 加了第 13 条测试来数。

这正是记忆里那条做法的应用：**跨语言对账时，要么认得每种写法，
要么只对不翻译的部分精确对账。**

### 另一半：从改动处指回文档

守卫只能在**已经写错之后**报警。真正的缺口是——改规则的人不知道
这条规则还写在另一个仓库的 12 份文件里。所以
`plugin_panes_layout_test` 那条测试的注释里加了这根线：
「这条规则也写在 SDK 的 README 里，12 种语言；再改的人要一并改。」

### 验证

把日语那份改回旧写法 → 守卫红，且**点名到具体文件**：

```
Actual: <1>
…/README_ja-JP.md 只把 bottom 当名字列了一次，没说填它会把窗格放到文档下面
```

### 涉及文件

- SDK 仓库：`README.md` + `docs/i18n/README_*.md`（共 12 份）、`CHANGELOG.md`
  （提交 `4245188`）
- `code/test/services/sdk_schema_agrees_test.dart` — 第 13 条
- `code/test/ui/widgets/plugin_panes_layout_test.dart` — 指向 SDK 的线索

## BUG-391：一个数字，三种说法，都不对
...
沿用上一轮记下的查法——**搜文档里的全称句式，逐句回代码核实**——
在主 README 上第一遍就撞到：

| 出处 | 说的 |
|------|------|
| 英文 README | 2777 tests |
| 11 份翻译 | 2432（各语言写法不同） |
| **实际** | **3005** |
...
**这个数每次提交都会变，而它写在 12 个文件里。**
手工保持精确是做不到的，所以第一次漂了之后就没人再试。
`readme_counts_test` 守着「22 种图表」和「8 个主题」，
唯独没守这个——因为那两个不常变，而这个天天变。

### 修复：把精确数换成有守卫的下限

12 份都改成「2600 多项测试」。下限的好处是**加测试永远不会让它变假**，
只有大批删测试才会——而那正是你想知道的时候。

守卫数的是源码里 `test(` / `testWidgets(` 的调用数（2648），
比运行时报告的 3005 少——`for` 循环里的一次调用会生成多个测试。
所以它证明的下限，对真实数字也成立。

> **数字一律写成不带分隔符的 `2600`**。`2.600`、`2 600`、`2,600`
> 在不同语言里都对，而一条得认全这些写法的守卫，会在数字明明在的时候
> 报告它不在。这是记忆里那条跨语言陷阱的直接应用。

## BUG-392：编辑器长出一项能力，十二份首页一份都没提
...
插件能返回一棵由文字、输入框、标签、下拉框、复选框、Markdown、图片和按钮
组成的树，由编辑器用自己的控件画出来（`sdk.ui`，11 种节点）。
**SDK 用 12 种语言完整文档化了它，主 README 一个字都没提。**

而且首页原话是**反着说的**：

> **⚙️ Own settings** … Plugins supply data, **never widgets**

这句话在设置页的范围内还站得住（插件给的是描述，控件是编辑器造的），
但读者读完的结论是「插件画不了界面」——现在这是错的。
**又一次是概括过期，而不是例子过期。**

### 修复

12 份都加一行「自己的界面」，并把那句会误导的概括改成
「插件描述它要什么，控件是编辑器的」——这句在两种情形下都成立。

### 守卫：按 emoji 对齐能力表

行是用 **emoji** 做键的：每行都以它开头，各语言里是同一个字符，
翻译时漏掉一行就会连 emoji 一起漏掉。英文版有而某份没有的，即报错。

这补上了一个已知盲区：SDK 那条形状守卫数的是标题数与代码块数，
而**能力行既不是标题也不是代码块**——四份翻译当年就是这样整段丢掉
「右侧边栏面板」的。
...
| 变异 | 结果 |
|------|------|
| 韩文版删掉那一行 | 红：`Actual: ['🎛']`，点名 `README_ko-KR.md` |
| 法文版把 2600 改成 9900 | 红：`…README_fr-FR.md 里没有那个下限数字` |

### 涉及文件

- `README.md` + `docs/i18n/README_*.md`（共 12 份）
- `code/test/services/readme_counts_test.dart` — 新增两条

## BUG-393：同一份文档，五处教你用 require，一处说没有 require
...
把「搜全称句式逐句核实」这条查法用到 SDK 的英文 README（插件作者照着写代码的那份），
67 条断言里挖出两个，方向相反。
...
> Only these. There is no `os`, no `package`, **no `require`**, no `dofile`, no `loadfile`…

而同一份文档的第 56、61、122、126、150、162–172、766 行**都在教怎么用 `require`**。

去代码里核实：Lua 自带的 `require` 确实被置空了，**然后编辑器又装了一个自己的**——
`plugin_script_runtime._installRequire` 的注释写得很清楚：

> 想要的不是「没有 require」，而是**一个走不出家门的**：宿主解析名字并交回源码，
> 所以这里从来看不到路径。

那份清单是在模块系统加入之前写的，之后没跟上。后果是**它一边发
`lib/marktext-plus.lua` 这个库文件，一边告诉作者加载它的手段不存在**。

**二、`ctx.view` 七种语言有、五种语言没有**

脚本能知道读者正处在 `source` / `preview` / `split` 哪种视图（`viewFor` 返回它）。
文档化在 7 份翻译里，**英文、德文、日文、韩文、中文都没有**——
和当年整段丢掉「右侧边栏面板」的是同样那四种语言。

> **两个方向相反**：第一个是英文过期而 11 份翻译已修正（与通常方向相反）；
> 第二个是英文和四份翻译一起漏。所以守卫必须**双向**查。

### 守卫：标识符是唯一能跨语言比的东西

散文在 11 种语言里，但**反引号里的标识符不翻译**——那正是插件作者要敲进代码的东西。

先去掉代码围栏（例子会重复），取所有反引号标识符做集合比较。
修完后 **12 份对 108 个标识符完全一致，零例外**。

> 唯一的差额是 `stdout`：英文某一条带反引号，11 份翻译不带——
> 而**英文自己上一条也不带**。把它改成不带，守卫就不需要任何例外清单。
> 一条零例外的守卫，比一条带例外的可信得多。

### 顺带核实为真的（不改）

| 文档说 | 实现 |
|--------|------|
| 树深超 12 层或超 500 节点则整棵拒绝 | `maxDepth = 12`、`maxNodes = 500` ✓ |
| 一次运行上限 8 步 | `if (++questions > 8) break;` ✓ |
| 只杀自己启动的进程 | 注册表按 pid 记录，且核对现在占用该 pid 的还是不是它 ✓ |
| 只有编译型插件有超时 | `plugin_process_host` 5 秒超时，只终止那个子进程 ✓ |
| `when` 三个取值 | `PluginMenuCondition { always, selection, noSelection }` ✓ |

### 验证

| 变异 | 结果 |
|------|------|
| 意大利文删掉 `ctx.view` 那一行 | 红：`Actual: ['ctx.view', 'preview', 'split']`，点名该文件 |
| 俄文加一个英文没有的标识符 | 红：`Actual: ['ctx.futureThing']`，说「英文才是要补的那一份」 |

### 涉及文件

- SDK 仓库：`README.md`、`docs/i18n/README_{de,ja,ko,zh}*.md`、`CHANGELOG.md`
  （提交 `b86d15a`）
- `code/test/services/sdk_schema_agrees_test.dart` — 第 14 条

## BUG-394：同一个形状，往前一个文件
...
上一轮发现人工测试计划停在改动前一天。**同一个信号往前一个文件再看一次**——
`ls -la` 一眼：

| 文件 | 最后改动 |
|------|---------|
| `CHANGELOG.md` | **09-08 22:57** |
| `bugfix.md` | 09-10 07:00 |

发布前检查第 3 项要求「CHANGELOG 的本版条目与那两份文档对得上」。逐条读下来，
本版 54 条里**整整三批一条都没有**：

- 右侧栏那一批（BUG-369/370/372/377）——没有 Apply 按钮、Markdown 原样显示、等模型时毫无动静
- 分屏滚动那一批（BUG-371/376）——预览里向上滚，源码跳到底部
- 语言与布局那一批（BUG-386/387/388）——插件页说英语、RTL 十三处、四个无名按钮

补了 10 条，并新开一节「Twelve languages, and one of them reads right to left」，
与 `release-notes.md` 的分节对齐。

### 守卫：第一版写错了，因为它对自己要防的情形放行

第一版按**体积比**：CHANGELOG / 发行说明 / 人工测试计划相对 `bugfix.md`
不得小于某个比例。写完顺手验了一下——

> 用改之前那份真实的 `manual-test.md` 算，比值 0.077，阈值 0.04，**它不会红**。

一条对着自己要防的情形放行的守卫，比没有更糟：它教所有人「这块有人管了」。
（同「断言要能区分假设」。）
...
守卫要求这个编号等于 `bugfix.md` 的最大编号。不是每条修复都需要人工步骤——
最近几条都是守卫和文档——**但那得有人决定过，而抬这个标记就是决定的地方**。

这和本轮反复奏效的形状是同一个：**让例外写下来，而不是靠推断**。

### 验证（这次用真实的旧文件，不是构造的）

| 变异 | 结果 |
|------|------|
| 换成 `HEAD~1` 那份真实的旧计划 | 红：`Actual: <null>`，「没有『已考虑到 BUG-N』的标记」 |
| 标记停在 388 而 bugfix 到 393 | 红：`Expected: <393> Actual: <388>` |

### 涉及文件

- `CHANGELOG.md` — 补 10 条 + 新增一节
- `docs/v1.6.2/manual-test.md` — 覆盖标记
- `code/test/services/version_docs_test.dart` — 按编号对账

## BUG-395：一个空串两种含义，采用就此失效
...
> 「AI 助手」插件，右侧边栏生成内容后，**点击采用实际并没有采用**。

### 根因：本项目自己记着的那个陷阱

```dart
// 修复前
if (_replaces.isEmpty) _replaces = replaces;   // 「保留第一次的」
```

空字符串同时担了两份工作：**「还没记下」**和**「替换整篇文档」**。
而「整篇文档」正是**没有选中任何东西时插件说的话**——也就是最常走的那条路。

于是：

| 步骤 | 插件发的 `replaces` | 编辑器记下的 | 结果 |
|------|-------------------|------------|------|
| 1. 无选中，写作 | `""`（整篇） | `""` | 此时按采用是好的 |
| 2. 追加一轮 | 上一次的答案（按设计 `ctx.selection` 就是它） | **被当成「还没记下」而采纳** | — |
| 3. 按采用 | — | 找上一次的答案在文档里的位置 → **-1** | **一个字没变** |

那段注释写得很清楚「保留第一次的，因为更短的改写要替换的仍是原来那一段」——
**意图完全正确，判据用错了**。

### 修复

`String _replaces = ''` → `String? _replaces`，判据 `if (isEmpty)` → `??=`。
null 是「还没记下」，`''` 是「整篇文档」，两者不再混。

### 已有的守卫为什么没拦住

有一条测试叫「what a refinement would replace is still the original」，
它只断言**采用按钮还在**：

> Apply is still offered — it would have gone if `replaces` had been overwritten…

**画出来不等于按得动。**这正是本库反复出现的那句：清单对账守的是「点名到齐」，
不是「各司其职」。新测试按下按钮并核对文档内容。

> 写这条测试时先踩了一次：沿用现成的 `echo` 脚本，它在无选中时发
> `replaces = "DOC"`——那个词也不在文档里，**第一轮就失败，是另一条路**。
> 换成与官方插件同形（无选中时发空串）才复现出读者那条。
> **构造测试数据时，形状要和真实的那个一样，不能只是"差不多"。**

### 验证

把 `??=` 改回 `if (isEmpty)` → 红，且 `Actual: 'the whole document'`——
文档一个字没变，与读者所见一致。

## BUG-396：生成中整条右侧栏变成一个转圈
...
> 右侧边栏开始进行 AI 写作之后，**整个右侧边栏都变成加载框了**，
> 输入框应该正常展示，只是不能发送而已。
...
读者回答之后 `_question` 已清空，而 `_turns` **要等内容到了才追加**——
中间这段时间两个条件都不成立，**读者刚刚打字的那个地方消失了**，
等模型答完才回来。

### 修复

条件加上「正在运行且这个面板会问问题」，框留着但灰掉——
`busy: _working` 本来就已经让它不能发送。

**收窄到 `_panelAsks` 是必要的**：第一版只加 `_working`，
于是插件自绘界面（`ui` 树）时会在它自己的表单下面多出一个灰框。
是既有测试「a tree the plugin drew appears in the drawer itself」发现的
（它断言只有一个 `TextField`）。

## BUG-397：采用之后抽屉整个关掉，对话历史一起没了
...
> 生成完了之后点击采用，**右侧边栏又直接消失，没有对话的历史记录了**，
> 应该允许撤销并且有历史记录。
...
`_turns` 还在内存里，但抽屉关了就看不到了——**恰恰在读者可能想
再提一个要求、或回头看看自己接受了什么的那一刻**。

### 修复

抽屉留着，只撤掉那个「采用」的提议（`_canApply` / `_replaces`）——
文字已经在文档里了，再按一次会写第二遍。

顺带修了既有测试的一句 reason：它写着「采用之后抽屉该收起来」，
而**决定变了，措辞要跟上**，否则误导下一个读者。

## BUG-398：预览模式下撤销是静默空操作
...
void undo() {
  ...
  if (controller == null || _undoStack.isEmpty) return;   // ← 直接返回
  final current = controller.text;
```

`_controller` 是**源码编辑器**的控制器。右侧栏在预览模式下用，
那里**没有源码编辑器**——于是 `undo()` 和 `_restore()` 都在
`_controller == null` 时一声不响地返回。

采用这条路是对的：它先 `pushHistory(edit.before, tabId: tabId)`
再写文档，**历史记录完全正确**——只是没有任何东西能把它放回去。
BUG-375 修的是「记到了哪个标签」，而「记下了却回不去」是另一半，从没被修。

### 修复

`undo()` / `redo()` 改为**返回该写回的文本**（`String?`）：
有源码编辑器时照旧写进输入框、调用方可以忽略返回值；没有时由调用方写回标签。
新增 `hasSourceEditor` 让调用方知道该不该自己写。

菜单栏的撤销/重做改走新的 `AppMenuBar.stepHistory`，
它把标签的正文作为 `current` 传进去——**预览模式下最新的文本在标签里，
而通知器读不到它**。

### 验证

`undo_without_a_source_editor_test` 三条：无控制器时撤销给出要写回的文本、
重做同样、只有一份快照时答 null（不假装退了一步）。

### 涉及文件

- `code/lib/providers/editor_provider.dart` — `undo` / `redo` 返回文本、`hasSourceEditor`
- `code/lib/ui/widgets/app_menu_bar.dart` — `stepHistory`
- `code/lib/ui/widgets/right_side_bar.dart` — 上面三处
- `code/test/providers/undo_without_a_source_editor_test.dart`（新增）
- `code/test/ui/widgets/right_sidebar_test.dart` — 两条新测试 + 一处措辞

## BUG-399：一轮的身份不该是它说了什么
...
读者报的四条里有一条是「**要有对话历史**」。修完抽屉不再关掉之后，
顺着同一处代码往下看，发现历史本身还会丢东西：
...
判断「这几段是不是同一轮」用的是**请求的文字本身**。于是
**同一个要求提两次**——第一次「短一点」不够短，再说一次「短一点」——
第二轮会**覆盖**第一轮，前一版草稿从历史里消失。

而「再说一次」恰恰是最自然的追加方式。

### 根因

那个判据想表达的是「这是同一轮的续传」，却用了一个**不唯一**的东西做身份。
分段续传要合并是对的，两轮碰巧说了同样的话不该被当成一轮。

> **一轮是一轮，它说了什么不是它的名字。**

### 修复

加一个 `_turnRecorded` 标志：新一轮开始时置假（回答问题时、追加一轮时、
换面板时各一处），第一次记录后置真。续传走更新，新一轮走追加。

### 验证

先写测试再改：同一个要求发两次，期望历史里有两条「shorter」。
修复前 `Found 1 widget`，修复后 2 条。把判据改回比文字 → 重新变回 1 条。
...
BUG-400 明写着「还没做的一半」：插件被拒时那句
「某插件没有申请 X 权限——X 是……」在服务层组装，拿不到 l10n，仍是英文。

**修法是让动作只带标识符，句子由 UI 组装**：新增
`PluginPermissionRefusedAction(pluginName, permission)`，
UI 用 `l10n.pluginPermissionRefused(...)` 加上已本地化的权限描述组句。

加这个成员时，**穷尽 switch 精确点名了两处必须处理的地方**——
这正是把兜底改成列名（BUG-382）的收益。

`plugin_contract_test` 那条「脚本能返回哪些动作」的守卫也红了，
它是对的：新成员是**编辑器自己发的**，脚本返回不了。
在那张表里写明第二个刻意的缺席及理由。

## BUG-402：空的占位窗格，替读者做了一个它不知道的决定

| 字段 | 内容 |
|------|------|
| 编号 | BUG-402 |
| 日期 | 2026-09-10 |
| 优先级 | **P0**（我自己在 BUG-395 引入的回归） |
| 状态 | 已修复 |

### 问题

官方插件的写作**答两次**：先一个空窗格带着提示词，模型答完再一个带
`apply` 和 `replaces` 的真窗格。

```lua
return sdk.pane("", { ai = prompts.writing(text, ctx.answer), ... })
```

而我在 BUG-395 把判据改成 `_replaces ??= replaces` ——
**第一个空窗格就把它锁成了 `''`（＝替换整篇文档）**，
第二个窗格点名的那一段被忽略。

后果是数据丢失：选中一段让它改写，按采用会**用答案覆盖整篇文档**。
变异验证里看得很清楚：`before OLD after` 变成了 `NEW`。

### 判据取了三次才对

| 写法 | 对 | 错 |
|------|----|----|
| `if (_replaces.isEmpty)` | 选中替换 | 追加一轮时被上一次的答案顶替（BUG-395） |
| `_replaces ??= replaces` | 整篇替换 | **空占位窗格锁死目标**（本条） |
| **`if (canApply) _replaces ??= replaces`** | 都对 | — |

对的那条读起来也最像话：**第一个「提出采用」的答案说了算**。
什么都不提供的窗格没有决定权。

### 顺带解开一个结构性缺口

这条路一直测不了：`AiChatService.complete` 是静态方法、直接联网，
**整条 `ai` 续跑路径（正是最近几个缺陷所在的地方）零覆盖**。
加了一个 `@visibleForTesting` 的替换点，测试可以给出模型的答案。

第一版复现（不带模型）**没能杀死变异**——因为它一次就返回了真窗格，
根本没走到占位那条路。有了替换点才复现出真实形状。
**「差不多的复现」会让你以为修好了。**

### 涉及文件

- `code/lib/ui/widgets/right_side_bar.dart` — 判据
- `code/lib/services/ai_chat_service.dart` — 测试替换点
- `code/lib/services/plugin_script_runtime.dart`、`plugin_command_service.dart`、
  `lib/ui/widgets/plugin_command_actions.dart` — BUG-401 的新动作
...
1. 六处全部接上 `answeredWithin`，主语各自写清楚
   ...
   `AiChatService.complete` 增加 `reply` 参数，与流式的 `idle` 分开——
   一个是等响应头，一个是等下一行。

   **`reply` 默认 10 分钟，远宽于 `idle` 的 120 秒**，这不是随手写的：
   非流式地问一个模型时，provider 通常**算完整个回答才发响应头**，
   所以这一段等的不是连接，是模型把一篇可能很长的文档想完。
   按一两分钟切会把今天能成功的长文翻译打断。它存在的意义是让
   「接了连接却永不出声」以报错收场，而不是无人能停的转圈——
   十分钟对这件事够短，对前一件事够长。
2. 守卫改成**计数契约**：每个文件里
...
## BUG-410：翻译整篇文档后，整篇文档留在插件的 settings.json 里
...
| 日期 | 2026-09-10 |
| 优先级 | P1 |
| 状态 | 已修复（插件侧修掉；编辑器侧加了可见性，不设硬上限） |

### 现象

通读官方插件的 Lua 之后实测：翻译一篇 **210 KB** 的文档，
`plugins/com.marktextplus.ai-translate/settings.json` 变成 **220 KB**，
其中 **209 636 字节是 `blocks` 键——整篇文档**。

```
PROBE: 文档 209778 字节
PROBE: settings.json = 219785 字节
PROBE:   blocks -> 209636 字节
PROBE:   其余各键 -> 7 到 498 字节
```

翻译结束后**没有任何东西清掉它**，它就一直躺在插件目录里，直到下一次翻译
覆盖它、或插件被卸载。

### 根因

插件按批翻译，需要在两次调用之间记住「还没译的部分」，而 `storage` 是它
唯一的状态。`remember()` 把整篇文档写进去是**对的**；错的是**译完之后没有放手**。

两层放大：

1. 这个文件**每次运行插件命令都要同步读一遍**（`_loadSettings`）。
   几兆的设置文件意味着每次用这个插件都卡一下——直接顶着「加载快」。
2. 读者的文档内容留在磁盘上，而没有任何地方说它在那里。

### 修复方案

**插件侧**（`plugin.lua`）：最后一批发出去之后就清掉副本。
...
中途放弃的翻译仍会留下一份，下一次翻译会覆盖它——**不能接受的是「译完了还留着」**。

**编辑器侧**：**不设硬上限**。拒绝写入会打断合法用途——正是这个插件在
按批翻译期间需要暂存整篇文档，那是对的做法。改为在超过一个
**任何真·设置都到不了的门槛**（4 MB）时写一条日志，说明是哪个插件、
多大、哪个键撑大的。让「插件为什么变慢了」查得出来，而不是加一道
会误伤的墙。

### 涉及文件

- `marktext-plus-ai-translate-plugin/plugin.lua`
- `code/lib/services/plugin_command_service.dart`
- `code/test/services/ai_translate_plugin_test.dart`（新增 1 条，跑真实插件）
- `code/test/services/plugin_settings_size_test.dart`（新增 3 条）

### 验证

| 变异 | 结果 |
|------|------|
| 插件不清理（退回今晚之前） | 「译完了还把整篇文档留在插件目录里」红 |
| 插件一开始就清（清得太早） | 「还没译完就把剩下的丢了」红 |
| 编辑器不再提示 | 提示那条红 |
| 提示里不说是哪个键 | 「要说清是哪个键撑大的」红 |

第二个变异是关键：**清得太早会丢掉没译的部分**，这是修这个 bug 时最容易
换来的另一个 bug，所以两个方向都要有测试。

---

## BUG-411：文档里写了 `{{instruction}}`，会被换成你刚输入的指令
...
对一篇内容为 `A: {{instruction}} B: {{text}} C: {{language}}` 的文档用「AI 写作」，
输入指令 `MAKE-IT-SHORT`，实测送到模型的是：

```
A: MAKE-IT-SHORT B: {{text}} C: {{language}}
```

读者的文档**被静默篡改**后才交给模型。改写结果带着篡改回来，点「采用」就写进正文。

写 Jinja、Handlebars、Vue 文档的人整天在写 `{{...}}`；这个插件自己的
提示词说明里也全是。

### 根因

`prompts.lua` 的 `build` 逐个键替换：
...
**替换进去的值会被后续的替换再扫一遍。** 文档作为 `{{text}}` 的值填进去之后，
下一轮的 `{{instruction}}` 替换扫到了它里面的花括号。

而哪个键先替换取决于 `pairs` 恰好给出的顺序——所以：

- 三条命令里**只有写作那条**今天会犯（翻译那条恰好 `text` 最后替换）
- 同一份文档两次运行**可能结果不同**

讽刺的是同一个文件里 `APPENDED` 那段的注释已经写着
「`pairs` 无序，同一条命令两次运行不该不一样」——**规则只应用到了当时看着的那个分支**，
这正是本库反复出现的形状。

### 修复方案

改成**单趟扫描模板**：走一遍 `system .. user`，遇到 `{{key}}` 就把值写进输出，
**写进去的值不再回头看**。顺带定死两件事：

- 顺序不再有影响（不再遍历 `values`）
- 认不出的 `{{name}}` **原样保留**——提示词是读者可编辑的，那对花括号可能是他自己的，
  填空等于静默删掉

### 涉及文件

- `marktext-plus-ai-translate-plugin/lib/prompts.lua`
- `code/test/services/ai_translate_plugin_test.dart`（新增 2 条，跑真实插件）

### 验证

| 变异 | 结果 |
|------|------|
| 退回逐个替换 | 「读者文档里的花括号是他自己的」红 |
| 认不出的占位符填空而非保留 | 保留那条红 |

顺带查了 SDK 的三份示例插件，没有同类写法（那里的 `{{}}` 是 JSDoc 类型注解）。

---

## BUG-412：SDK 权限表里五行承诺了编辑器从未做过的能力
...
| 日期 | 2026-09-11 |
| 优先级 | P2 |
| 状态 | 已修复（标注 + 守卫；能力本身要不要做仍待定） |

### 现象

BUG-409 查出十八个权限里十个没有执行点。这一条是它**对外的那一半**：
SDK 的 README 权限表写着

| 权限 | 表里写的 | 实际 |
|------|---------|------|
| `ui.toolbar` | 「加一个工具栏按钮」 | 编辑器**不画任何插件工具栏** |
| `ui.statusBar` | 「加一个状态栏项」 | 清单里根本没有状态栏这类贡献 |
| `ui.commandPalette` | 「往命令面板加命令」 | 命令面板读 `CommandRegistry`，没人往里注册插件命令 |
| `clipboard.read/write` | 「剪贴板」 | 两个运行时里**零引用** |
| `workspace.read/write` | 「用户打开的文件夹下的文件」 | 同上 |

五行、七个权限。作者读了一行、声明了权限、在 manifest 里写好条目、
然后什么都不会发生——**这是最贵的一类文档错误**。

而这份 README 有 11 种译文，全都这么写。

### 根因

权限的名字先于能力落地：清单能声明、安装对话框会展示、README 也照着描述了，
而后面那一步没做。没有任何东西把「README 怎么说」和「编辑器做了什么」对起来。

### 修复方案

**不删行**——它们是清单的一部分，将来做出来时会被兑现。改为在这五行上打 `†`，
并在表下加一段说明：这个权限是真的、也会展示给读者，但背后的能力还没有；
今天声明它，插件什么也得不到。12 种语言全部照做。

**锚点用 `†` 而不是某个英文词**：它在十二种译文里是同一个字符，而表下那段话不是。

**谁来决定哪几行该有 †**：编辑器的测试。这条守卫放在
`every_permission_is_enforced_test` 里——分类本来就在那儿，避免出现第三份清单。
把原来的 `unenforced` 拆成两张表：

- `noCapability`（7 个）：编辑器没做这个能力 → README 必须打 †
- `ungated`（1 个，`storage.local`）：能力在、只是没设卡 → 不打 †，因为
  「保留自己的设置文件」这句话是**真的**
...
- SDK 的 12 份 README + CHANGELOG
- `code/test/services/every_permission_is_enforced_test.dart`

### 验证

| 变异 | 结果 |
|------|------|
| 法文版去掉一个 †（谎称已实现） | 点名 `README_fr-FR.md` |
| 给 `ai.chat` 打上 †（它是实现了的） | 点名 `README.md` |

### 顺带被另一条守卫抓住

新测试第一版用 `if (sdk == null) return;` 跳过——`repo_dependent_tests_test`
...
## BUG-414：列表项里的围栏，内容像列表项时被拆成真列表项
...
预期是两项，第一项底下带一个代码块。实际解析成**三项**，围栏被劈成两个
**空**代码块，`- 假的` 升格成了真列表项。
...
**写 Markdown 教程的人天天这么写**——在一个步骤底下用代码块演示列表语法。

### 根因

`_collectListItems` 的循环，**第一个分支**问的是「这行是不是新列表项」，
而它**不知道自己在围栏里**：

```dart
while (i < lines.length) {
  if (_startsListItem(lines[i]) || ...) { /* 开新项 */ }
  ...
  if (blocks.isNotEmpty && _indentColumns(lines[i]) > 0) { /* 归入当前项 */ }
```

标题和引用之所以正确，是因为它们不像列表项，落到了下面那个「缩进行归入当前项」
的分支。**只有列表标记能走到上面那个分支**——所以这个缺陷只在一种内容上显形，
另外三种看起来都好好的。

### 修复方案

在循环里跟踪围栏状态，**只对缩进的开启行**（那才是属于某一项的围栏；
顶格的围栏是块解析器的事，原样不动）。闭合判断复用解析器已有的
`_closesFence`，不写第六份。

### 验证

六条测试，两个方向都覆盖：修好的两种、本来就对的三种（防止将来修上面那个
分支时把它们带坏）、**真正的嵌套列表仍然是列表**（另一种改坏法：把所有缩进行
都吞掉）、闭合规则（更长围栏里的短围栏是内容、`~~~` 同理）、顶格围栏不受影响。

变异：拿掉围栏跟踪 → 三条红。

---

## BUG-415：勾选复选框时按行计数，不认围栏，改错行
...
BUG-414 的另一半，也是它**用户直接看得见**的那一半。同一份文档：
...
勾第二个复选框（预览里画的就是 `- [ ] 二`），`[x]` **写进了代码块里那一行**，
而勾的那一项纹丝不动。读者的代码块被静默改写。

### 根因

`_toggleTask` 按序号找行的办法是**数所有看起来像列表项的行**：

```dart
for (var i = 0; i < lines.length; i++) {
  if (!md.MarkdownParser.continuesListItems(...)) continue;
  seen++;
  if (seen == index) { lineIndex = i; break; }
}
```

它扫的是该列表块的全部源码行，**没有任何围栏意识**。BUG-414 修好之后
解析器说「两项」，而这个循环仍数出三行——差一位，正好落在围栏里那一行。

**两个 bug 必须都修**：只修解析器，预览少画一个框，但勾选依旧写错行。

### 修复方案

计数前先取得逐行的围栏状态，围栏内的行一律跳过。用的是
`MarkdownSyntaxHighlighter.fenceStates`——**不写第六份围栏规则**，
而它与解析器的一致性由 `fence_rule_agreement_test` 按着。

### 涉及文件

- `code/lib/services/markdown_parser.dart`（BUG-414）
- `code/lib/ui/editor/markdown_renderer.dart`（BUG-415）
- `code/test/services/fence_inside_a_list_item_test.dart`（新增 6 条）
- `code/test/ui/editor/task_toggle_test.dart`（新增 1 条）

### 这两条是怎么找到的

不是扫出来的，是**问已有守卫的兄弟**：`fence_rule_agreement_test` 的注释
自己写着「这条规则写了五份」并点名了五处，而它只比了其中两份
（大纲 ↔ 源码窗格）。逐个去问剩下三份，链接定义扫描七种情形全对，
**前缀裁剪那份带出了这两个**。

守卫的注释里往往列着当年出事的清单——**那份清单本身就是下一轮的题目**。

---

## BUG-416：首页承诺的性能预算比实测更严；11 份译文说高亮是单遍
...
README 功能表的「大文件」一行，两处与事实不符，而且**分处在英文与译文两边**：

| 哪一份 | 写的 | 实际 |
|--------|------|------|
| 英文 | 「四倍文档不超过**六倍**开销」 | 测试断言的是 `lessThan(one * 8)`——**八倍** |
| **11 份译文** | 「解析、**语法高亮**和搜索均为单遍处理」 | 英文版早已订正：解析和搜索是单遍，**高亮是超线性的**，这正是它在 128 KB 以上停掉的原因 |

第一条的代价具体而隐蔽：**一个照着首页那句话做性能优化的贡献者，
会从「测试没红」里得到错误的安心**——他以为守卫在 6 倍处，实际在 8 倍。

第二条是「订正了原件，没订正副本」：`bugfix.md` 里 BUG-347 的记录
引用的正是当年那句 **Parsing, highlighting and search are all single-pass**，
说明英文后来改过，而 11 份译文停在了那一版。

### 根因

数字写在两处：测试里的字面量 `8`，和首页里的英文单词 six。没有任何东西对账。

上限当年从 6 提到 8（六在本机连过五次、在 CI 上以 6.05 挂掉，于是买了余量），
改的人动了测试没动 README——这是本库反复出现的形状，只是这次落在最多人读的那页上。

### 修复方案

1. 数字收进 `test/support/cost_limits.dart`（`costGrowthLimit` / `costSpan`），
   测试从那里取，**不再有字面量**。
2. 英文首页订正为 eight。
3. 11 份译文把「高亮」从「单遍处理」那份清单里去掉——它们那句话的后半
   （128 KB 以上停掉）本来就是对的，只需删掉列举里的一项。
4. 守卫 `the front page quotes the cost budget the suite enforces`：
   把常量映射成英文数词，去核首页那一行。**只核英文**——
   11 份译文描述这条预算时不引用数字，而**说得少的句子说不出假话**；
   哪天有译文写上了数字，再把它加进来。
5. `bugfix.md` 里 BUG-347 的旧记录**不改写**，只加一条带日期的补注说明后来改成了八倍。

### 验证

| 变异 | 结果 |
|------|------|
| README 退回「six」 | 「测试的上限是 8 倍，README 说的是别的」 |
| 把 `costGrowthLimit` 改成 7 | 「测试的上限是 7 倍，README 说的是别的」 |

第二个方向是关键：它证明守卫钉的是**常量**，而不是把 8 又抄了一遍。

### 顺带确认为准确的

同一行里「高亮在 128 KB 以上停掉」——`maxHighlightedLength = 128 * 1024`，对得上。

---

## BUG-417：验「打包出来的插件能不能跑」的四条测试，从未执行过
...
`packaged_plugin_test` 解压**读者真正会下载的那个压缩包**，把插件按安装的样子
铺开，再从包里跑起来。它的注释写着建它的缘由：

> 已经有两次，插件被「安装」成只复制了入口文件——那是半个插件。

四条断言：包里带齐入口 `require` 的三个模块、包里的版本与仓库一致、
**真的能从包里跑起来**、菜单与 12 种语言的字符串还在。

它们以 `PLUGIN_ZIP` 这个环境变量为开关，而**全库没有任何地方设置它**——
不在 workflow、不在脚本、也没有一行说明。**从写下那天起，它们在任何地方都没跑过。**

### 为什么这比「没写」更糟

跑测试时它们显示为 **skipped**。「跳过」读起来像「此处不适用」，
而不是「从来没有一处适用过」。于是这个项目的守卫清单上写着「打包已覆盖」，
而那一格是空的。

### 根因

守卫本身也是「写好了却没接上」的一例——本库反复出现的那个模式，
这次的对象是守卫。开关建好了，没人去开。

### 修复方案

1. **CI 里打包并设开关**：`analyze` 作业本来就检出了插件仓库，
   在跑测试前把它 `zip` 成压缩包并写进 `GITHUB_ENV`。
   Lua 插件没有构建步骤，**把目录打成 zip 就是发布本身**，
   所以这正是「读者会下载的那个包」（排除 `.git` 与 `.github`）。
2. **加一条防复发的守卫** `a_gated_test_is_a_test_that_runs_test`：
   测试里每一个 `Platform.environment['X']` 开关，`X` 都必须出现在
   CI 的 workflow 里。**以后加一个开关，就必须同时安排谁来打开它。**

### 涉及文件

- `.github/workflows/ci.yml`
- `code/test/a_gated_test_is_a_test_that_runs_test.dart`（新增）

### 验证

| 变异 | 结果 |
|------|------|
| 包里少 `lib/prompts.lua`（「半个插件」） | 「提示词模块不在包里，三个功能一个也跑不起来」红 |
| 包里的版本改成 9.9.9 | 「包里的版本和仓库里的对不上」红 |
| CI 里删掉打包那一步（退回今晚之前） | 元守卫点名 `PLUGIN_ZIP（test/services/packaged_plugin_test.dart）` |

本机带上 `PLUGIN_ZIP` 跑全量：**3096 通过，跳过从 5 条降到 1 条**。

### 剩下那一条跳过，是合理的——但同类

`plugin_js_runtime_test` 的「the engine runs a plugin end to end」只能在
**构建出来的应用里**跑（`flutter test` 里没有 QuickJS 库），所以 JS 运行时的
端到端路径同样从不自动执行。

**当天补了便宜的那一半**（2026-09-11）：证明它**能跑**要 `integration_test`
加无头显示；证明它**随包发出去了**只要一行。而后者才是现实中会发生的那种退化——
那个库是靠一个传递依赖的构建规则进来的，不是靠本仓库写的任何东西，
**升一次版就可能掉**。

于是 Linux 与 Windows 两个构建作业各加一步：产物里必须有
`libquickjs_c_bridge_plugin.so` / `quickjs_c_bridge.dll`，
找不到就把所有 `.so`／`.dll` 列出来再失败——**让下次是「名字变了」而不是一句干巴巴的报错**。
