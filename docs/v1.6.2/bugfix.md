...
| BUG-420 | 2026-09-11 | 录 3 秒 GIF 要等 52 秒，工具在自己文档写的上限上不可用 | P2 | 已修复 |
...
`record_gif` 是 MCP 五个工具里**唯一没有任何测试调用过**的一个
（其余四个至少有协议层的桩测试）。实机第一次调用它：

| 请求 | 帧数 | 往返 |
|------|------|------|
| 1 秒 × 5 fps | 5 | 10.5 秒 |
| 1 秒 × 20 fps | 20 | 32.8 秒 |
| **3 秒 × 10 fps（默认）** | 30 | **51.7 秒** |

而它的说明写着「**最多五秒**」——那将是约 90 秒，**超过多数 MCP 客户端的超时**。
我自己早先几次调用用的 25/30 秒超时，全都会失败。

**一个在自己文档写的上限上无法使用的工具。**

### 根因

不是抓帧慢。单独量一次 `screenshot`（抓帧 + PNG + base64 + HTTP）只要 **245 ms**，
30 帧抓图约 7 秒。那 45 秒花在 **GIF 编码**上——
`img.encodeGif` 的默认设置是**神经网络量化器 + Floyd–Steinberg 抖动**。

那套默认是为**照片**准备的。而这里编码的是一扇**平涂的界面窗口**。

### 用真实截图量出来的对比

十帧 1266×635，取自本会话真实抓到的编辑器截图：

| 设置 | 耗时 | 体积 | 平均色差 |
|------|------|------|---------|
| `encodeGif` 默认 | **22.7 秒** | 609 KB | 0.09 / 255 |
| octree + 不抖动 | **0.9 秒** | **381 KB** | **0.02 / 255** |

**默认设置在三个维度上全输**：慢 25 倍、文件大 60%、色差还更大。
给一张本来没有噪点的截图做抖动，噪点要占字节，也要拉开与原图的距离。

### 修复方案

`GifEncoder(quantizerType: octree, dither: none)`，并把编码从抓帧里拆出来
（`WindowCapture.encodeFrames`），这样**不需要窗口就能量**——
上面那张表正是这么量出来的。

### 验证

测试**按体积断言，不按时钟**：墙钟上限在快机器上抓不到、在慢机器上会无故变红
（`cost_stays_linear_test` 已论证），而字节数在哪台机器上都一样，
且表达的正是同一件事——噪点少了。

| 变异 | 结果 |
|------|------|
| 退回库的默认设置 | 红 |
| 只关抖动、量化器仍是 neural | 红（体积的好处也来自 octree） |

### fixture 第一版是错的，值得记

第一版 fixture 画的是**三种平涂颜色**，两种设置产出**byte 相同的 19309 字节**——
抖动无事可做，测试在拿一个东西和它自己比。

真实界面之所以有差别，是**抗锯齿的字形边缘**。给 fixture 加上渐变底和柔边之后，
差别才出现（105503 vs 84xxx）。

> **fixture 不像被测对象时，测试会以「通过」的形式失败。**
