...
|---------|-------------|
| **🔐 Permissões** | Declaradas no manifesto, mostradas na página da extensão e **impostas**. O VS Code e o IntelliJ mostram uma lista e depois confiam; aqui ninguém revê nada, por isso quem verifica é o editor. Uma extensão que pergunta ao modelo sem `ai.chat` recebe um não, e você fica a saber que ela tentou |
...
| **✍️ Escrever de volta** | Uma extensão pode reescrever o que você selecionou — depois de lho mostrar. O resultado vem com um botão Aplicar, e aplicar passa pelo histórico do editor: um desfazer traz tudo de volta A resposta aparece à medida que é escrita, não depois de terminada. |
| **⚙️ Definições próprias** | O editor desenha a página a partir do que a extensão declarou: um interruptor para um interruptor, um campo oculto para um segredo. A extensão descreve o que quer; os componentes são do editor |
| **🎛️ Interface própria** | Não só uma página de definições: um comando pode responder com uma árvore de textos, campos, fichas, listas, caixas, Markdown, imagens e botões, que o editor desenha com os seus próprios componentes no quadro de onde o comando partiu. Carregar num deles devolve todos os campos da árvore, por isso a extensão não tem de se lembrar do formulário que desenhou |
...
| **`get_state`** | Que versão está a correr — pergunte depois de `update_app` para ver o que voltou — e o que está aberto: separadores, modo de vista, extensões instaladas, quadros preenchidos |
| **`read_startup_trace`** | Quanto tempo levou cada passo do arranque, incluindo a parte antes do Dart |
| **`control`** | 18 ações: abrir, fechar, guardar e mudar de separador, escrever conteúdo, mudar o modo de visualização, fechar um painel, executar um comando de um plugin, abrir o painel de um plugin, mudar uma opção, reabrir um separador fechado, mover e redimensionar a janela — **e instalar um plugin ou substituir a aplicação por uma compilação da CI** |
...
| **Dependências diretas** | 27 | 56 (pacote desktop) | código fechado |
