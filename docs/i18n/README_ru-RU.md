...
| | |
|---------|-------------|
| **🧩 Открытые плагины** | Один файл на Lua или JavaScript, без SDK и без сборки, в песочнице и в рамках прав, которые он сам заявил. Ищутся через [GitHub Topic: `marktext-plus-plugin`](https://github.com/topics/marktext-plus-plugin); каждый помечен как «сообщество / не проверено» |
| **⚡ Быстрый запуск** | Ни встроенного браузера, ни редакторского фреймворка — 27 прямых зависимостей |
| **📄 Большие файлы** | Разбор и поиск — за один проход, с бюджетами, на которых падают тесты. Свыше 128 КБ подсветка отключается: это последний размер, который открывается примерно за секунду. Редактировать по-прежнему можно, и цвета вернутся, когда файл уменьшится |
| **🧪 Покрыто тестами** | Более 2600 тестов на разборщик, экспорт, провайдеры, среды плагинов и виджеты редактора |

### Плагины

Пишутся на Lua или JavaScript — один файл и манифест, никакой сборки, и тот же файл работает на всех трёх платформах. Плагин может состоять из нескольких файлов, а `require` достаёт только до его собственного каталога.

| Возможность | Описание |
|---------|-------------|
| **🔐 Права** | Заявлены в манифесте, показаны на странице плагина и **соблюдаются**. VS Code и IntelliJ показывают список и дальше доверяют; здесь никто ничего не проверяет, поэтому проверяет редактор. Плагину, который обратился к модели без `ai.chat`, отвечают отказом, а вам сообщают, что он обращался |
| **🪟 Области** | Редактор и так делит вкладку между исходником и предпросмотром; это деление предложено наружу. До четырёх ячеек, разделители тянутся, а для ячейки, которую никто не заполнил, не рисуется ничего |
| **✍️ Запись обратно** | Плагин может переписать выделенное — показав вам сначала. Результат приходит с кнопкой «Применить», и применение идёт через историю редактора: одна отмена возвращает всё назад Ответ появляется по мере того, как пишется, а не после того, как дописан. |
| **⚙️ Свои настройки** | Редактор рисует страницу по тому, что заявил плагин: переключатель для переключателя, скрытое поле для секрета. Плагин говорит, чего он хочет; виджеты — редактора |
| **🎛️ Свой интерфейс** | Не только страница настроек: команда может ответить деревом из текста, полей, фишек, списков, флажков, Markdown, картинок и кнопок, которое редактор рисует своими виджетами в той области, откуда команда пришла. Нажатие возвращает все поля дерева сразу, так что плагину не нужно помнить нарисованную форму |
| **🌍 Свои языки** | Плагин несёт столько языков, сколько захотел его автор, — независимо от двенадцати, на которых говорит редактор |
| **🔑 Ключи не передаются** | Учётные данные держит редактор, и запрос делает он же. Плагин даёт запрос и получает текст |

Начните с [SDK для плагинов](https://github.com/marktext-plus-plugins/marktext-plus-plugin-sdk) — три готовых примера, которые можно скопировать, и документация на одиннадцати языках.

### Для ИИ-агентов

Необязательный MCP-сервер, **выключенный, пока вы его не включите**: он открывает порт на вашей машине, и всякий, кто до него доберётся, сможет читать ваши документы и управлять редактором. Поэтому включаете его вы, и он несёт токен, который можно сменить.

| Инструмент | Описание |
|---------|-------------|
| **`read_logs`** | Журнал редактора и его плагинов, с отбором по плагину и по важности |
| **`screenshot`** | Окно, как оно выглядит сейчас |
| **`record_gif`** | Не больше пяти секунд — чтобы посмотреть анимацию |
| **`get_state`** | Какая версия запущена — спросите после `update_app`, чтобы увидеть, что вернулось, — и что открыто: вкладки, режим просмотра, установленные плагины, заполненные области |
| **`read_startup_trace`** | Сколько занял каждый шаг запуска, включая часть до старта Dart |
| **`control`** | 19 действий: открыть, закрыть, сохранить и переключить вкладку, записать содержимое, сменить режим просмотра, закрыть панель, выполнить команду плагина, открыть панель плагина, изменить настройку, открыть закрытую вкладку, переместить окно и изменить его размер — **а также установить плагин или заменить приложение сборкой из CI** |

19 действий — под теми именами, которые отправляет агент:
...
| **Прямых зависимостей** | 27 | 56 (пакет desktop) | закрытый код |
