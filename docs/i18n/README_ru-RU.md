...
| **🧪 Покрыто тестами** | 2417 тестов на разборщик, экспорт, провайдеры, среды плагинов и виджеты редактора |
...
|---------|-------------|
| **🔐 Права** | Заявлены в манифесте, показаны до установки и **соблюдаются**. VS Code и IntelliJ показывают список и дальше доверяют; здесь никто ничего не проверяет, поэтому проверяет редактор. Плагину, который обратился к модели без `ai.chat`, отвечают отказом, а вам сообщают, что он обращался |
