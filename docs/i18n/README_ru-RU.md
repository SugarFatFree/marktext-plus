...
| **⚡ Быстрый запуск** | Ни встроенного браузера, ни редакторского фреймворка — 26 прямых зависимостей |
| **📄 Большие файлы** | Разбор, подсветка и поиск — за один проход, с бюджетами, на которых падают тесты. Свыше 128 КБ подсветка отключается: это последний размер, который открывается примерно за секунду. Редактировать по-прежнему можно, и цвета вернутся, когда файл уменьшится |
...
| **✍️ Запись обратно** | Плагин может переписать выделенное — показав вам сначала. Результат приходит с кнопкой «Применить», и применение идёт через историю редактора: одна отмена возвращает всё назад |
...
| **Прямых зависимостей** | 26 | 56 (пакет desktop) | закрытый код |
