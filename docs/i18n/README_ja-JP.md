...
| **⚡ 高速起動** | 埋め込みブラウザもエディタフレームワークもなく、直接依存は 27 個 |
| **📄 大きなファイル** | 解析と検索は一度きりの走査で、遅くなればテストが落ちる予算つき。128 KB を超えるとハイライトを諦めます——一秒ほどで開ける最後の大きさです。編集は変わらずでき、小さくなれば色は戻ります |
| **🧪 テスト** | パーサー、エクスポーター、provider、プラグインランタイム、エディター widget を 2600 を超えるテストで |
...
| 機能 | 説明 |
|---------|-------------|
| **🔐 権限** | マニフェストに書き、プラグインのページで示し、そして**強制します**。VS Code や IntelliJ は一覧を見せて拡張を信用しますが、ここでは誰も審査しないので、エディタが確かめます。`ai.chat` なしにモデルを呼んだプラグインは断られ、呼んだことがあなたに伝わります |
| **🪟 ペイン** | エディタはもともとタブをソースとプレビューに分けています。それを開いたものです。最大四つ、区切りはドラッグでき、誰も埋めていないマスは描かれません |
| **✍️ 書き戻し** | 選択したところをプラグインが書き直せます——ただし先に見せてから。結果には「適用」ボタンが付き、適用はエディタの履歴を通るので、取り消し一回で戻ります答えは書き上がってからではなく、書かれるそばから現れます。 |
| **⚙️ 自分の設定** | プラグインが宣言したとおりに、エディタが設定ページを描きます。スイッチにはスイッチ、秘密には伏せ字の入力欄。プラグインは何が欲しいかを述べ、ウィジェットはエディタのものです |
| **🎛️ 自分の画面** | 設定ページだけではありません。コマンドは、テキスト・入力欄・チップ・ドロップダウン・チェックボックス・Markdown・画像・ボタンからなる木を返せて、エディタがそれを自分のウィジェットとして、コマンドが始まったペインに描きます。ボタンを押すと木の中の全項目がまとめて返るので、一歩前に描いた入力欄を覚えておく必要はありません |
...
| **`control`** | 19 のアクション：タブを開く・閉じる・保存する・切り替える、内容を書き込む、表示モードを変える、ペインを閉じる、プラグインのコマンドを実行する、プラグインのパネルを開く、設定を変える、閉じたタブを開き直す、ウィンドウを動かして大きさを変える、**さらにプラグインのインストールと、CI のビルドによるアプリ本体の置き換え** |

エージェントが送る名前での 19 のアクション：
...
| **直接依存の数** | 27 | 56（desktop パッケージ） | クローズドソース |
