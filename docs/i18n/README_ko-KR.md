...
| **📄 큰 파일** | 파싱, 하이라이팅, 검색 모두 한 번의 훑기로 처리하며 느려지면 실패하는 예산이 있습니다. 128 KB를 넘으면 하이라이팅을 포기합니다 — 1초 남짓에 열리는 마지막 크기입니다. 편집은 그대로 되고, 작아지면 색이 돌아옵니다 |
...
| **✍️ 문서에 쓰기** | 선택한 부분을 플러그인이 고쳐 쓸 수 있습니다 — 다만 먼저 보여 주고 나서. 결과에는 「적용」 단추가 붙고, 적용은 에디터의 기록을 거치므로 되돌리기 한 번이면 돌아옵니다 |
