...
| **🧪 테스트** | 파서, 내보내기, provider, 플러그인 런타임, 편집기 위젯을 2417개 테스트로 |
...
| 기능 | 설명 |
|---------|-------------|
| **🔐 권한** | 매니페스트에 적고, 설치 전에 보여 주고, 그리고 **강제합니다**. VS Code와 IntelliJ는 목록을 보여 준 뒤 확장을 믿지만, 여기서는 아무도 심사하지 않으므로 에디터가 확인합니다. `ai.chat` 없이 모델을 부른 플러그인은 거절되고, 불렀다는 사실이 당신에게 전해집니다 |
