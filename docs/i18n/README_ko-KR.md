...
| | |
|---------|-------------|
| **🧩 열린 플러그인** | Lua나 JavaScript 파일 하나. SDK도 빌드도 필요 없고, 샌드박스에서 돌며 스스로 선언한 권한만 가집니다. [GitHub Topic: `marktext-plus-plugin`](https://github.com/topics/marktext-plus-plugin)에서 찾을 수 있고, 모두 커뮤니티/미검증으로 표시됩니다 |
| **⚡ 빠른 시작** | 내장 브라우저도 에디터 프레임워크도 없이, 직접 의존성 27개 |
| **📄 큰 파일** | 파싱과 검색은 한 번의 훑기로 처리하며 느려지면 실패하는 예산이 있습니다. 128 KB를 넘으면 하이라이팅을 포기합니다 — 1초 남짓에 열리는 마지막 크기입니다. 편집은 그대로 되고, 작아지면 색이 돌아옵니다 |
| **🧪 테스트** | 파서, 내보내기, provider, 플러그인 런타임, 편집기 위젯을 2600개가 넘는 테스트로 |

### 플러그인

Lua나 JavaScript로 씁니다. 파일 하나와 매니페스트, 빌드 없음, 같은 파일이 세 플랫폼에서 돕니다. 필요하면 여러 파일이 될 수 있고, `require`는 자기 디렉터리 안에만 닿습니다.

| 기능 | 설명 |
|---------|-------------|
| **🔐 권한** | 매니페스트에 적고, 플러그인 페이지에서 보여 주고, 그리고 **강제합니다**. VS Code와 IntelliJ는 목록을 보여 준 뒤 확장을 믿지만, 여기서는 아무도 심사하지 않으므로 에디터가 확인합니다. `ai.chat` 없이 모델을 부른 플러그인은 거절되고, 불렀다는 사실이 당신에게 전해집니다 |
| **🪟 창** | 에디터는 원래 탭을 소스와 미리보기로 나눕니다. 그것을 열어 준 것입니다. 최대 네 칸, 구분선은 끌 수 있고, 아무도 채우지 않은 칸은 그리지 않습니다 |
| **✍️ 문서에 쓰기** | 선택한 부분을 플러그인이 고쳐 쓸 수 있습니다 — 다만 먼저 보여 주고 나서. 결과에는 「적용」 단추가 붙고, 적용은 에디터의 기록을 거치므로 되돌리기 한 번이면 돌아옵니다 답은 다 쓰인 뒤가 아니라 쓰이는 대로 나타납니다. |
| **⚙️ 자기 설정** | 플러그인이 선언한 대로 에디터가 설정 페이지를 그립니다. 스위치에는 스위치, 비밀에는 가려진 입력란. 플러그인은 무엇을 원하는지 말할 뿐이고, 위젯은 에디터의 것입니다 |
| **🎛️ 자기 화면** | 설정 페이지만이 아닙니다. 명령은 글·입력란·칩·드롭다운·체크박스·Markdown·그림·버튼으로 된 나무를 돌려줄 수 있고, 에디터가 그것을 자기 위젯으로 그 명령이 시작된 창에 그립니다. 버튼을 누르면 나무 안의 모든 항목이 함께 돌아오므로, 플러그인은 한 걸음 전에 그린 양식을 기억할 필요가 없습니다 |
| **🌍 자기 언어** | 플러그인은 만든 사람이 원하는 만큼의 언어를 스스로 가집니다. 에디터가 쓰는 열두 가지와는 무관합니다 |
| **🔑 열쇠는 넘어가지 않음** | 자격 증명은 에디터가 쥐고 요청도 에디터가 보냅니다. 플러그인은 프롬프트를 주고 글을 받습니다 |

[플러그인 SDK](https://github.com/marktext-plus-plugins/marktext-plus-plugin-sdk)에서 시작하세요 — 그대로 베낄 수 있는 완전한 예제 셋과, 열한 언어의 문서가 있습니다.

### AI 에이전트를 위해

선택 사항인 MCP 서버이며, **켜지 않으면 돌지 않습니다**. 당신의 기계에 포트를 여는 일이고, 거기에 닿는 것은 당신의 문서를 읽고 에디터를 몰 수 있으므로, 직접 켜야 하고 다시 만들 수 있는 토큰을 지닙니다.

| 도구 | 설명 |
|---------|-------------|
| **`read_logs`** | 에디터와 플러그인의 로그. 플러그인별, 심각도별로 좁힐 수 있습니다 |
| **`screenshot`** | 지금 창의 모습 |
| **`record_gif`** | 최대 5초. 움직임을 보기 위한 것 |
| **`get_state`** | 열린 탭, 보기 모드, 설치된 플러그인, 채워진 창 |
| **`read_startup_trace`** | 시작의 각 단계가 걸린 시간 — Dart가 실행되기 전 구간까지 |
| **`control`** | 열두 가지 동작: 탭 열기·닫기·저장·전환, 내용 쓰기, 보기 모드 변경, 창 닫기, 플러그인 명령 실행, 플러그인 패널 열기, 설정 변경, **그리고 플러그인 설치와 CI 빌드로 앱 자체를 교체하는 것** |
...
| **직접 의존성 수** | 27 | 56(desktop 패키지) | 비공개 소스 |
