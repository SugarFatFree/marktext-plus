...
| | |
|---------|-------------|
| **🧩 إضافات مفتوحة** | ملف Lua أو JavaScript واحد، بلا حزمة تطوير وبلا بناء، يعمل في صندوق رملي ولا يملك إلا ما أعلنه من أذونات. تُكتشف عبر [GitHub Topic: `marktext-plus-plugin`](https://github.com/topics/marktext-plus-plugin)، وكلٌّ منها موسوم بـ «مجتمعية / غير مُوثَّقة» |
| **⚡ بدء سريع** | لا متصفح مضمَّن ولا إطار محرِّر، و27 اعتمادًا مباشرًا |
| **📄 الملفات الكبيرة** | التحليل والبحث بمرور واحد، وبميزانيات تسقط الاختبارات إن تجاوزتها. وفوق 128 كيلوبايت يتوقف التلوين — وهو آخر حجم يُفتح في نحو ثانية. والتحرير يبقى كما هو، وتعود الألوان إن صغُر الملف |
| **🧪 مُختبَر** | أكثر من 2600 اختبار تغطي المحلل والمصدّرات وال providers وبيئات الإضافات وعناصر المحرِّر |

### الإضافات

تُكتب بـ Lua أو JavaScript — ملف وبيان، بلا بناء، والملف نفسه يعمل على المنصّات الثلاث. وللإضافة أن تكون عدة ملفات، و`require` لا يبلغ إلا داخل مجلدها.

| الميزة | الوصف |
|---------|-------------|
| **🔐 الأذونات** | مُعلَنة في البيان، ومعروضة في صفحة الإضافة، و**مُطبَّقة**. يعرض VS Code وIntelliJ قائمةً ثم يثقان؛ وهنا لا يراجع أحد شيئًا، فالمحرِّر هو الذي يتحقق. والإضافة التي تسأل النموذج بلا `ai.chat` تُمنَع، ويُقال لك إنها حاولت |
| **🪟 المساحات** | المحرِّر يقسم اللسان أصلًا بين المصدر والمعاينة؛ وهذا التقسيم مُتاح للإضافات. أربع خانات على الأكثر، وفواصل تُسحب، ولا يُرسم شيء لخانة لم يملأها أحد |
| **✍️ الكتابة في المستند** | للإضافة أن تعيد صياغة ما حدَّدته — بعد أن تعرضه عليك. ويأتي الناتج ومعه زر «تطبيق»، والتطبيق يمرّ بسجل المحرِّر، فتراجعٌ واحد يعيد كل شيء تظهر الإجابة أثناء كتابتها، لا بعد اكتمالها. |
| **⚙️ إعداداتها الخاصة** | يرسم المحرِّر صفحة الإعدادات مما أعلنته الإضافة: مفتاح للمفتاح، وحقل مخفيّ للسرّ. الإضافة تصف ما تريد، والعناصر عناصر المحرِّر |
| **🎛️ واجهتها الخاصة** | وليس صفحة الإعدادات وحدها: يمكن لأمرٍ أن يُجيب بشجرةٍ من نصوص وحقول وشرائح وقوائم منسدلة ومربّعات اختيار وMarkdown وصور وأزرار، يرسمها المحرِّر بعناصره في المساحة التي انطلق منها الأمر. وضغط زرٍّ يعيد كل حقول الشجرة معًا، فلا تحتاج الإضافة أن تتذكّر ما رسمته قبل خطوة |
| **🌍 لغاتها الخاصة** | تحمل الإضافة ما شاء مؤلفها من اللغات، بمعزل عن اثنتي عشرة يتكلمها المحرِّر |
| **🔑 مفاتيحك لا تُسلَّم** | المحرِّر يمسك بالاعتمادات ويرسل الطلب. والإضافة تعطي الموجِّه وتتلقى نصًّا |

ابدأ من [حزمة تطوير الإضافات](https://github.com/marktext-plus-plugins/marktext-plus-plugin-sdk) — ثلاثة أمثلة كاملة تُنسخ كما هي، وتوثيق بإحدى عشرة لغة.

### لوكلاء الذكاء الاصطناعي

خادم MCP اختياري، **متوقف حتى تُشغِّله**: فهو يفتح منفذًا على جهازك، وكل من بلغه استطاع أن يقرأ مستنداتك وأن يقود محرِّرك. لذلك أنت من يُشغِّله، وهو يحمل رمزًا يمكنك توليده من جديد.

| الأداة | الوصف |
|---------|-------------|
| **`read_logs`** | سجل المحرِّر وسجلات إضافاته، مع التصفية بحسب الإضافة وبحسب الخطورة |
| **`screenshot`** | النافذة كما هي الآن |
...
| **`control`** | 19 إجراءً: فتح التبويبات وإغلاقها وحفظها والتبديل بينها، كتابة المحتوى، تغيير طريقة العرض، إغلاق لوح، تنفيذ أمر إضافة، فتح لوح إضافة، تغيير إعداد، إعادة فتح لسان أُغلق، تحريك النافذة وتغيير حجمها — **وكذلك تثبيت إضافة أو استبدال التطبيق نفسه ببناء من CI** |

الإجراءات الـ 19، بالأسماء التي يرسلها الوكيل:
...
| **الاعتماديات المباشرة** | 27 | 56 (حزمة desktop) | مغلق المصدر |
