...
| **🧪 测试充分** | 2417 项测试覆盖解析器、导出器、providers、插件运行时和编辑器组件 |
...
| 功能 | 说明 |
|---------|-------------|
| **🔐 权限** | 写在 manifest 里，安装前给你看，并且**强制执行**。VS Code 和 IntelliJ 列出权限然后信任扩展；这里没有任何人做审核，所以由编辑器来查。没有 `ai.chat` 却去问模型的插件会被拒绝，而且会告诉你它问了 |
| **🪟 窗格** | 编辑器本来就把标签页分成源码和预览，这个能力开放出来：最多四格，分隔条可拖，没人填的格子不画 |
| **✍️ 写回文档** | 插件可以改写你选中的内容——但会先给你看。结果带一个「采用」按钮，采用会走编辑器的历史栈，撤销一下就回来 |
| **⚙️ 自己的设置** | 编辑器按插件声明的字段画设置页：开关就是开关，密码就是隐藏输入框。插件提供数据，从不提供控件 |
