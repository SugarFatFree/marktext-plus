...
| **⚡ 快速启动** | 没有内嵌浏览器和编辑器框架，只有 27 个直接依赖 |
| **📄 大文件** | 解析和搜索均为单遍处理，并由性能测试设置预算。超过 128 KB 关闭语法高亮——那是还能在一秒左右打开的最后一个尺寸；文档照常可编辑，缩小后颜色会回来 |
| **🧪 测试充分** | 2600 多项测试覆盖解析器、导出器、providers、插件运行时和编辑器组件 |

### 插件

用 Lua 或 JavaScript 写——一个文件加一份 manifest，无需构建，同一个文件在三个平台上都能跑。需要时插件可以有多个文件，`require` 只能触及它自己的目录。

| 功能 | 说明 |
|---------|-------------|
| **🔐 权限** | 写在 manifest 里，插件页面上列给你看，并且**强制执行**。VS Code 和 IntelliJ 列出权限然后信任扩展；这里没有任何人做审核，所以由编辑器来查。没有 `ai.chat` 却去问模型的插件会被拒绝，而且会告诉你它问了 |
| **🪟 窗格** | 编辑器本来就把标签页分成源码和预览，这个能力开放出来：最多四格，分隔条可拖，没人填的格子不画 |
| **✍️ 写回文档** | 插件可以改写你选中的内容——但会先给你看。结果带一个「采用」按钮，采用会走编辑器的历史栈，撤销一下就回来答案是边写边出现的，不是等写完才给你。 |
| **⚙️ 自己的设置** | 编辑器按插件声明的字段画设置页：开关就是开关，密码就是隐藏输入框。插件描述它要什么，控件是编辑器的 |
| **🎛️ 自己的界面** | 不止设置页：一条命令可以返回一棵由文字、输入框、标签、下拉框、复选框、Markdown、图片和按钮组成的树，由编辑器用自己的控件画在命令发起的那个窗格里。按下按钮时树里每个字段都会一并回传，插件不必记住它上一步画了什么 |
| **🌍 自己的语言** | 插件自带它作者想要的任何语言，与编辑器的十二种无关 |
| **🔑 永远拿不到你的密钥** | 编辑器持有凭据并发出请求。插件给出提示词，拿回文本 |

从[插件 SDK](https://github.com/marktext-plus-plugins/marktext-plus-plugin-sdk) 开始——三个可直接照抄的完整示例插件，以及十一种语言的文档。
...
| 工具 | 说明 |
|---------|-------------|
| **`read_logs`** | 编辑器和插件的日志，可按插件和严重程度过滤 |
| **`screenshot`** | 当前窗口的样子 |
| **`record_gif`** | 最长五秒，用来看动画效果 |
| **`get_state`** | 在跑的是哪个版本（`update_app` 之后问它，就知道回来的是哪个），以及开了哪些标签、视图模式、装了哪些插件、窗格里有什么 |
| **`read_startup_trace`** | 启动每一步花了多久，包括 Dart 还没跑起来的那一段 |
| **`control`** | 14 个动作：开关标签、保存、切换标签、写入内容、切换视图模式、关闭窗格、运行插件命令、打开插件面板、修改设置、重新打开关闭的标签页、移动和调整窗口大小，**以及安装插件、用 CI 的构建替换应用本体** |
