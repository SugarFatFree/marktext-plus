...
| BUG-125 | 2026-08-29 | 段落保留行首空白（预览里可见）、空链接目标、不同符号的列表被并成一个 | P2 | 已修复 |
| BUG-126 | 2026-08-29 | 简写引用链接 `[foo]` 完全不支持 | P2 | 已修复 |
| BUG-127 | 2026-08-29 | BUG-124 修得不完整：`**` 拒绝后单星号分支接手，产生带星号的斜体 | P2 | 已修复 |
| BUG-128 | 2026-08-29 | 表格紧跟在段落下面时整块被当成段落，表格不渲染 | P2 | 已修复 |
| BUG-129 | 2026-08-29 | 引用块里的任务列表：复选框点了没反应 | P2 | 已修复 |
| BUG-130 | 2026-08-29 | 引用/列表里的图表：工具栏四个按钮要僵死 300ms | P2 | 已修复 |
...
## BUG-125：段落行首空白、空链接目标、列表切分

继续用 CommonMark 语料推进，这次是 Links 与 List items 两类。

### 先修比较口径

列表那批失败**几乎全是我的比较口径问题**：CommonMark 保留代码块内容末尾的换行，
本项目的导出器不保留，于是每个含代码块的例子都因为一个尾部空格而算失败——
把真正的列表问题全遮住了。渲染上两者毫无差别，归一化掉之后：**259 → 308**。

**这不是修好了 49 个 bug，是我之前少数了 49 个通过项。** 记在这里以免日后误读。
...
HTML 会把行首空格折叠，所以**导出看起来是对的**；但**预览画的是 Flutter 的
`Text`，空格会实实在在显示出来**。列表项下面的段落、或随手缩进两格的段落，
在预览里都会明显偏右。

按 CommonMark 逐行去掉行首空白。308 → 313。

### 其二：链接目标不能为空

`[TODO]()` 是很常见的占位写法，目标部分的正则用了 `+`（至少一个字符），
于是整个链接**退回成字面文本**。改为 `*`。

### 其三：换用不同的项目符号没有开始新列表

CommonMark 规定换一个标记字符就是另一个列表——想让两个列表挨着写的人正是这么
分隔它们的。`_startsAnotherList` 只比较了"有序/无序"，没比较标记字符，
于是 `- a` `- b` `+ c` 合成了一个列表。313 → 315。

### 涉及文件

- `code/lib/services/markdown_parser.dart`
- `code/test/services/commonmark_spec_test.dart`（下限提到 315，新增 3 条）
...
本项目支持 `[文字][标签]` 与 `[标签][]` 两种引用形式，**唯独没有最常用的简写
形式 `[标签]`**——而把定义集中放在文末、正文里直接写 `[标签]` 正是引用链接
最典型的用法（README 尤其如此）。图片的 `![标签]` 同样缺失。

### 修复方案

在总正则末尾追加两条分支（追加而非插入，组编号不动）。**未定义的标签原样留下**
——正文里大量的方括号不是链接（`[sic]`、`[1]`、随手的批注），把它们变成指向
不存在目标的链接，比不支持简写更糟。

### 连带修好的一处

新分支使未定义的 `[a [b] c]` 走"原样输出为文本 span"的路径，于是原本一个文本
span 被拆成了三个——一条既有测试（"后面没有目标的方括号仍然是纯文本"）因此变红。

内容是对的，拆开只是 span 更碎。正确的处理是**合并相邻的纯文本 span**，
而不是放宽那条断言：任何"匹配了但解析不出东西"的分支都会产生这种碎片，
而每个下游消费者都得多走几段。已在 `parseInline` 收尾处统一合并。

### 比较口径又修一处

`<img … />` 与 `<img …>` 的空元素写法差异——之前归一化了 `<hr>` 和 `<br>`
却漏了 `<img>`，Images 那一类 22 例里大部分是因此判失败的。

### 得分

315 → **341**（口径修正与真实修复各占一部分）。

### 涉及文件

- `code/lib/services/markdown_parser.dart`
- `code/test/services/commonmark_spec_test.dart`（下限提到 341，新增 3 条）


---

## BUG-127：BUG-124 修得不完整，而我写的测试没抓住

### 现象

BUG-124 给 `**` `***` `__` `___` 补上了"分隔符内侧不能是空白"的规则。但补完之后：

| 输入 | 修 BUG-124 之前 | 修完之后 | 应该是 |
|---|---|---|---|
| `** foo bar**` | `<strong> foo bar</strong>` | **`<em>* foo bar</em>*`** | 原样 |
| `2 ** 3 ** 4` | 3 被加粗 | **`<em>* 3 *</em>`** | 原样 |
| `**foo bar **` | 加粗 | **`<em>*foo bar *</em>`** | 原样 |

`**` 分支按新规则拒绝了，**单星号分支立刻接了过去**——结果是带可见星号的斜体，
比原来的"加粗"还难看。**问题被挪了位置，没有被解决。**
...
**只断言"没有加粗"**。而实际输出是斜体——断言通过了，问题还在。

一条只说"不该是 X"的断言，在结果变成 Y 时会安静地放行。已改为断言**实际输出的
文本与类型**：`2 ** 3 ** 4` 必须是单个纯文本 span 且内容一字不差，
同时四种正常写法（`**x**` `*x*` `__x__` `_x_`）必须给出对应的类型。

### 根因

CommonMark 的规则是按**分隔符串**（delimiter run）来的：`**` 是一串两个，
不能从中间劈开用。而本项目用正则逐个分支匹配，单星号分支不知道自己踩在一串
里面。

补上两处零宽断言：开分隔符后面不能紧跟同种字符，闭分隔符前面也不能。
`_` 同理。
...
- `code/lib/services/markdown_parser.dart`
- `code/test/services/commonmark_spec_test.dart`（下限 352，断言强化）
...
CommonMark 规范里**没有表格**，所以上一轮建立的 648 例语料对表格一字未提。
表格是日常用得最多的结构之一，于是按 GFM 的规则单独穷举 16 种写法。

**参照物用的是真实现，不是我的记忆**：本机
`marktext-light/node_modules` 里有 `marked`，直接跑它对同一批输入的输出来比。
这一步很值——16 种里有两种，我原本以为是 bug 的，跑完发现本项目和 marked
行为一致：

- `| \`x|y\` | 2 |`：代码里的竖线**仍然切分单元格**。看起来不对，但 GFM 是先
  按竖线切格、再解析行内标记的，反引号保护不了它。marked 也切成两格。
- 一行单元格多于表头时**丢弃多余的**，少于时**补空**。两边一致。
...
**整块被当成一个段落，表格不渲染。** marked（以及 GitHub）的结果是
段落 `文字` + 一张表。

段落的终止条件里列了标题、分隔线、代码围栏、引用、有序/无序列表，
**唯独没有表格**。中间空一行就正常——所以这个问题只在"忘了空行"时出现，
而那正是最容易发生的情况。

### 修复方案

把表格的判定条件（表头行 + 分隔行 + 两者列数相同）抽成 `_startsTable`，
块循环与段落循环共用同一个判断——不是在段落循环里再写一份。

### 涉及文件

- `code/lib/services/markdown_parser.dart`
- `code/test/services/gfm_table_test.dart`（新增，9 条）


---

## BUG-129：引用块里的任务列表，复选框点了没反应

### 排查方式

任务列表有**交互**——点复选框会改写文档——所以出错代价比单纯渲染大。
先按 10 种形态过解析层，全部正确；其中 `- [ ] `（复选框后什么都没有）
被当成普通项，**用 marked 核实后与参考实现一致，不是 bug**。

真正的问题在交互层，而且是**两个叠在一起**。

### 其一：找不到那一行

`_toggleTask` 靠"数第几个列表项行"定位要改的行。引用里的列表，
`sourceOfBlock` 返回的是**带 `>` 前缀的原始行**（必须带，因为要原样写回），
而 `startsListItem('> - [ ] 一')` **不匹配**。于是 `lineIndex` 恒为 -1，
函数**静默返回**——点了没反应，没有报错，没有任何迹象说明发生过什么。

修法：判定时剥掉 `>` 前缀，写回时用原行。

### 其二：手势被外层的双击识别器吃掉

修好其一之后，测试**仍然不过**。加长等待时间到 600ms 才通过——说明
复选框要等双击超时才响应。

代码里对**顶层**任务列表早有这条排除，注释写得很清楚：

> 把它包进双击识别器会让识别器进入手势竞技场，在双击超时前一直不让出——
> 于是每个复选框都要先僵死约 300ms。勾选是远更频繁的动作，所以它优先。

但那个判断写的是 `node is ListNode && node.items.any(isTask)`——
**只认"自己就是任务列表"**。引用里的任务列表，被包装的是**引用块**，
判断不成立，于是又被包了进去。

改为"**这个块内部任何地方画了复选框**就不包装"（沿 `walk` 遍历子树）。
一个块里既有清单又想双击编辑，勾选仍然优先——与顶层的取舍一致。

### 测试

7 条端到端测试：真的点复选框、断言写回的文档一字不差。其中引用那条
**不给额外等待时间**——需要等双击超时才响应的复选框，读者早就已经点第二次了。

### 涉及文件

- `code/lib/ui/editor/markdown_renderer.dart`
- `code/test/ui/editor/task_toggle_test.dart`（新增，7 条）


---

## BUG-130：引用/列表里的图表，工具栏按钮僵死

### 排查方式

上一轮（BUG-129）的教训是"规则在一处修了、没推广到相邻情形"。
把这条当作透镜，回头看同一个函数——**里面还有一处形式完全相同的判断**：

```dart
if (_holdsATaskList(node)) return child;          // 刚改成看整棵子树

if (node is md.CodeBlockNode &&                   // 仍然只认"自己就是图表"
    MermaidParser.handlesLanguage(node.language)) {
  return child;
}
```

两条规则解决的是**同一个问题**（双击识别器占住手势竞技场，让块内自带的点击目标
僵死约 300ms），注释里也写明了是"同一回事"。但一条已经推广到子树，另一条没有。
...
嵌套的图表，它工具栏上的复制源码、编辑源码、全屏、下载**四个按钮**都要等双击
超时才响应。

### 修复方案

两条合并成一条 `_holdsOwnControls`：**这个块的子树里任何地方有自带点击目标的
东西**（任务列表的复选框、图表的工具栏），就不包装。

### 测试

4 条。其中一条是**反向**的：普通段落、标题、引用、普通列表、非图表代码块
**必须仍然被包装**——这条豁免如果悄悄扩大，结果就是整个预览不能编辑了，
而那是不会有人报的那种"功能消失"。

### 涉及文件

- `code/lib/ui/editor/markdown_renderer.dart`
- `code/test/ui/editor/tap_target_exemption_test.dart`（新增，4 条）
...
上一轮发现源项目有 **70 份端到端测试**（`muya/e2e/tests/`），那是它对自身行为
最精确的规范。逐份看用例名，挑能在本机验证的对照。

先对**引用链接**（`reference-link-image.spec.ts` 规定了大小写不敏感、定义可在
引用之前、缺失时保持字面等）：**13 种形态全部正确**，这块干净。

再对**标题**（`paragraph-and-headings.spec.ts` 有一条"七个井号不该变成标题"）：
...
CommonMark 允许任何块前面有至多 3 个空格（4 个才是缩进代码块），所以
`   # 标题` 是标题。`#` 单独一行是空标题——**那正是标题被敲出来的中间状态**。

### 修复中触发的两个连带问题

**其一：有示例让解析器抛异常。** 把标题内容改成可选组之后，两处 `group(2)!`
拿到 null。这条是被 CommonMark 语料测试里**单独那条"没有任何示例让解析器抛
异常"**抓到的——它与得分分开断言正是为了这个：答案不同是分歧，抛异常是文档
根本打不开。

**其二：大纲开始统计列表项里的标题。** `headingOutline` 读的是**原始文本**，
放宽缩进后它无法再区分"顶层缩进 3 格的标题"与"列表项里的标题"。
既有测试（"步骤里携带的标题两边都不该统计"）当场变红。

处理方式是让大纲**保持严格**（只统计顶格的），并写明理由：漏掉一个缩进的顶层
标题，比在大纲里列出一个预览没有滚动目标的条目要轻——后者会让它之后的**每一个
条目都跳错位置**。
...
- `code/lib/services/markdown_parser.dart`
- `code/test/services/commonmark_spec_test.dart`（下限 353，新增 4 条）
...
源项目的端到端规范 `editing/undo-redo.spec.ts` 有一条
**「打出的词应当切分成独立的撤销步」（#3825）**，写得很具体：

> 打 `hello world`，第一次撤销**只丢掉 `world`**，第二次才丢掉 `hello`。

### 现象与根因

本项目的历史快照**只由 300 ms 防抖驱动**：
...
也就是说，**连续打字期间一个快照都不入栈**——只有停下来才记一次。写一整段
不停顿，然后按一次撤销，**整段消失**。这正是源项目 #3825 修的那个问题。

### 修复方案

在防抖之外，**打出词边界字符时立即入栈**。判定条件写得很窄：文本恰好比上一次
长一个字符、且是追加在末尾、且那个字符是词边界——**粘贴、删除、替换都不算**，
仍然交给防抖。

**中文标点必须在边界集合里**：中文句子没有空格，只认空格和西文标点的话，
一整段中文又会退回成一个撤销步。已纳入 `。，；：！？、」』）】》""'`。

### 一处我自己的错误

第一版判定写成"文本比**上一次快照**长一个字符"。但快照只在入栈时更新，
打了几个非边界字符之后长度差就大于 1，条件**永远不成立**——测试里表现为
撤销一次直接回到空文档。应当比对的是**上一次的文本**，不是上一次的快照。

### 涉及文件

- `code/lib/ui/editor/source_editor.dart`
- `code/test/ui/editor/undo_granularity_test.dart`（新增，4 条）

其中一条按源项目规范原样复现（`hello world` → 撤销一次剩 `hello`），
一条专测中文按标点断开，另两条守住 redo 的对称性与撤销到底为空。


---

## BUG-133：输入法选字过程被当成正式输入

### 来源

源项目端到端规范 `typing/ime.spec.ts` 三条用例的共同规则是
**「只在 compositionend 之后才提交文本」**。

对照本项目：`composing`（组合中）这个状态在整个编辑器里**一次都没有被读过**。

### 现象

拼音输入 `你好`，若候选过程中出现过标点（`hao,` 这类串很常见），
按一次撤销拿回的是 **`hao,`**——**一段读者从未打出、只在候选窗口里存在过的中间串**。
...
上一轮（BUG-132）给撤销加了"按词边界切分"，判定条件是"刚输入的字符是词边界"。
而输入法在选字期间**每敲一个字母都会重写整段文本**，其中出现逗号时就满足了这个
条件——于是候选串被当成了一次正式输入记进历史。

**这是我上一轮修复引入的问题**，而且不止影响撤销：`_onTextChanged` 里所有"看
文本"的功能都在看候选串——包括斜杠菜单（正在选字时打出 `/` 会弹出插入菜单，
挡住候选窗口）与代码块语言选择器。

### 修复方案

组合输入期间统一不动作：不记历史、不弹斜杠菜单、不同步语言选择器。
`widget.onChanged` **照常调用**——否则预览会在选字过程中僵住。

抑制**必须随组合结束而结束**：否则用输入法打完一个词之后就再也不记历史了。
这是单独一条测试。
