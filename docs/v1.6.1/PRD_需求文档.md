...
| FEAT-095 | 2026-09-02 | 插件权限和菜单/工具栏贡献描述 | P1 | 高 | 进行中 |
...
| 实现方案 | manifest 增加 `permissions`、`commands`、`toolbar`；宿主保留固定布局槽位，禁止任意坐标和任意 Widget 注入 |
| 验收标准 | manifest 可解析并拒绝非法贡献；后续接入命令面板、右键菜单和工具栏注册时不改变主窗口布局稳定性 |
