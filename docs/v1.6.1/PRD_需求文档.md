...
| FEAT-117 | 2026-09-04 | 插件页作为真正的标签页打开 | P1 | 中 | 已完成 |
| FEAT-118 | 2026-09-04 | 短答案改用贴边悬浮卡片，可随时关闭 | P1 | 中 | 已完成 |
| FEAT-119 | 2026-09-04 | 窗格按数量对称布局，分隔条可拖动 | P1 | 中 | 已完成 |
| FEAT-120 | 2026-09-04 | 插件在列表中说明自己是什么，并支持多语言 | P1 | 中 | 已完成 |
| FEAT-121 | 2026-09-04 | 社区列表区分可安装/可更新/已安装 | P2 | 低 | 已完成 |
| FEAT-122 | 2026-09-04 | 记录插件来源，列表可说明正式版还是预发布 | P2 | 低 | 已完成 |
| FEAT-123 | 2026-09-04 | 悬浮卡片可拖动，并在其中提问 | P1 | 中 | 已完成 |
| FEAT-124 | 2026-09-04 | 全文翻译按预算成批发送 | P1 | 中 | 已完成 |
...
## FEAT-117：插件页作为真正的标签页打开

| 字段 | 内容 |
|------|------|
| 实现日期 | 2026-09-04 |
| 需求描述 | 点击插件打开的详情页，应该新开一个标签页，而不是在已打开的标签页内部再加一层 |
| 用户场景 | 用户一边看文档一边翻插件详情，两者都应该在标签栏里各占一格，随时切回来 |
| 实现方案 | 见 BUG-246。`TabInfo.pluginDetail(entry)`，id 为 `plugin:<插件 id>`，无文件路径 |
| 验收标准 | 不进入会话持久化、不进已打开文件列表、不触发自动保存；同一插件再点是回到那个标签而不是叠一个；标签栏用扩展图标把它和文档区分开 |
| 涉及文件 | `tab_info.dart`、`plugin_provider.dart`、`home_screen.dart`、`editor_tab_bar.dart` |

## FEAT-118：短答案改用贴边悬浮卡片

| 字段 | 内容 |
|------|------|
| 实现日期 | 2026-09-04 |
| 需求描述 | 「翻译选中内容」应该出现悬浮窗，像 tips 一样：页面滚动它不动，先显示正在翻译，完成后原地显示译文；并且随时可以主动关闭 |
| 用户场景 | 译文是拿来跟原句对照的，所以原句必须还在、还能滚 |
| 实现方案 | `PluginTipLayer` 贴在文档区右上角，不引入遮罩。同一张卡片从「进行中」换成译文 |
| 验收标准 | 显示前后 `ModalBarrier` 数量不变；卡片距右 12、距顶 12，宽度足以阅读；**进行中也能关**，关掉后模型返回的答案不再弹回来 |
| 涉及文件 | `plugin_tip.dart`、`plugin_command_actions.dart`、`home_screen.dart` |
...
|------|------|
| 实现日期 | 2026-09-04 |
| 需求描述 | 一宫格是全部；二宫格左右对称；三宫格上半左右对称分开、下半不分；四宫格上下两半都左右对称分开 |
| 用户场景 | 全文翻译要跟原文长时间对照，译文该和原文一样宽，而不是右侧一条窄缝 |
| 实现方案 | 见 BUG-252。布局由填充数量决定，槽位名只用于标识窗格 |
| 验收标准 | 每一步左右/上下对称；分隔条可拖动，手感与源码/预览之间那根一致；没有窗格时不包任何多余层级 |
| 涉及文件 | `plugin_panes.dart`、`plugin_grid_test.dart` |

## FEAT-120：插件在列表中说明自己是什么

| 字段 | 内容 |
|------|------|
| 实现日期 | 2026-09-04 |
| 需求描述 | 插件列表要显示名称和简介，并且支持多语言 |
| 用户场景 | 扫一眼列表要能判断这个插件是干什么的——这跟决定要不要装是同一个问题，而已安装列表恰恰答不上来 |
| 实现方案 | `PluginManifest` 新增 `description`；名称与简介都走插件自带的 `locales`，和它显示的其它字符串一样。`PluginCatalogEntry.installed(manifest, locale:)` 也走同一条路，locale 为空时经默认语言解析，不会露出键名 |
| 验收标准 | 名称保持字面量 `AI Translate` 并以自身为翻译键——不解析名称的旧版本显示的是一个名字，而不是 `plugin.name` 这个词；简介缺省时不写进 JSON，也不占列表位置 |
| 涉及文件 | `plugin_manifest.dart`、`plugin_catalog_entry.dart`、`plugin_panel.dart`、`plugin_detail_view.dart`、插件 `manifest.json`（12 种语言）、SDK `README.md` 与 `schema/` |

## FEAT-121：社区列表区分可安装/可更新/已安装

| 字段 | 内容 |
|------|------|
| 实现日期 | 2026-09-04 |
| 需求描述 | 在线安装之后，发现列表里那一条应该变成已安装状态 |
| 用户场景 | 刚装完的插件，列表看上去跟装之前一模一样，只能再按一次下载才知道 |
| 实现方案 | 见 BUG-245 |
| 验收标准 | 已安装显示「已安装」且不可按；目录版本更新时给的是更新按钮而不是「已安装」——报个「已安装」就收手，读者从此再也收不到更新 |
| 涉及文件 | `plugin_catalog_entry.dart`、`plugin_panel.dart`、`plugin_detail_view.dart` |
...
|------|------|
| 实现日期 | 2026-09-04 |
| 需求描述 | 插件列表只显示名称、简介、版本号、正式版还是预发布版 |
| 用户场景 | 「0.1.3」和「0.1.3，预发布」不是同一个承诺，读者有权知道自己拿的是哪一个 |
| 实现方案 | 见 BUG-256。安装时把 release 的 `prerelease` 与 tag 写进 `sources.json`，卸载时忘掉 |
| 验收标准 | 没有记录的（旧安装、手工 ZIP）只显示版本号——它们背后本就没有一个 release 可供回答；不按版本号前导零去猜 |
| 涉及文件 | `plugin_manager.dart`、`plugin_catalog_service.dart`、`plugin_panel.dart` |

## FEAT-123：悬浮卡片可拖动，并在其中提问

| 字段 | 内容 |
|------|------|
| 实现日期 | 2026-09-04 |
| 需求描述 | 翻译选中内容的悬浮窗应该可以拖动；目标语言的提问不该是一个更大的独立弹窗 |
| 用户场景 | 卡片可能正好压住要对照的那一段；而提问和答案是同一次交互的两半，不该一半是大对话框、一半是小卡片 |
| 实现方案 | 见 BUG-258 与 BUG-260 |
| 验收标准 | 拖动限制在窗格内且算上卡片自身宽度；关掉提问等于不回答，等待的运行以 null 收场而不是永远挂着 |
| 涉及文件 | `plugin_tip.dart`、`plugin_provider.dart`、`plugin_command_actions.dart` |

## FEAT-124：全文翻译按预算成批发送

| 字段 | 内容 |
|------|------|
| 实现日期 | 2026-09-04 |
| 需求描述 | 全文翻译一次发的内容太少，浪费性能，优化 |
| 用户场景 | 几十段的文档就是几十次往返，每次都付一遍延迟 |
| 实现方案 | 见 BUG-261。切段之上加一层批次，约 1500 字符 |
| 验收标准 | 不切开围栏；超预算的单段独走；标题不单独成批；仍然是多批而不是整篇一次——失败只损失一批，读者能先看到开头 |
| 涉及文件 | 插件 `lib/blocks.lua`、`plugin.lua` |
