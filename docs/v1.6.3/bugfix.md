...
| BUG-474 | 2026-09-13 | 清空一个从未保存过的标签页，它仍算「有未保存的工作」——自动化接口因此无法销毁自己建的草稿 | P2 | 已修复 |
| BUG-475 | 2026-09-13 | BUG-472 把最长卡顿从 7.4 秒变成 24.3 秒换来 17% 总时间——逐帧实测后回退 | P1 | 已修复 |
| BUG-476 | 2026-09-13 | CLAUDE.md 自己没被对账：token 数落后 2 个，而它给的自查配方数出 46 而不是 22 | P2 | 已修复 |
| BUG-477 | 2026-09-13 | 自动化接口写进文档的内容撤不回来——五个写入者里唯一没压还原点的那个 | P2 | 已修复 |
...
| BUG-500 | 2026-09-24 | **双击打开文件要先把整个 Flutter 引擎启动一遍**才发现「已经有实例了」——那一秒多才是读者说的「打开第二个文件卡」 | **P1** | 已修复（待真机确认） |
| BUG-501 | 2026-09-24 | **卸载之后安装目录还留着**：程序运行时在自己旁边写的诊断文件不是安装器装的，卸载器不认识它们，一个就够让整个文件夹留下 | P2 | 已修复 |
| BUG-502 | 2026-09-24 | **关窗口后窗口还杵在屏幕上，直到进程真的死掉**——`window_manager` 的 destroy() 只是投一条退出消息，从不销毁窗口。读者说的「关闭严重卡顿」就是这个 | **P1** | 已修复（真机量到） |
| BUG-503 | 2026-09-24 | **最小化状态下关闭，把「窗口不在任何地方」当成窗口位置存了下来**，下次启动窗口小到看不见、位置在屏幕外，读者找不回来 | **P1** | 已修复（读者撞到） |
...
## BUG-451：打错的图表在导出的 PDF 里显示一块英文错误面板

**现象**：文档里有一处 Mermaid 图表写错（比如把 `graph` 打成 `grahp`），
**预览里显示的是本地化的错误框**，而导出成 PDF / Word / HTML 之后，
文档里嵌着一张**英文红色错误面板的图片**：

```
Mermaid parse error
Unrecognised diagram type: "grahp".
```

**这比屏幕上的英文更糟**：它被烙进了一份读者要发给别人的文档里。

**全屏查看那一处也是英文**（点图表放大）。

**根因**：Mermaid 包只依赖 Flutter，够不到应用的翻译，所以它自带的错误框是英文的——
**这是有意的**。应用的办法是传一个 `errorBuilder` 自己措辞，而**预览传了，另外两处没传**：

| 画图表的地方 | 传了 `errorBuilder` 吗 |
|-------------|----------------------|
| 预览（`MermaidRenderer._buildDiagram`） | ✅ |
| 导出的离屏渲染（`app_menu_bar._renderMermaidToImage`） | ❌ |
| 全屏查看（`_MermaidFullscreenViewState`） | ❌ |

**包自带错误框上的注释把这件事说成了有意的**，理由是「reaching the app's
translations would end that（包的独立性）」——**那个理由对包内成立，对调用者不成立**。
导出是应用代码，能像预览一样传一个 builder，包的独立性丝毫不受影响。
**注释里的理由推不出它想推的结论**，这与两轮前我自己写错理由是同一类。

**修复**：措辞与错误框抽成一份共用（`lib/ui/widgets/mermaid_failure.dart`：
`describeMermaidFailure` + `MermaidFailureBox`），三个地方都用它。
**l10n 键一个都不用新加**——六个键早就有了，缺的只是有人去用。

**图表类型名不翻译**（有测试钉住）：它们是**要被打出来的东西**。
告诉读者去写「流程图」，他没有地方可写。打错的那个词也要原样引回给他。

**涉及文件**：

- `code/lib/ui/widgets/mermaid_failure.dart`（新增）
- `code/lib/ui/widgets/mermaid_renderer.dart`（预览与全屏都改用共用的；删掉私有的
  `_localisedFailure` 与内联错误框）
- `code/lib/ui/widgets/app_menu_bar.dart`（离屏渲染传 builder）
- `code/test/ui/a_broken_diagram_is_explained_in_every_language_test.dart`（6 条）

**守卫**：`lib/` 里**每一处** `MermaidDiagram(` 都必须传 `errorBuilder`，否则包自带的
英文框就会到达读者——这条正是漏点被发现的方式（它一开始就报出了全屏那一处，
我原本只找到两处）。断言扫到的调用点 ≥ 2，防止守卫空过。

**守卫第一次跑出四处，其中三处是文档注释里的示例**（`mermaid.dart` 开头三段
`/// ```dart` 用法示例）。滤掉注释行才对——这正是本库已有守卫都做的事，
理由也一样：「解释为什么没人画它的那句话，不该被读成有人在画它」。

**四次变异**：

| 变异 | 红在哪 |
|------|--------|
| 导出不传 builder（回到原状） | 守卫点名 `app_menu_bar.dart:1443` |
| 两种失败给同一句话 | 「读者不知道该改什么」并列出三句 |
| 把图表类型名也翻译掉 | 「打错的那个词要引回给读者」 |
| 守卫不滤注释 | 红——文档示例被当成泄漏 |

---

## BUG-452：撤销历史按步数限，于是 10 MB 文档的历史能占到 2 GB

**现象**：编辑一个大文档一会儿之后，进程占用暴涨。每一步撤销快照都是**整份文档的副本**，
而上限限的是**步数**：
...
而且是**每个标签页一套**，撤销与重做**各一套**。这是进程里最大的一块，而没有任何东西
在量它——直接违反本项目开头那句「占用低、支持大文件」。

**根因**：上限的单位错了。原注释**看到了风险的形状**：
...
「整份副本」「会无限增长」都说对了，然后**数错了东西**。
而同一个代码库里、几个文件之外的高亮缓存**正是按字符数限的**
（`cacheCharBudget = 128 KiB`，带 LRU 与逐出循环）——兄弟没学到这一课。

**修复**：两个上限同时生效，收敛到一处 `_trim`（原先三处各写一遍）：

- **步数仍是 200**，所以文档小到「200 份副本很便宜」时行为完全不变——
  **约 80 KB 以下的文档一步不少**，而这几乎是全部 Markdown 文件。
- **新增 16 MB 的字符预算**，把病态情形从 GB 变成 16 MB。
- **底线是 2 条**：栈顶是当前状态，只剩 1 条就无处可退，撤销键会变成什么也不做。
  **单个快照比整个预算还大时也留着**——大文档上撤销一次的价值高于这份预算。
- **从头部逐出**（最旧的先走），因为撤销要的正是最近的过去。

**为什么是求和而不是维护一个累计值**：`String.length` 是常数时间、栈最多 200 条，
所以这是在一个**本就被 300 ms 防抖挡着**的推入上做 200 次加法；
而累计值是第二份要与 `redo`（在两个栈之间搬移条目）保持同步的状态。

**代价实测（第 1 条）**：典型文档（20 KB × 400 次推入，栈全程满在 200）取 6 轮最小值：
...
**增加 1.3 µs，而推入被 300 ms 防抖限着**——是防抖窗口的 0.0004%。

**`redo` 不需要再限**：它只是在两个栈之间**搬移**条目，总量不变；
在那里逐出反而会扔掉读者刚刚退回去的状态。

**涉及文件**：

- `code/lib/providers/editor_provider.dart`（`historyCharBudget`、`_trim`、三处 trim 合一、
  三个 `@visibleForTesting` 访问器）
- `code/test/providers/the_undo_history_is_bounded_in_bytes_test.dart`（5 条）

**验证（四次变异，第一次就是修复前的行为）**：

| 变异 | 红在哪 |
|------|--------|
| 去掉字符预算（＝修复前） | 「撤销历史超出了字符预算」 |
| 底线改成 1 条 | 「历史被削到一条，撤销键会变成什么也不做」 |
| 从末尾逐出 | 「丢掉的是最近的状态，而撤销要的正是最近的过去」 |
| 预算改到 1 MB | 「小文档的撤销步数被削减了，这是回归」 |

最后一条专门存在，是为了**证明这个修复没有拿典型情形去换病态情形**。

**顺手把这条尺子在全库用完了**（「每条都是整份副本，却按条数限」）：

| 候选 | 结论 |
|------|------|
| `app_log.dart` 日志缓冲（500 行） | **没问题**。消息都是短诊断句；最长的插值是 HTTP 响应体，而它在 AI 路径里已截断到 400 字符。最坏约 200 KB，相对文档可忽略 |
| `side_bar.dart` 搜索结果（500 条） | **没问题**。每条只是一行摘要，而且可搜索字节数本身另有 2 MB 上限 |
| `markdown_renderer.dart` 批次大小（2000 块） | **没问题**。是一次渲染多少块，不是留下多少副本 |
| `plugin_manager.dart` 的三个上限 | **本来就是按字节的**（64 MB / 256 MB / 10000 条目） |

**撤销历史是本库唯一一处**「每条是整份输入副本」遇上「按条数限」。这条尺子到此用尽。

---

## BUG-453：每次选区变化都把整个选区拷一份进状态

**现象**：在大文档里按住 Shift+↓ 逐行扩选，越选越卡；选完之后那份副本**留在
provider 状态里**，与文档本体、文本框自己那份、撤销历史并列。

**根因**：`_onSelectionChanged` 每次都做两件 O(选区) 的事，而三个读者**一个都不需要它
被推过来**：

```dart
ref.read(editorProvider.notifier).setSelectedText(
  selection.isCollapsed ? '' : text.substring(selection.start, selection.end),
);
// setSelectedText 里还有：if (state.selectedText == text) return;  ← 整串比较
```

三个读者（预览的富文本复制、插件命令的两处）**全部是 `ref.read`**，在命令运行的那一刻
才要这个值——没有任何地方反应式地 watch 它。所以这是「急切推入、极少读取」。

**手势整体是 O(n²)**：按住 Shift+↓ 时选区逐步变大，每一次按键拷一个更大的字符串。
...
**全选是免费的**——Dart 对 `substring(0, length)` 直接返回同一对象。
**这正是它长期没被发现的原因**：谁想验证「选区拷贝贵不贵」，第一反应都是按 Ctrl+A。

**修复**：让 notifier 承担整件事，控件只传**范围**。

- `setSourceSelection(TextSelection)` —— 存两个偏移量，**控件连可拷贝的东西都没有**。
- `setPreviewSelection(String)` —— 预览没有偏移量可存（它的选区是渲染后的文本，
  来源散在它覆盖的那些块里），而 Flutter 在调用我们之前**已经**建好了那个字符串，
  所以这里没有多拷任何东西。
- `selectedText()` —— 按需取。
- `EditorState.selectedText` **字段删除**，于是编译器把 5 个调用点全指了出来。

**「最后写入的那个窗格答话」这条语义必须原样保留**（原先两个窗格写同一个字段）：
从预览运行的插件不能拿到源码区一分钟前选的东西。两条测试钉住它，
包括「把光标放回源码区」要清掉预览的旧选区。

**涉及文件**：

- `code/lib/providers/editor_provider.dart`
- `code/lib/ui/editor/source_editor.dart`、`markdown_renderer.dart`（写入各一、读取一）
- `code/lib/ui/widgets/plugin_command_actions.dart`（读取两处）
- `code/test/providers/the_selection_is_read_not_copied_test.dart`（7 条）

**验证（四次变异，第一次即修复前的行为）**：

| 变异 | 红在哪 |
|------|--------|
| `setSourceSelection` 退回拷贝选区（＝修复前） | 「记录一个大选区比记录一个小选区贵得多（3382 µs vs 5 µs）」 |
| 预览选区不清掉源码区范围 | 「最后改动的那个窗格才答话」 |
| 源码区折叠时不清掉预览选区 | 「把光标放回源码区之后，预览的旧选区仍然被当作当前选区」 |
| 范围不做 clamp | 越界抛异常 |

**第一次做这条代价守卫时它是无效的，而这件事本身值得记下来**：我把大选区写成了
`0..length`，也就是**全选**——恰好是代价为零的那一种，所以变异 1 一开始**没有被抓住**。
...
在长行较多的文档里（尤其**分屏**，窗格更窄），点大纲里的标题跳过去，
**落点完全不对**——实测要第 20 行，那一行落在视口**下方 1497 px**，即整屏之外。

六个入口都走这条路：

| 入口 | 位置 |
|------|------|
| 大纲（目录） | `side_bar.dart:1069` |
| 侧栏搜索结果（两处） | `side_bar.dart:701 / 732` |
| 预览里点击定位到源码（两处） | `markdown_renderer.dart:671 / 2716` |
| 查找栏，目标是预览时 | `find_replace_bar.dart:293` |

### 根因

**BUG-455 与 BUG-456 的同一个根因的第三、第四份副本。** 这一轮不是又碰巧撞上，
而是修完 456 之后**把这条规则的所有副本列了一遍**才找出来的：

```dart
final targetOffset = ((line - 1) * lineHeight).clamp(…);   // _scrollToTargetLine
final targetOffset = (line * lineHeight) - viewportHeight / 2 + lineHeight / 2;  // 打字机
```

两处都在用「文档行号 × 假设行高」当屏幕位置。窗格比最长行窄时，
每一处换行都让答案少算一行。

### 修复

新增 `_contentYOfLine(line)`：用已缓存的行首偏移量 + `_contentYOf`（BUG-455 建的
那把仪器），问正在画文字的那个字段。两处都改用它，问不到时才退回乘法。

### 涉及文件

- `code/lib/ui/editor/source_editor.dart`
- `code/test/ui/editor/a_jump_to_a_line_lands_on_that_line_test.dart`（新增）
...
打开「打字机模式」，光标所在行**并没有停在窗格中间**。实测：800 px 的视口里，
光标行画在 **722 px** 处——贴着底边，正是「把光标滚进视野、底部留两行」的样子。

### 根因：两个都在滚，后跑的那个总赢

`_onSelectionChanged` 里原本是：

```dart
_scrollToTypewriterPosition(line);          // 同步，animateTo 到中间
WidgetsBinding.instance.addPostFrameCallback((_) {
  if (mounted) _showCaret();                // 帧后，jumpTo 到最近边缘
});
```

`_showCaret` 是**窗格改为自己滚动**时加的（它的注释写着「the only thing that
keeps the caret on screen」），而它**在帧后跳**，于是每次都把刚开始的居中动画
按回最近边缘。打字机模式从此就没生效过。

**这是怎么确认的**：修了乘法之后那条测试仍然不动，我没有猜，而是插了一个探针
把目标值改成 `0.0` 并打印——探针显示**分支执行了、`top=1984.0` 也算对了**，
而最终结果仍是 722。能算对却不生效，说明后面有东西覆盖它。

（顺带说明为什么一直没人发现：现有的两条 typewriter 测试只断言**配置开关**
被切换了，没有一条看过它到底滚到哪——「两份清单没人对账」的又一例。）
...
WidgetsBinding.instance.addPostFrameCallback((_) {
  if (!mounted) return;
  if (_scrollToTypewriterPosition(line)) return;   // 处理了就到此为止
  _showCaret();
});
```

`_scrollToTypewriterPosition` 现在返回「我管了吗」。都放到帧后还顺带修掉一件事：
两者要的位置都来自文本排版，而**选区变化的那一刻排版还不存在**。
...
| 变异 | 结果 |
|------|------|
| 目录跳转退回乘法 | 红：那行在视口下方 1885 px |
| 打字机退回乘法 | 红 |
| 退回「两个都跑」的旧顺序 | 红：回到 722 |
| 打字机声称处理了却不滚动 | 红：最后一行画在 3024 px 处，视口 800 |
| ~~去掉 `clamp` 的上界~~ | **绿——等价变异**：`animateTo` 本身会被 scroll physics 夹住，平台已经做了这件事 |
...
## BUG-464：拖入或双击打开一个读不了的文件，标签页闪一下就消失
...
把一个**读不了**的 `.md` 文件拖进窗口（权限不足、网络共享断开、拖放与读取之间被别的
程序删掉）——标签页出现，转一下圈，**消失**，屏幕上没有任何说明。
在文件管理器里**双击**一个同样读不了的 `.md` 也一样：应用起来了，什么都没有，
也不说为什么。

### 根因：同一条规则的第三、第四份副本没跟上

「打开文档」在这个应用里有**五处**各自的实现（侧边栏、文件▸打开、打开最近、
启动参数、拖放）。其中三项关键不变量的分布：

| | 已打开时聚焦现有标签页 | 加入最近列表 | 读失败时告诉读者 |
|---|---|---|---|
| 侧边栏 | ✓（且 `addTab` 本身也去重） | ✓ | ✓ `failTabLoading` |
| 文件▸打开 | ✓（靠 `addTab`） | ✓ | ✓ `reportOpenFailure` |
| 打开最近 | ✓（靠 `addTab`） | ✓ | ✓ |
| **启动参数** | ✓ | ✓ | **✗ 只 `removeTab`** |
| **拖放** | ✓ | ✓ | **✗ 只 `removeTab`** |

前三处的注释里还留着当年修它的话——「The sidebar's open has always said so;
this one gave no tab and no message」。**这是同一句话的第三、第四次。**

两处更具体的证据：

- 启动那处的注释原文是 `// Handle error: remove the loading tab **or show error
  state**`——「或者显示错误状态」，等于承认这个选择**从来没做完**。
- 拖放那处外层 catch 的注释写着 `// Skip files that can't be read`，
  但**读取早就搬进 post-frame 回调里了**，那个 try 里已经没有任何读文件的代码。

（顺带核实了两件我一开始怀疑、结果**不是**缺陷的事：`addTab` **本身**就有
「同一文件不开第二个标签页」的去重，所以五条路都受保护；五处的 `addRecentFile`
也都在。没有去「修」不存在的问题。）

### 修复

两处内层 catch 都改为 `removeTab(tabId)` + `reportOpenFailure(e)`。
拖放那处的外层 catch 也改为上报，并把那句不准确的注释换成事实。

**为什么是每个文件一条消息，而不是像「扩展名不对」那样在末尾报一个总数**：
那些读取是在循环结束**之后**才完成的 post-frame 回调里，到那时已经没有东西可数。
一次拖进好几个读不了的文件，比拖进一个罕见得多。这条权衡写在代码注释里。

启动那处**没有推翻已有的决定**：外层「某个文件打不开就跳过，其余照常打开」
是另一个决定，保持不变；改的只是「跳过时一声不响」。

### 涉及文件

- `code/lib/ui/screens/home_screen.dart`
- `code/test/ui/widgets/failure_reaches_the_reader_guard_test.dart`

### 守卫：把「有几处」也钉住

仓库里本来就有一条 `failure_reaches_the_reader_guard_test`，职责正是这件事，
但它只盯着三个具名调用。新增的这条改成**复数**：

```dart
final guards = everyCatchAround('home_screen.dart', '_readDocument(path)');
expect(guards.length, 2);   // 多一条打开文档的路，这里就会红
for (final guard in guards) { expect(guard, contains('reportOpenFailure')); }
```

**顺带修了那条守卫自己的一个缺陷**：它的正则用 `[^{}]*` 限定 try 的范围
（作者注明这是为了不让范围越过要找的 catch），而启动那条路的 try 里有
`StartupTrace.mark('file read (${bytes.length} bytes)')`——**`${…}` 的花括号把
范围断开了**，于是它只找到两处中的一处。改成允许 `${…}` 插值，并让单数版
`catchAround` 委托给复数版，**一个正则一处修**。原有三条断言不受影响。
...
一个文件在编辑器里打开着，**别的东西把它删了**——`git checkout`、同步客户端、
另一个程序、命令行 `rm`。编辑器这一侧：

| 观察点 | 改之前 |
|--------|--------|
| 标签页外观 | **毫无异样**：没有修改点、没有横幅 |
| 状态栏 | 照旧显示那个路径 |
| 内容 | 留着（这一点是对的） |
| 关掉这个标签页 | **不会询问**（`isModified` 为 false）→ **文件和内容两头都没了，全程一声不响** |

### 根因：一个 catch 把读者唯一需要知道的事咽了下去
...
「保留内容」确实是安全的选择，注释也说对了一半——**但它对「说出来」这件事什么都没做**。

对照之下，**读者自己删除**那条路早就处理了：侧栏的删除会调 `pathDeleted` 关掉标签页，
那里的注释还写着「opened-files 那个菜单早就关了标签页，这个却把它留在一个已消失的
文件上」。**外部删除是同一件事的另一半，没人做。**

（注意：外部删除**不能**照搬 `pathDeleted` 去关标签页——那会毁掉读者手里唯一的副本。
两条路的正确结果本就不同。）
...
```dart
if (!await File(path).exists() && mounted) _setDiskConflict(tab.id, true);
```

**不需要新增文案**：横幅现有的话对删除同样成立——
「Changed on disk — auto-save is paused for this file」（确实变了；自动保存也确实
因 `diskConflict` 而停）。冲突的三条出路也仍然讲得通，其中**「覆盖」正是把文件写回去**的那一条。

**只在文件确实消失时报，而不是任何读失败都报**：别的程序写到一半的文件会短暂读不了，
为那种情况升横幅是一次误报，还要读者自己去清掉。这一条单独有一条测试
（`chmod 000` 造「在但读不了」）。

### 顺带补了一道异步守卫

`_onDiskChange` 在 `await` 之后**直接读 `state`**，而这是流回调：窗口在事件在途时关闭，
读 `state` 就会抛，且抛进流里无人接。补上 `if (!mounted) return;`——
与 BUG-149 之后 `markSaved` 加的那道是同一个窗口。

### 涉及文件

- `code/lib/providers/tab_provider.dart`
- `code/test/providers/tab_reload_test.dart`（新增 2 条）

### 验证：两次变异

| 变异 | 结果 |
|------|------|
| 退回静默的 catch | 红：「文件没了而标签页看起来毫无异样，关掉它就两头都没了」 |
| 任何读失败都报冲突（去掉 `exists` 判断） | 红：「文件还在，只是这一刻读不了——不该报冲突」 |
...
新增 `TabNotifier.recordExternalEdit(id, next)`（当时叫 `recordPreviewEdit`，BUG-477
把它改成了不限于预览的名字）：设置 history 所属标签页（纯预览模式下
...
## BUG-472：预览补画的上限只让事情更糟

> **这一条的结论是错的，已由 BUG-475 逐帧实测推翻并回退。**
> 下面保留原文，因为「错在哪里」比结论本身有用：整段只算了**遍历已画块**的开销，
> 没算**新块的构造**——而后者是主项，每块约 1.7 ms 对 0.12 ms。
> 去掉上限之后，最后一趟会在**一帧里**建半篇文档。
...
这一版一直在真机上跑 MCP 验证。本项目的卖点里写着「支持大文件」，于是我用编辑器
**自己的秒表**（日志里的 `preview drew N blocks in T ms`）量了四组：

| 内容 | 块数 | 补画完成 |
|------|------|---------|
| 200 KB 散文 | 1 462 | 247 ms |
| 200 KB 代码围栏 | 4 015 | 1 012 ms |
| **1 MB** | **25 368** | **14.0 s** |
| **2 MB** | **50 736** | **37.8 s** |

文档翻倍，时间是 2.7 倍。**超线性，也就是有个平方项。**

（用编辑器自报的耗时而不是外部计时：这件事跨帧发生，外面测不出来——
见 `[[measurement-that-contradicts-itself]]`。）

### 根因：一个没写理由的常量

`_scheduleNextBatch` 里：
...
先说清楚它**不是**因为已画的块被重建了——`_blockWidgets` 正是为了拦住这件事而存在，
而且拦住了（它自己的注释就写着：交回同一个 widget 实例，element 不重建，
render object 不重新布局）。真正按「已画块数」重复付出的是另外三样：

1. `build` 里那个 `for` 循环遍历全部已画块（每个一次 map 查询 + 一次 `add`）；
2. Column 的 element 逐个对账那 N 个孩子；
3. **`SingleChildScrollView` 不做视口剔除**——整棵子树每帧都被 paint 走一遍。

所以给步长封顶：

- **一趟也没变短**——一趟的代价由 `rendered` 决定，跟 `step` 无关；
- **贵的趟数变多了**，而且多的数量随文档增长：

| 文档 | 封顶 2000 | 翻倍 |
|------|----------|------|
| 25 368 块 | 19 趟，走过 198 918 个块（文档的 7.8 倍） | 10 趟，50 918 个（2.0 倍） |
| 50 736 块 | 31 趟，682 686 个（**13.5 倍**） | 11 趟，101 886 个（**还是 2.0 倍**） |

翻倍策略走过的总量永远约等于文档的两倍；封顶策略是「文档的多少倍」本身在随文档
长大——这就是那个平方项。实测的 2.7 倍比「走过总量」预言的 3.4 倍要小，
因为补画里还有线性的那一半：**每个块的构造、布局、首次绘制，不管步长怎么切都只付一次。**
**最贵的一趟两种做法完全相同**：最后一趟，走过全部。

这是「注释里记着当年怎么坏的地方多半没守卫」的反面：**一个连注释都没有的常量，
它想防的那件事在这段代码里根本不成立。**

### 我第一次写下的机制是错的

初版提交（3246e75）把根因写成「每一趟重建已画的全部块」。**这条被同一个文件
157 行的注释否证**——那段注释说的正是「blocks 住在 Column 里所以每帧都会被重建，
`_blockWidgets` 就是为此存在」。缓存是有效的；重复付的是遍历、对账和绘制，
不是构造。算术和结论都不变（同一个 `sum(rendered)`），但机制必须说对，
否则下一个照这段注释推理的人会得出错的结论。修正在下一个提交里。

### 修复

去掉上限，把这段算术取个名字，理由和实测数字写在它头上：
...
放在公开的 `MarkdownRenderer` 上而不是私有 State 里——**测试够不到私有类**，
而这条算术正是唯一测得了的那一半（补画跨帧，耗时测不了，趟数测得了）。

### 测试

`the_preview_fills_in_logarithmically_test`，五条，测的是**性质**而不是某个数字：

- 25 368 块在 11 趟以内完成（封顶时 19 趟）；
- **文档翻倍只多一趟**，不是多一倍趟数——封顶会把这条变成直线；
- 总构建量小于文档的 3 倍（封顶是 7.8 倍和 13.5 倍）；
- 装得下一批的文档一趟都不补；
- 永远正好停在最后一块，且没有重复的一趟。
...
- `code/lib/ui/editor/markdown_renderer.dart`
- `code/test/ui/editor/the_preview_fills_in_logarithmically_test.dart`（新增 5 条）

### 验证

| 变异 | 结果 |
|------|------|
| 把 2000 的上限加回去（即缺陷原状） | 红，且失败信息直接印出 19 趟的整条序列 |
| 每帧固定只加 50（更极端的封顶） | 红 |
...
真机上有一个我上一轮通过自动化接口建的草稿标签页 `mcp-verify.md`（无路径、10 个字符），
它挡着 `update_app`。按顺序试：

```
set_content  → wrote 0 characters to mcp-1789264116577608
close_tab    → not while "mcp-verify.md" has unsaved work — closing it would
               throw that away, and nothing here can press Save
```

**内容已经清空了，它还说有未保存的工作。**

于是自动化接口有一个闭不上的环：能 `new_tab`、能 `set_content`，**却无法销毁
自己建的标签页**——`close_tab` 拒绝有未保存工作的标签页（这条守卫是故意的，
那一端没人能按保存），而清空文本并不能解除那个状态。
**一个以「现场没有人」为前提的接口，只能靠一个人来收尾。**

对读者也一样：其他编辑器关一个被你删空的未命名文档都不问。

### 根因

`updateContent` 无条件写 `isModified: true`。对有文件的标签页这是对的，
对从未保存过的标签页就不对：**空 + 从未保存 = 没有可丢的东西。**
...
**没有改成「和磁盘上的内容比较」**——那要在内存里多存一份文档副本，
而这个编辑器是给大文件用的（1 MB 文档就是多 1 MB）。定向规则，O(1)，不多占一个字节。

有文件的标签页照旧：**删光一份写过的文档是改动，不是「没有改动」**，
这一条有单独的测试，并且顺带断言那次改动没有顺手写盘。

### 顺带

`updateContent` 头上有两句重复的 summary（同一个 `///` block 里叠了新旧两版）。
留下新的那句，把 `[removeTab]` 那段理由接在它下面。

### 涉及文件

- `code/lib/providers/tab_provider.dart`
- `code/test/providers/an_emptied_scratch_tab_holds_nothing_to_lose_test.dart`（新增 4 条）

### 验证

| 变异 | 结果 |
|------|------|
| 恢复 `isModified: true`（即缺陷原状） | 红 |
| 只留 `content.isNotEmpty`（写过的文档清空后也算没改） | 红，正是那条「删光是改动」 |
| 只留 `tab.filePath != null`（草稿永远不算改过） | 红，正是那条「写过东西仍不许悄悄丢掉」 |

## BUG-475：我的上一个「修复」用 3.3 倍的卡顿换了 17% 的总时间
...
| 日期 | 2026-09-13 |
| 优先级 | P1 |
| 状态 | 已修复（回退 BUG-472 的行为改动，保留它带来的文档与守卫） |

### 怎么发现的：重读自己写下的那句话

BUG-472 的注释里我写了：

> **The most expensive single pass is identical either way: the last one, which
> walks all of them.**

回头读，这句只对**遍历已画块**那一项成立。一趟补画做两件事：

1. **遍历**已经在屏幕上的每个块（Column 里的孩子逐个对账，而且滚动视图不剔除视口，
   整棵子树每帧重画一遍）；
2. **构造**这一趟新增的块。

翻倍时最后一趟新建 **N/2** 个块，封顶 2000 时只新建 2000 个。
**构造比遍历贵得多，而我整段只算了遍历。**

### 怎么量的：让 `Isolate.run` 那趟落地，然后逐帧计时

本机跑不了完整构建，但 widget test 里可以逐帧计时。两个坑：

- **规模没过线**：4000 块上三种做法量不出差别（最差帧 307/288/285 ms）——
  上限 2000 在 4000 块上几乎不生效。
- **只量到了前缀**：25 000 块必然超过 1500 行，`safePrefix` 一定返回前缀，
  整篇要等 `Isolate.run` 那趟落地，而它的 future 在 FakeAsync 区里永不完成。
  要用 `tester.runAsync` 放它跑真实时间。

绕过这两点之后，25 000 块的逐帧耗时（本机，比真机慢约 3 倍）：

| 上限 | 最差单帧 | 总和 |
|------|---------|------|
| 无（BUG-472 改成的） | **24 286 ms** | 40 106 ms |
| 4000 | 10 324 ms | 42 941 ms |
| **2000（原本发布的）** | **7 382 ms** | **48 235 ms** |
| 1000 | 4 820 ms | 55 467 ms |
| 500 | 3 041 ms | 69 962 ms |

翻倍那一行的逐帧序列是：`138, 527, 1074, 2648, 7163, 24286, 4032, 238`。
**一帧冻住 24 秒。**

### 成本模型（从这组数里反推）

- 遍历：每个**已画**块每帧约 **0.12 ms**；
- 构造：每个**新增**块约 **1.7 ms**（十四倍）。

于是总时间 ≈ 构造 N 个块（25 000 × 1.7 ≈ 42 s，**不可避免**）
＋ 0.12 × Σ(已画块数)。步长只决定后面那一项加多少，以及那 42 秒被切成什么形状的帧。

**这条曲线上没有免费的改进**：每一档减少卡顿都要拿总时间换，单调。
原本的 2000 本来就在曲线上。

### 还有一条更要紧的结论

那个 0.12 ms 是**地板**：25 000 块时，任何一趟在加任何东西之前就已经要花三秒。
**所以没有任何步长能让这么大的文档流畅补画**——这是「块住在一个非惰性 Column 里」
的性质，`_blockWidgets` 的注释写了为什么它是非惰性的（屏幕外标题没有 `GlobalKey`
就跳不过去；`SelectionArea` 够不到没建的孩子）。

### 修复

行为**回到 2000**，即 v1.6.2 发布的样子。保留的是 BUG-472 真正有价值的那部分：

- 那个裸常量现在叫 `maxBatchSize`，头上写着完整的成本模型和上面那张表；
- `nextRenderedCount` 仍然是一个具名、可测的函数；
- 守卫换成了**真正要守的性质**：「没有哪一趟建的块超过上限」（这条就是那 24 秒的帧），
  外加「小于上限的文档一路翻倍」（上限不许让常见情形付代价）
  和「重复遍历总量是文档的小倍数」。

对外的说明也撤了：发行说明里那条「1 MB 文档补画快了三倍」是错的，已删除；
BUG-472 的人工测试步骤也删了，因为行为没有变化。

### 涉及文件

- `code/lib/ui/editor/markdown_renderer.dart`
- `code/test/ui/editor/the_preview_fills_in_bounded_steps_test.dart`（替换掉
  `the_preview_fills_in_logarithmically_test`，后者断言的是被推翻的那个性质）
- `docs/v1.6.3/release-notes.md`、`docs/v1.6.3/manual-test.md`（撤回对外说明）

### 验证

| 变异 | 结果 |
|------|------|
| 去掉上限（BUG-472 的做法） | 红，「25368 块的第 7 趟一次建了 3200 个块」 |
| 固定步长 2000（不先翻倍） | 红，小文档不再一路翻倍 |
| 上限降到 500 | 红 ×2（小文档付了代价 + 重复遍历超标） |

**复现那组逐帧数据的办法**（不进测试套件，太慢——25 000 块一轮约 90 秒）：
写一个 widget test，`pumpWidget` 之后
`await tester.runAsync(() => Future.delayed(const Duration(seconds: 8)))`
让整篇解析落地，然后循环 `tester.pump(16ms)` 并用 `Stopwatch` 记每一帧。

## BUG-476：教这件事的那个文件自己没被对账
...
`.claude/CLAUDE.md` 会在每次会话开头被读进去，而且它自己写着「这些指示**覆盖**任何默认行为」。
它有一整节讲「一份清单对外宣称、另一份是实现、没人比较它们」，还列了六个实例。

**而它自己没有任何守卫。** 它开头就承认过：「这里不写死——写死过一次，落后了五个小版本」。

逐条核对它里面可机械检查的断言，三处不成立：

| 断言 | 实际 | 后果 |
|------|------|------|
| `AppThemeTokens` 包含 **14** 个颜色 token | **16** 个字段（15 个颜色 + `brightness`） | 照它读代码的人会以为少了两个 |
| 「核实办法：`grep 'case DiagramType\.' …`」 | 这条命令数出 **46** | **配方核不了它要核的那个数**——照着做的人会以为「22 种」是错的 |
| SDK 界面上限「今天一致，明天没人管」 | 已经有人管了（`sdk_schema_agrees_test`，连阿拉伯数字都处理） | 派下一个人去建一个已经存在的守卫 |

其余引用**全部准确**：3 个测试名都在，所有文件路径都在，
两处「不存在」的断言（`scripts/release.sh`、`PluginResultPanel`）也都成立。
8 个主题、12 种语言也对。

那个配方为什么是 46：同一个 `DiagramType` 出现在好几个 switch 里。
正确数法是 `DiagramType.values` 去掉 `unknown`——**而这件事已经有人在做**
（`readme_counts_test`），所以文档里现在指向那条测试，而不是再给一条自己的命令。
**文档里的一条数法就是这个数的第二份实现。**

### 修复

三处改正，加上守卫 `the_project_memory_counts_what_the_code_has_test`，查五件事：

1. 图表类型数（从 `DiagramType.values` 去掉 `unknown` 数）；
2. 主题数（从 `AppTheme.themeNames`）；
3. 主题字段数（从 `AppThemeTokens` 的源码数，加一个就自己动）；
4. 语言数（数 `.arb` 文件）；
5. 它提到的每个文件与每个测试都存在，**并且它说不存在的两样确实不存在**。

只查得了的部分。那份文件大半是判断，那才是它值得存在的原因，也是任何测试都守不住的部分。

**找不到文件时跳过而不是失败**：只 check out `code/` 是有人会做的事，
一条在那里必然红的守卫只会教人忽略它。

### 涉及文件

- `.claude/CLAUDE.md`
- `code/test/the_project_memory_counts_what_the_code_has_test.dart`（新增 5 条）

### 验证

| 变异 | 结果 |
|------|------|
| token 数改回 14（即缺陷原状） | 红 |
| 图表数改成 21 | 红 |
| 主题数改成 9 | 红 |
| 指一个不存在的文件 | 红，点名那个路径 |
| 指一个不存在的测试 | 红，点名那个测试 |
| 让 `PluginResultPanel` 复活 | 红，点名那个文件 |

## BUG-477：代理写进去的东西撤不回来
...
### 怎么发现的：读 BUG-470 那条分支的兄弟

BUG-470 修的是「预览里的编辑撤不回来」。**同一个问题的兄弟是「还有谁在写文档内容」。**
`grep updateContent(` 数出五个写入者，逐个对照「有没有压还原点」：

| 写入者 | 压还原点？ | 判定 |
|--------|-----------|------|
| `plugin_apply`（采纳插件改写） | ✓ | 对 |
| `plugin_command_actions`（插件命令结果） | ✓ | 对 |
| `home_screen.onContentChanged`（编辑窗格汇报） | 由上游压 | 对——源码窗格自己压，预览走 BUG-470 那条 |
| `app_menu_bar.stepHistory`（撤销/重做把结果写回） | ✗ | **对**，它就是撤销本身 |
| **`mcp_provider` 的 `set_content`** | **✗** | **缺陷** |

于是代理改写了文档，读者按 Ctrl+Z——要么**什么也不发生**（历史是空的），
要么**退到读者上一次自己打字的状态**，跨过了代理那一步。

**读者没有打这些字。撤销本来就是为了拿回不是自己做的事。**

### 修复

已有的方法就是干这个的，只是名字是预览专属的。改名对齐既有词汇：

```
recordPreviewEdit(id, next)  →  recordExternalEdit(id, next)
```

`updateContent` 早就有 `external:` 这个参数表示「不是有人在打字」，
而**那些写入正是没人替它们记还原点的写入**。`set_content` 分支在写之前调它。

没有抄第二份「把当前内容压成还原点」的代码——那是这个仓库一直在删的形状。

### 涉及文件

- `code/lib/providers/tab_provider.dart`（改名 + 说明为什么不限于预览）
- `code/lib/providers/mcp_provider.dart`
- `code/lib/ui/editor/split_editor.dart`、`code/lib/ui/screens/home_screen.dart`（跟着改名）
- `code/test/providers/what_an_agent_wrote_can_be_taken_back_test.dart`（新增 2 条）

### 验证

| 变异 | 结果 |
|------|------|
| 去掉 `set_content` 前的那次记录（即缺陷原状） | 红，「撤销没有东西可退——还原点没被压进去」 |
| 去掉 `setHistoryTab(id)` | 红（历史不知道是哪个标签页） |
| 去掉「内容相同就不记」那道闸 | **绿，等价变异** |

最后一条如实写进了测试文件：`undo` 要有两条记录才肯退，`pushHistory` 又会丢掉与栈顶
相同的快照——**两张网盖一个洞**，我试过的每种排列在去掉那道闸后行为都一样。
所以没有为它写测试：一条无论代码在不在都通过的测试比没有测试更糟。

## BUG-478：连按替换，一次撤销退掉一整串
...
BUG-477 是「五个内容写入者里谁压了还原点」。同一条不变量在**源码窗格内部**还有一批兄弟：
程序化的批量编辑。`grep pushHistory lib/ui/editor/source_editor.dart` 数出三处程序化编辑：

| 批量编辑 | 先压快照？ |
|---------|-----------|
| `_moveBlock`（整块上下移动） | ✓ |
| 表格编辑（插入/删除行列） | ✓，注释还写着理由 |
| **查找替换的两个按钮** | **✗** |

那条注释说的话，逐字适用于替换：

> A snapshot first: **the whole table is reformatted, so a single undo has to
> take the reader back to what they had.**

### 根因

两个替换按钮直接写编辑器的控制器——也就是**打字那条路**，而那条路的历史是
**300ms 防抖**的。于是比 300ms 更快的一串替换只会进栈一条，而且是**最后那一个状态**：

- 连按 Replace 十次再 Ctrl+Z → **十处全退**；
- 「全部替换 a→x，接着全部替换 b→y」→ **一次退掉两轮**。

而 `_replace` 自己的注释写着「**反复按 Replace 会一路走下去**」——
**那正是设计中的用法，也正是这个窗口开着的时候。**

### 修复

按同文件既有写法，写控制器之前先压替换前的文本：

```dart
_closeTheUndoStep(text);
widget.textController!.value = TextEditingValue(text: newText, …);
```

`pushHistory` 会丢掉与栈顶相同的快照，所以「按了但没改变任何东西」不会留下噪声。

### 涉及文件

- `code/lib/ui/widgets/find_replace_bar.dart`
- `code/test/ui/widgets/one_replacement_is_one_undo_test.dart`（新增 3 条）

### 验证

| 变异 | 结果 |
|------|------|
| 两处都不压（即缺陷原状） | 红，「一次退回了两处替换」 |
| 只有单个替换压，全部替换不压 | 红，「两次全部替换被并成了一步」 |
| 压的是替换**后**的文本 | **绿，等价变异** |

最后一条如实写进了测试文件：没有防抖在场时，前后两种压法**可证明等价**
（每一次压都封了一步，这一次记的值正是下一次会记的值）。
差别只在防抖窗口里——300ms 内打的字还没被记录，只有「替换前」那一版能保住它。
要测出来需要真的 `SourceEditor` 和 find bar 同处一树，而那两者的
`rawContent` 只有在同时构建它们的那个界面里才同步。
**写下来，而不是塞一条做不实的测试。**

## BUG-479：打开一份大文档，状态栏在 1200 像素就溢出
...
### 怎么发现的：写 BUG-480 的测试时撞上的

给「纯预览模式不该说高亮已关闭」写测试，一开一份 129 KB 的文档，
测试立刻报 **`A RenderFlex overflowed by 49 pixels on the right`**——**窗口 1200 像素宽**。

而这个仓库**有一条专门防它的测试**：`narrow_window_test`，五个宽度 × 三条 bar，
理由写得很清楚（「窗口没有最小尺寸，每个宽度都是有人能拖到的」）。

它测不到，因为它**pump 的是一个没有打开任何标签页的 bar**。
「语法高亮已关闭」只在文档超过 128 KB 时出现，于是那一格在那条测试里**永远是假的**，
宽度阶梯就是在没有它的情况下调出来的。

> **一条防灾难的测试，搭台搭在了一个读者只会路过、不会停留的状态上。**

### 修复（两处）

**一、标签的宽度要算进阶梯**。它是**被翻译的一句话**，十二种语言各不相同，
所以宽度是**量出来的**（`TextPainter`，和 bar 自己要用的那支一样），不是猜一个常量：

```dart
final explanation = highlightOff ? l10n.statusHighlightOff : null;
final taken = explanation == null ? 0.0 : _widthOf(explanation, style, context) + _dividerWidth;
final width = constraints.maxWidth - taken;   // 阶梯用这个宽度
```

于是标签在场时，最没用的那几个计数**提前让位**——这正是那段注释自己写的原则。

**二、极窄时标签自己也要让位**（`width >= 300`）：
到了连字数都要绕着它排的宽度，**一条黄黑条纹解释不了任何事情**。

### 涉及文件

- `code/lib/ui/widgets/status_bar.dart`
- `code/test/ui/widgets/narrow_window_test.dart`（15 条 → 30 条，每个宽度多一轮「有文档打开」）

### 验证

| 变异 | 结果 |
|------|------|
| 标签宽度不算进阶梯（即缺陷原状） | 红，**1200px 溢出 49px** |
| 极窄时标签不让位 | 红，**700px 溢出 40px** |

### 我自己又踩了一次同一个陷阱

第一版新增的那一轮**仍然测不到**——因为 `AppConfig()` 的默认 `editMode` 是 `preview`，
而 BUG-480 刚让这个标签只在有源码窗格时出现。所以那一轮里标签照样不显示，两个变异全绿。

**加一轮测试不等于那一轮走进了要测的那条路。** 现在「有文档」那一轮显式用 `EditMode.split`，
理由写在 override 旁边。

## BUG-480：纯预览模式下说了一句关于不存在的窗格的话
...
顺着「**编辑器说了与事实不符的话**」这条镜子扫状态栏——它每一格都是一句关于文档的断言，
而且每一格旁边都记着一段「它以前说的是假话」的历史（编码写死 UTF-8、换行写死 LF）。

`highlightOff` 那一格只看内容长度，**不看当前是什么模式**。而纯预览模式
**压根不建源码编辑器**（`DeferredEditorBuilder` 只建屏幕上那一种），
没有高亮，也就没有高亮可失去。读者在预览模式打开一份 200 KB 文档，
会看到「语法高亮已关闭」——**一句关于不在屏幕上的那个窗格的话**。

### 顺带：一个没人读的访问器指出了规则被抄在哪

`HighlightingController.isHighlightingSuspended` 全库**只有它自己的定义**，
零个读者——而状态栏把这个判断又写了一遍。这正是「没人调用的集中式 helper
指出了规则被抄散在哪」那条。

**但这一次不改成去问它**，理由写在条件旁边：状态栏用
`activeTabProvider.select(内容长度 > 上限)`，这个选择器**恰好在答案翻转时才触发重建**；
去读控制器会让状态栏**每次击键都重建**，除非再为它加一个 provider。
阈值用的是高亮器自己的常量，所以两者不会漂移。那个无人读的访问器已删除。

### 涉及文件

- `code/lib/ui/widgets/status_bar.dart`
- `code/lib/ui/editor/highlighting_controller.dart`（删掉无人读的访问器）
- `code/test/ui/widgets/highlighting_off_is_said_where_it_is_true_test.dart`（新增 4 条）
...
### 这一条是 BUG-481 末尾写下的那件事

上一轮结尾记着：「`source_editor.dart` 里还有 29 处直接写控制器，只有 7 处压了还原点」，
并且明确说**不要在那一轮顺手动 29 处**，值得单独一轮并配一条穷尽的守卫。这就是那一轮。

### 先分类，再改

实际是 **31 处**。分类的关键不是「改不改文本」（31 处里 30 处都改），
而是**「命令式的」还是「打字顺带的」**：

| 类 | 例 | 该不该断开撤销步 |
|---|---|---|
| **命令** | 加粗、标题、缩进/反缩进、清除格式、复制行、插入块/链接/图片/front matter、表格编辑、移块、切换松散列表、斜杠菜单、语言选择器、剪切、粘贴 | **该**——一次 Ctrl+Z 要退回一个命令 |
| **打字** | 自动配对的插入与删除、回车续列表、跳过已有的引号、`didUpdateWidget` 带进来的外部文本 | **不该**——每个按键一个步会让撤销变成逐字 |
...
原先自己压了还原点的 4 处（剪切、粘贴、移块、表格编辑）也改走它——**一件事一个做法**。
打字那 6 处保持直接写，并且**在守卫里写明了为什么**。

### 守卫：新增一处写入不分类就红

`every_controller_write_is_classified_test`：扫 `source_editor.dart` 里每一处
`_controller.value = `，取它所在的方法名或 switch 分支名，要求这个名字要么是入口本身、
要么在 `typing` 名单里（每条带理由）。

两条断言：

1. **每处写入都在线的一侧**——新写一个方法直接写控制器，会被点名到方法名；
2. **入口先记录后写入**——顺序反了就等于记下了改完之后的状态，那不是可以退回去的那一版。

### 行为测试：连做两次，从不做一次

`two_commands_come_back_one_at_a_time_test`，四条：两次加粗、两个标题、两次复制行、两次移块。

**每条都是做两次。** 做一次是测不出来的：编辑器在 `initState` 里把初始内容压进了栈，
`undo` 又会把屏幕上的当前状态补上去再退——所以单个命令无论压不压都能退回来。
第二次才落在「防抖还没关上第一步」的那个缝里。（这条教训昨天在 BUG-481 里学到，这次直接用上了。）

### 涉及文件

- `code/lib/ui/editor/source_editor.dart`（25 处改走入口 + 入口 + 去掉 4 处重复压栈）
- `code/test/ui/editor/every_controller_write_is_classified_test.dart`（新增 2 条）
- `code/test/ui/editor/two_commands_come_back_one_at_a_time_test.dart`（新增 4 条）
- `code/test/ui/widgets/menu_copy_rich_guard_test.dart`（钉法跟着改，见下）

### 验证

| 变异 | 结果 |
|------|------|
| 入口不再记录（即 25 处原本的状态） | 红（行为测试 + 守卫同时） |
| 先写后记 | 红，「先写后记等于记下了改完之后的状态」 |
| 把 `_setHeadingLevel` 改回直接写控制器 | 红，守卫点名到 `{_setHeadingLevel}` |

### 一条守卫的钉法第二次跟着实现改

`menu_copy_rich_guard_test` 昨天刚从「钉在 app_menu_bar.dart 里」改成「钉在每处调用上」。
今天它又红了：它要求 `cut` 分支里有 `pushHistory`，而记录搬到入口里去了。
...
## 无编号：「它在文档之外做了什么」剩下的两个轴，扫过，干净

2026-09-14。FEAT-159 的问法是**「代码做了什么、而读者可能不知道」**，
第一个轴（网络）找到了那个关不掉的更新检查。剩下两个轴这一轮扫完，**都干净**。

### 轴二：它往磁盘写什么，有界吗

| 写入者 | 上限 |
|--------|------|
| `PluginLogger`（每个插件一份） | **512 KB + 一份轮转**，超了就 rename 成 `.1` |
| `AppLog`（应用自己的） | **内存里 500 条环形**，不写盘 |
| 安装程序的日志 | **只取最后 3 行**，注释写明「Inno 的日志是几千行逐个文件复制，那不是读日志的人想看的」 |

**顺手用自己的镜子照了一次**（[[bound-the-thing-that-grows-not-the-count]]）：
`AppLog` 按**条数**限，那 500 条 × 每条不限长＝字节无界。
于是逐个看了全部 **16 处**调用——没有一处把文档内容、插件输出、AI 答案写进去，
全是「名字 + 数字」式的构造消息，最长的是 `$error`，而 Dart 的 `FormatException`
自己就会把源文本截窗。**按条数限在这里是够的，因为每条的长度本来就有界。**

### 轴三：它会启动什么外部程序 / 打开什么链接

`Process.start` / `Process.run` / `launchUrl` 共 18 处，逐个定性：揭示文件（`open`/资源管理器）、
装更新（读者或 `update_app` 触发）、`ps -p <pid>` 查自己留下的插件子进程、
编译型插件宿主（刻意休眠）、帮助菜单的三个链接。**没有一条是内容驱动的**——
除了从文档里点开的链接，而那一条：

- **scheme 用允许名单**（`http` / `https` / `mailto` / `tel`），
  拒绝 `javascript:` / `vbscript:` / `data:`；
- 连 **`java\nscript:` 这种「scheme 里塞空白绕过朴素检查」**都有测试；
- 指针悬停时预览底部会显示目标地址（点之前就能看见去哪）。

判定逻辑住在 `link_target.dart` 里、和「去做这件事」分开，有 `link_target_test`
——它的注释还记着当年为什么要分开：判定曾经埋在处理函数里，
于是**锚点那一类漏了很久**（`[see below](#conclusion)` 落到路径分支、当成文件找不到、静默无反应）。
...
## 无编号：BUG-482 那条分类，「不该断开」的那一半原来没有行为守卫

2026-09-14。核实自己在 BUG-482 里的断言——「打字有意保持不变」——**发现它没有证据**。

那次把 25 处命令式写入收敛到 `_writeAsOneStep`，并刻意留下 6 处按键写入
（自动配对的插入与包裹、退格删配对、回车续列表、跳过已有右引号、`didUpdateWidget`）。
两半的守卫情况是不对称的：

| 半边 | 守卫 |
|------|------|
| 命令**要**断开撤销步 | `two_commands_come_back_one_at_a_time_test`（行为） |
| 按键**不该**断开 | 只有 `every_controller_write_is_classified_test`（**看代码形状**） |

**而形状守卫抓不到这个方向**：把某处按键写入接进 `_writeAsOneStep`——那正是
「看起来像做对了」的改法——那个名字**在它的允许名单里**，于是守卫变绿，
而撤销静默变成逐字。`autopair_test` 里 `undo` 出现 **0 次**，所以这一半此前一点行为证据都没有。

### 补上的守卫

`typing_is_not_one_undo_per_key_test`，四条，一处按键写入一条。

**搭台踩了两个坑，都写进文件了：**

1. **widget test 里打不出普通字母。** 编辑器不处理某个键时是框架插入字符，
   而 widget test 不模拟那一步（`autopair_test` 自己也这么写）。
   所以普通字母**经控制器**写入（那正是框架插入所走的路），只有编辑器**自己处理**的键才模拟。
2. **每条都必须先打一个字母、再按那个键。** 不先打的话，要退回去的那个状态
   **本来就在栈上**（编辑器在 `initState` 里压了文档初始状态），多压一次毫无差别、
   变异是等价的。先打一个字母把栈顶变旧——**而那正是读者「打字打到一半」时的真实处境**。

### 验证：四处逐个变异，各自只红自己那一条

| 把哪一处接进 `_writeAsOneStep` | 红的是 |
|---|---|
| 回车续列表 | 「续列表那一下和它前面打的字被分成了两次撤销」 |
| 退格删配对 | 「配对与删除各自断开了撤销步，一次输入要按三次撤销」 |
| 自动配对插入 | 「配对那一下自己断开了撤销步」 |
| 打配对键包裹选区 | 「包裹那一下自己断开了撤销步」 |

剩下两处刻意不在范围内，理由写在文件里：`didUpdateWidget` 是外部文本进来
（归标签页的历史，另有两条测试守着）；「跳过已有右引号」只移动光标、不改文本，
没有可退回的东西。
...
### 端到端验证：通过（2026-09-14 17:23）

CI 造出带修复的构建之后，在读者机器上重跑了一次 `update_app`，
**全程没有任何手动操作**：

```
17:23:07  安装程序已启动（update_app 返回）
17:23:16  新进程写下第一行日志        ← 9 秒，自己回来的
17:23:22  端口可连
```

日志是**全新的**（内存日志随进程清空，只剩启动行），证明确实是新进程。
而安装程序自己的收尾行这次多了一句：

```
Filename: D:\MarkText Plus\marktext_plus.exe | Deinitializing Setup. | Log closed.
                ↑ Inno 在记录那条 [Run] 启动指令被执行了
```

**上一次这三行是**「Need to restart Windows? No | Deinitializing Setup. | Log closed.」
——**没有这一行**，因为 `skipifsilent` 把它跳过了。这句话的有无，就是这个缺陷的有无。
...
## BUG-498：同一个双击，记不记得住取决于编辑器当时开着没开着
...
为了给自动化接口加「打开一个已存在的文件」，先去找这件事现在由谁做
——**结果找到四份实现**，并排一看就露了：

| 实现 | 去重 | 先出占位标签 | 进「最近文件」 | 读失败时 |
|---|---|---|---|---|
| 侧栏 `_openFileInTab` | ✓ | ✓ | ✓ | `failTabLoading` |
| 菜单 `AppMenuBar.openFile` | 靠 `addTab` | ✗（读完才建） | ✓ | `reportOpenFailure` |
| **`openFilesFromSecondInstance`** | ✓ | ✗ | **✗** | 静默 |
| 启动参数 `_openStartupFiles` | — | ✓ | ✓ | 移除标签 + 上报 |

第三行是**应用已经在运行时双击文件**走的那一条。于是：同一个双击动作，
**编辑器开着就记不住、没开着就记得住**。

### 修复

给它补上 `addRecentFile`，与其余三条一致。

### 顺带（不是修复，是记下来）

「把一个路径打开到当前窗口」有四份实现，差异不止这一项（占位标签、读失败的收尾
都不一样）。**它们该收敛成一份**，但那是一次涉及四个调用点的改动，不该和这一条
缺陷混在一起做——所以差异写在上面那张表里，等有人专门做它，而不是等下一次被撞到。

自动化接口的 `open_file`（FEAT-165）**没有再写第五份**：它复用这一条，
并多带一个 `forceThisWindow`（理由见那里）。

### 守卫

`second_instance_test`「a file opened this way is remembered in Recent Files」。
变异（去掉那一行）立刻红。
...
| 日期 | 2026-09-24 |
| 优先级 | **P1** |
| 状态 | 已修复，**真机待确认**（这条改的是 Windows runner，本机既编译不了也测不了） |

### 我先测错了东西

读者报的是「**打开第二个文件**卡」和「**关闭窗口**卡」。
而我 BUG-497 那一轮量的是 `activate_tab`（在两个**已经打开**的标签页之间切换）
和 `set_content`——**两样都不是他说的动作**，窗口关闭更是一次没测过。
拿代用品量完就下结论，是这一轮最该记住的错。

用真正的动作重量（`open_file` 走的正是双击时走的那条 `openFilesFromSecondInstance`）：
...
**一点不卡。** 而这恰恰指出了问题：MCP 这条路**绕过了**双击才有的那一段。

### 根因：单实例检查在 Dart 里，而 Dart 要等引擎起来

双击一个文件时 Windows 会**启动第二个进程**。它必须：

```
462 ms   进程创建 → runner 入口（Windows 映射 exe 和各 DLL）
~600 ms  引擎启动、加载 AOT 快照
   ↓
然后才轮到 Dart 的 WindowsSingleInstance.ensureSingleInstance 发现
「已经有一个实例了」，把路径从命名管道转发过去，自己退出。
```

**一秒多的整份启动，只为了转发一个路径然后退出。** `windows/runner/main.cpp` 里
原本**没有任何**提前拦截——数字是读者机器上启动追踪里现成的。

### 修复：把检查搬到引擎启动之前

用的是那个包自己的协议：命名互斥体 `marktext_plus_instance.win.mutex` 说明
「有人在」，参数走命名管道 `\\.\pipe\marktext_plus_instance`，内容是 JSON 数组。

三条决定写进了代码注释：

- **只 `OpenMutex` 探测，绝不创建。** 创建了的话，同一个进程里稍后那次 Dart 检查
  会认为自己是第二实例，于是把**唯一在跑的那一份**关掉；
- **任何一步失败都落回原路**（互斥体不在、管道还没监听、写失败）——
  这条路只能让启动更快，不可能让启动失败；
- **转义没把握就放弃。** 只处理反斜杠和引号（路径正常只需要这两个），
  遇到别的需要转义的字符就返回空、走旧路径。理由是失败形态不对称：
  对端解析失败会把错误吞掉，**文档就静悄悄打不开**；
  而「慢得和以前一样」是个好得多的失败。

### 关于「关闭窗口卡」

**这一条我没有修，因为证据不支持我去改它。** 读者机器上追踪里三次关闭：
...
而且**关闭看门狗一行都没写过**（它在 `destroy()` 挂住时每 100 ms 写一行）。
退出后的拖尾早先已经用 `ExitProcess` 处理过（见 main.cpp 里那段注释）。

所以从「Dart 收到通知」往后全都是快的，能藏住几秒的只剩
**「点击」到「Dart 收到通知」之间**——而那一段变慢只有一个原因：**UI 线程正忙**。
上面那个「第二个进程整份启动」恰好是让它忙的东西，所以这条修复**可能**一并缓解它。

要确认还需要读者回答一个问题：**点 × 之后，是窗口在屏幕上赖着几秒，
还是窗口立刻消失、而程序还在后台？** 两种答案指向完全不同的两半。

### 本机验不了的部分，说清楚

`windows/runner/main.cpp` 在这台机器上**既不能编译也不能测试**（连 C++ 编译器都没有）。
所以：

- **编译错误**由 CI 的 Windows 任务兜住（该 runner 用 `/W4 /WX` 构建，警告即失败）；
- **行为**只能在读者机器上确认：装上之后双击一个 `.md` 文件，应当**立刻**打开。

为降低风险，上面那条「转义没把握就放弃」把最容易出错的分支整个删掉了。

### 涉及文件

- `code/windows/runner/main.cpp`
...
卸载 MarkText Plus 之后，安装文件夹还在原处，里面躺着一两个 `.log`。
删掉一个编辑器却发现它的文件夹还杵在那儿的人，有理由怀疑它还留下了些别的什么。

### 根因分析

程序**运行时**会在自己的可执行文件旁边写诊断文件：

| 文件 | 谁写的 | 为什么写在这儿 |
|------|--------|---------------|
| `startup-trace.log` | Dart（`StartupTrace`） | 启动慢要能在读者机器上看 |
| `last-exit.log` | runner（FEAT-167） | 关闭耗时，Dart 够不着的那两段 |

**这两个都不是安装器装的**，所以 Inno 的卸载器压根不知道它们存在，
而 Inno 只删自己装过的东西——剩一个陌生文件，整个目录就留下了。

这属于「一份清单对外宣称，另一份是实现，没人比较它们」：
安装器的清单里没有它们，而写它们的代码在另一边，两边谁也不看谁。

### 修复方案

两个 workflow 生成的 iss 里都补 `[UninstallDelete]`。**两个都补**——
先前有过一次守卫只查 `ci.yml`（那个安装包没人装）而放过 `release.yml`
（真正被安装的那个）的教训。

### 守卫与变异验证

`uninstalling_takes_away_what_the_app_wrote_test`：
**文件名从写它们的源码里读出来**（Dart 的 `pathSeparator}xxx.log`、
main.cpp 的 `L"xxx.log"`），再去两个 workflow 里核对——
这样改名字时守卫跟着改名走，而不是留在原地继续绿。

| 变异 | 结果 |
|------|------|
| 只在 `release.yml` 补，`ci.yml` 漏掉 | 红 |
| 只清一个文件，另一个忘了 | 红 |
| 改了写出的文件名，卸载那边没跟上 | 红 |
...
- `code/test/utils/uninstalling_takes_away_what_the_app_wrote_test.dart`（新增）

---

## BUG-502：关窗口之后，窗口还在屏幕上站着

### 现象

读者第三次说「关闭应用窗口还是很慢，严重卡顿」。

### 根因分析（这次是量出来的，不是想出来的）

装上 FEAT-167 的计时之后，读者机器上的真实一次关闭：

```
[close] the previous close: 0 ms for the button to be heard,
        128 ms from being heard to the last instruction this process ran
```

两件事：

1. **0 ms**——点击立刻被收到，UI 线程没堵。「窗口不响应」这条排除。
2. 这句话用的是**退化措辞**，因为 `gone-ms` 报了 **-1**。

第 2 条才是答案。`-1` 不是测量失败，**是实话**：

```cpp
void WindowManager::Destroy() {
  PostQuitMessage(0);       // window_manager 0.4.3
}
```

`windowManager.destroy()` **根本不销毁窗口**，它只往队列里投一条退出消息。于是：

| 时刻 | 发生什么 | 读者看到 |
|---|---|---|
| 0 ms | 点击被收到 | 窗口在 |
| ~24 ms | Dart 存几何、停监视 | 窗口在 |
| 128 ms | 消息循环退出，进程最后一条指令 | **窗口还在** |
| 之后数秒 | Windows 拆进程：卸载 51 MB 安装的每个 DLL、回收工作集 | **窗口一直站在那里，什么也不干** |

追踪里那句 `window destroyed` 也是假的——它记的是 `destroy()` 调用返回，不是窗口消失。
**这个编辑器对自己说了一句与事实不符的话，而它正好盖住了读者抱怨的那件事。**

### 修复方案（两半，缺一不可）

**第一半**：消息循环一结束就 `ShowWindow(handle, SW_HIDE)`。
藏起来而不是销毁是刻意的——`DestroyWindow` 会拆 Flutter 视图，
那恰恰是这条修复想从读者面前挪走的活；该落盘的东西在循环结束前就写完了。

**第二半**：`TerminateProcess(GetCurrentProcess(), 0)` 取代 `ExitProcess`。

只做第一半是**遮症状，而且会造出一个新的坑**——这一点是读者问出来的：

> 「如果我关闭窗口，然后又马上打开一个文件，会有问题吗？」

会。窗口瞬间消失等于告诉读者「关完了」，而旧进程还在拆的那几秒里
**仍然持有单实例互斥量和那条命名管道**，只是已经没有活着的东西在读了
（管道服务端跑在一个已停的 Dart isolate 里，句柄要到进程销毁的最后一步才关）。
于是新启动会：发现「已有实例」→ 把路径写进管道 → 自己退出 →
**文件永远不会打开，而且一声不响**。

这个竞态本来就存在（Dart 侧的检查同理），但把窗口藏掉会让读者**更容易**撞上它。
立即结束进程同时解决两件事：读者眼里关得干净，互斥量与管道随进程一起消失，
竞态窗口从「几秒」缩到可忽略。

**代价照实说**：`TerminateProcess` 跳过所有 DLL 的卸载钩子，
某个库若打算在那里刷新什么，它刷不成了。

**「这里没有东西需要刷新」是查过的，不是声称的**：

| 问题 | 查法 | 结果 |
|---|---|---|
| 有没有用户态缓冲的写入 | `grep -rn "openWrite\|IOSink" lib/` | **一处都没有** |
| 那都用什么写 | 逐个数写盘者 | 全是 `writeAsString`/`writeAsBytes` 系列，返回时数据已交给操作系统 |
| 日志会不会丢尾巴 | 读 `AppLog` | 内存里 500 行的环形缓冲，**本来就不落盘**，任何退出方式都留不下它 |
...
与原先的「几秒」不是一个量级，而**彻底消除它要改的是单实例握手本身**
（让转交方确认对面还活着），那是另一件事，没有在这条里顺手做。

### 守卫与变异验证

| 断言 | 变异 | 结果 |
|---|---|---|
| 窗口在结束之前被藏起来 | 删掉 `ShowWindow` | 红 |
| 全文件不许再出现 `ExitProcess` | 把结尾改回 `ExitProcess` | **红 3 条** |
| 结束用的是 `TerminateProcess(GetCurrentProcess()` | 同上 | 红 |

---

## BUG-503：最小化时关闭，窗口位置被存成「不在任何地方」

### 现象

读者：「打开时窗口直接跑到了屏幕外面去了，找都找不到了。」

### 根因分析

`_saveWindowGeometry` 无条件读取窗口的尺寸与位置。而**最小化的窗口不在任何地方**——
Windows 给它报的是停放角落（-32000,-32000）和几十像素的尺寸。这被当作「窗口在哪」写进了配置。

触发它的正是这次更新：**安装器在窗口处于最小化状态时关掉了编辑器**。
真机取证时 `get_state` 两次都报 `{"state":"minimized","width":158,"height":26}`。

### 修复方案

最小化时**保留已存的几何**——那是窗口最后一次「确实在某处」时的位置，正是这份记录存在的意义。

顺带把数字写进追踪（`window bounds read: WxH at X,Y (minimized)`、
`window placement decided: stored … → opening …`）：
这次为了弄清「存下来到底是什么数」多跑了一趟，下次一眼就能看完。

### 为什么恢复那一侧没能救回来

`WindowPlacement.fit` 本来就该把落在所有屏幕之外的窗口拉回中央，
新增的测试证明**它确实能**（在未修改的旧代码上就是绿的）。
所以这两件事是两个缺陷而不是一个：存储侧存了一个从来不存在的位置。
恢复侧为什么仍然失手，要等新追踪里的真实数字——已经埋好，下次启动自证。

### 守卫与变异验证

- `window_placement_test`「a window parked where Windows parks minimized ones
  comes back」：旧配置还躺在读者机器上，这一半必须继续成立；
- `a_minimized_window_is_not_where_the_window_is_test`：源码守卫，查「问了」
  且「问在写之前」——**问完照样存，是同一个缺陷前面加了一句问话**。变异（删掉那个提前返回）红。

### 涉及文件

- `code/lib/ui/screens/home_screen.dart`、`code/lib/main.dart`
- `code/windows/runner/{main,utils,flutter_window}.cpp`、`utils.h`
- 三份测试（新增一份、改两份）
