...
**但每一趟补画重建的是已经画出来的全部块，不只是新增的那些。** 一帧的代价等于
`rendered`，与这一帧加了多少块无关。所以给步长封顶：

- **一帧也没变短**——帧长由 `rendered` 决定，跟 `step` 无关；
- **长帧变多了**，而且多的数量随文档增长：

| 文档 | 封顶 2000 | 翻倍 |
|------|----------|------|
| 25 368 块 | 19 趟，198 918 次块构建（文档的 7.8 倍） | 10 趟，50 918 次（2.0 倍） |
| 50 736 块 | 31 趟，682 686 次（**13.5 倍**） | 11 趟，101 886 次（**还是 2.0 倍**） |

翻倍策略的总工作量永远约等于文档的两倍；封顶策略是「文档的多少倍」本身在随文档
长大——这就是那个平方项。**最坏的一帧两种做法完全相同**：最后一趟，重建全部。
